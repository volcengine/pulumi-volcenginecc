...
.PHONY: build development only_build build generate generate_sdks build_sdks install_sdks
...
.make/generate_java: .make/install_plugins bin/pulumi-java-gen .make/schema
	...
	cp -f gralde.file ./sdk/java/build.gradle
	cp -f settings.gradle.file ./sdk/java/settings.gradle
	@touch $@
...
.make/build_python: .make/generate_python
	cd sdk/python/ && \
		...
		sed -i.bak -e 's/^VERSION = .*/VERSION = "$(PROVIDER_VERSION)"/g' ./bin/setup.py && \
		rm ./bin/setup.py.bak && \
		cd ./bin && \
...
# `cmd/pulumi-resource-volcenginecc/schema.json` is valid and up to date.
