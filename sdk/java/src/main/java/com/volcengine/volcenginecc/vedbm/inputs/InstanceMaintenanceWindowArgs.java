// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.volcengine.volcenginecc.vedbm.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import java.lang.Integer;
import java.lang.String;
import java.util.List;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class InstanceMaintenanceWindowArgs extends com.pulumi.resources.ResourceArgs {

    public static final InstanceMaintenanceWindowArgs Empty = new InstanceMaintenanceWindowArgs();

    /**
     * Maintenance cycle granularity. Default value: Week.
     * 
     */
    @Import(name="dayKind")
    private @Nullable Output<String> dayKind;

    /**
     * @return Maintenance cycle granularity. Default value: Week.
     * 
     */
    public Optional<Output<String>> dayKind() {
        return Optional.ofNullable(this.dayKind);
    }

    /**
     * Specify which day of the month is the maintenance window. Default is empty, meaning every day is specified.
     * 
     */
    @Import(name="dayOfMonths")
    private @Nullable Output<List<Integer>> dayOfMonths;

    /**
     * @return Specify which day of the month is the maintenance window. Default is empty, meaning every day is specified.
     * 
     */
    public Optional<Output<List<Integer>>> dayOfMonths() {
        return Optional.ofNullable(this.dayOfMonths);
    }

    /**
     * Which day of the week is the maintenance window. Default value is every day: Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday.
     * 
     */
    @Import(name="dayOfWeeks")
    private @Nullable Output<List<String>> dayOfWeeks;

    /**
     * @return Which day of the week is the maintenance window. Default value is every day: Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday.
     * 
     */
    public Optional<Output<List<String>>> dayOfWeeks() {
        return Optional.ofNullable(this.dayOfWeeks);
    }

    /**
     * Instance maintenance window. Format: HH:mmZ-HH:mmZ (UTC time).
     * 
     */
    @Import(name="maintenanceTime")
    private @Nullable Output<String> maintenanceTime;

    /**
     * @return Instance maintenance window. Format: HH:mmZ-HH:mmZ (UTC time).
     * 
     */
    public Optional<Output<String>> maintenanceTime() {
        return Optional.ofNullable(this.maintenanceTime);
    }

    private InstanceMaintenanceWindowArgs() {}

    private InstanceMaintenanceWindowArgs(InstanceMaintenanceWindowArgs $) {
        this.dayKind = $.dayKind;
        this.dayOfMonths = $.dayOfMonths;
        this.dayOfWeeks = $.dayOfWeeks;
        this.maintenanceTime = $.maintenanceTime;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(InstanceMaintenanceWindowArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private InstanceMaintenanceWindowArgs $;

        public Builder() {
            $ = new InstanceMaintenanceWindowArgs();
        }

        public Builder(InstanceMaintenanceWindowArgs defaults) {
            $ = new InstanceMaintenanceWindowArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param dayKind Maintenance cycle granularity. Default value: Week.
         * 
         * @return builder
         * 
         */
        public Builder dayKind(@Nullable Output<String> dayKind) {
            $.dayKind = dayKind;
            return this;
        }

        /**
         * @param dayKind Maintenance cycle granularity. Default value: Week.
         * 
         * @return builder
         * 
         */
        public Builder dayKind(String dayKind) {
            return dayKind(Output.of(dayKind));
        }

        /**
         * @param dayOfMonths Specify which day of the month is the maintenance window. Default is empty, meaning every day is specified.
         * 
         * @return builder
         * 
         */
        public Builder dayOfMonths(@Nullable Output<List<Integer>> dayOfMonths) {
            $.dayOfMonths = dayOfMonths;
            return this;
        }

        /**
         * @param dayOfMonths Specify which day of the month is the maintenance window. Default is empty, meaning every day is specified.
         * 
         * @return builder
         * 
         */
        public Builder dayOfMonths(List<Integer> dayOfMonths) {
            return dayOfMonths(Output.of(dayOfMonths));
        }

        /**
         * @param dayOfMonths Specify which day of the month is the maintenance window. Default is empty, meaning every day is specified.
         * 
         * @return builder
         * 
         */
        public Builder dayOfMonths(Integer... dayOfMonths) {
            return dayOfMonths(List.of(dayOfMonths));
        }

        /**
         * @param dayOfWeeks Which day of the week is the maintenance window. Default value is every day: Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday.
         * 
         * @return builder
         * 
         */
        public Builder dayOfWeeks(@Nullable Output<List<String>> dayOfWeeks) {
            $.dayOfWeeks = dayOfWeeks;
            return this;
        }

        /**
         * @param dayOfWeeks Which day of the week is the maintenance window. Default value is every day: Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday.
         * 
         * @return builder
         * 
         */
        public Builder dayOfWeeks(List<String> dayOfWeeks) {
            return dayOfWeeks(Output.of(dayOfWeeks));
        }

        /**
         * @param dayOfWeeks Which day of the week is the maintenance window. Default value is every day: Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday.
         * 
         * @return builder
         * 
         */
        public Builder dayOfWeeks(String... dayOfWeeks) {
            return dayOfWeeks(List.of(dayOfWeeks));
        }

        /**
         * @param maintenanceTime Instance maintenance window. Format: HH:mmZ-HH:mmZ (UTC time).
         * 
         * @return builder
         * 
         */
        public Builder maintenanceTime(@Nullable Output<String> maintenanceTime) {
            $.maintenanceTime = maintenanceTime;
            return this;
        }

        /**
         * @param maintenanceTime Instance maintenance window. Format: HH:mmZ-HH:mmZ (UTC time).
         * 
         * @return builder
         * 
         */
        public Builder maintenanceTime(String maintenanceTime) {
            return maintenanceTime(Output.of(maintenanceTime));
        }

        public InstanceMaintenanceWindowArgs build() {
            return $;
        }
    }

}
