// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.volcengine.volcenginecc.vke.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import java.lang.Integer;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class DefaultNodePoolNodeConfigDataVolumeArgs extends com.pulumi.resources.ResourceArgs {

    public static final DefaultNodePoolNodeConfigDataVolumeArgs Empty = new DefaultNodePoolNodeConfigDataVolumeArgs();

    /**
     * File system. Options:   - Ext4 (default)   - Xfs
     * 
     */
    @Import(name="fileSystem")
    private @Nullable Output<String> fileSystem;

    /**
     * @return File system. Options:   - Ext4 (default)   - Xfs
     * 
     */
    public Optional<Output<String>> fileSystem() {
        return Optional.ofNullable(this.fileSystem);
    }

    /**
     * Target mount directory after disk formatting.
     * 
     */
    @Import(name="mountPoint")
    private @Nullable Output<String> mountPoint;

    /**
     * @return Target mount directory after disk formatting.
     * 
     */
    public Optional<Output<String>> mountPoint() {
        return Optional.ofNullable(this.mountPoint);
    }

    /**
     * Placement group ID
     * 
     */
    @Import(name="placementGroupId")
    private @Nullable Output<String> placementGroupId;

    /**
     * @return Placement group ID
     * 
     */
    public Optional<Output<String>> placementGroupId() {
        return Optional.ofNullable(this.placementGroupId);
    }

    /**
     * Disk capacity, in GiB
     * 
     */
    @Import(name="size")
    private @Nullable Output<Integer> size;

    /**
     * @return Disk capacity, in GiB
     * 
     */
    public Optional<Output<Integer>> size() {
        return Optional.ofNullable(this.size);
    }

    /**
     * Create a data disk using a snapshot. You can call the DescribeSnapshots API to query the snapshot ID. Note: Only ultra-fast SSD and efficient cloud disks support creating disks from snapshots. Only snapshots in &#39;available&#39; status can be used to create new disks; snapshots in &#39;creating&#39;, &#39;rolling back&#39;, &#39;deleting&#39;, or &#39;error&#39; status cannot be used to create new disks
     * 
     */
    @Import(name="snapshotId")
    private @Nullable Output<String> snapshotId;

    /**
     * @return Create a data disk using a snapshot. You can call the DescribeSnapshots API to query the snapshot ID. Note: Only ultra-fast SSD and efficient cloud disks support creating disks from snapshots. Only snapshots in &#39;available&#39; status can be used to create new disks; snapshots in &#39;creating&#39;, &#39;rolling back&#39;, &#39;deleting&#39;, or &#39;error&#39; status cannot be used to create new disks
     * 
     */
    public Optional<Output<String>> snapshotId() {
        return Optional.ofNullable(this.snapshotId);
    }

    /**
     * Placement subgroup
     * 
     */
    @Import(name="subgroupNumber")
    private @Nullable Output<Integer> subgroupNumber;

    /**
     * @return Placement subgroup
     * 
     */
    public Optional<Output<Integer>> subgroupNumber() {
        return Optional.ofNullable(this.subgroupNumber);
    }

    /**
     * Disk type: ESSD*PL0: ultra-fast SSD cloud disk with performance level PL0. ESSD*FlexPL: ultra-fast SSD cloud disk with performance level PL1.
     * 
     */
    @Import(name="type")
    private @Nullable Output<String> type;

    /**
     * @return Disk type: ESSD*PL0: ultra-fast SSD cloud disk with performance level PL0. ESSD*FlexPL: ultra-fast SSD cloud disk with performance level PL1.
     * 
     */
    public Optional<Output<String>> type() {
        return Optional.ofNullable(this.type);
    }

    private DefaultNodePoolNodeConfigDataVolumeArgs() {}

    private DefaultNodePoolNodeConfigDataVolumeArgs(DefaultNodePoolNodeConfigDataVolumeArgs $) {
        this.fileSystem = $.fileSystem;
        this.mountPoint = $.mountPoint;
        this.placementGroupId = $.placementGroupId;
        this.size = $.size;
        this.snapshotId = $.snapshotId;
        this.subgroupNumber = $.subgroupNumber;
        this.type = $.type;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(DefaultNodePoolNodeConfigDataVolumeArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private DefaultNodePoolNodeConfigDataVolumeArgs $;

        public Builder() {
            $ = new DefaultNodePoolNodeConfigDataVolumeArgs();
        }

        public Builder(DefaultNodePoolNodeConfigDataVolumeArgs defaults) {
            $ = new DefaultNodePoolNodeConfigDataVolumeArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param fileSystem File system. Options:   - Ext4 (default)   - Xfs
         * 
         * @return builder
         * 
         */
        public Builder fileSystem(@Nullable Output<String> fileSystem) {
            $.fileSystem = fileSystem;
            return this;
        }

        /**
         * @param fileSystem File system. Options:   - Ext4 (default)   - Xfs
         * 
         * @return builder
         * 
         */
        public Builder fileSystem(String fileSystem) {
            return fileSystem(Output.of(fileSystem));
        }

        /**
         * @param mountPoint Target mount directory after disk formatting.
         * 
         * @return builder
         * 
         */
        public Builder mountPoint(@Nullable Output<String> mountPoint) {
            $.mountPoint = mountPoint;
            return this;
        }

        /**
         * @param mountPoint Target mount directory after disk formatting.
         * 
         * @return builder
         * 
         */
        public Builder mountPoint(String mountPoint) {
            return mountPoint(Output.of(mountPoint));
        }

        /**
         * @param placementGroupId Placement group ID
         * 
         * @return builder
         * 
         */
        public Builder placementGroupId(@Nullable Output<String> placementGroupId) {
            $.placementGroupId = placementGroupId;
            return this;
        }

        /**
         * @param placementGroupId Placement group ID
         * 
         * @return builder
         * 
         */
        public Builder placementGroupId(String placementGroupId) {
            return placementGroupId(Output.of(placementGroupId));
        }

        /**
         * @param size Disk capacity, in GiB
         * 
         * @return builder
         * 
         */
        public Builder size(@Nullable Output<Integer> size) {
            $.size = size;
            return this;
        }

        /**
         * @param size Disk capacity, in GiB
         * 
         * @return builder
         * 
         */
        public Builder size(Integer size) {
            return size(Output.of(size));
        }

        /**
         * @param snapshotId Create a data disk using a snapshot. You can call the DescribeSnapshots API to query the snapshot ID. Note: Only ultra-fast SSD and efficient cloud disks support creating disks from snapshots. Only snapshots in &#39;available&#39; status can be used to create new disks; snapshots in &#39;creating&#39;, &#39;rolling back&#39;, &#39;deleting&#39;, or &#39;error&#39; status cannot be used to create new disks
         * 
         * @return builder
         * 
         */
        public Builder snapshotId(@Nullable Output<String> snapshotId) {
            $.snapshotId = snapshotId;
            return this;
        }

        /**
         * @param snapshotId Create a data disk using a snapshot. You can call the DescribeSnapshots API to query the snapshot ID. Note: Only ultra-fast SSD and efficient cloud disks support creating disks from snapshots. Only snapshots in &#39;available&#39; status can be used to create new disks; snapshots in &#39;creating&#39;, &#39;rolling back&#39;, &#39;deleting&#39;, or &#39;error&#39; status cannot be used to create new disks
         * 
         * @return builder
         * 
         */
        public Builder snapshotId(String snapshotId) {
            return snapshotId(Output.of(snapshotId));
        }

        /**
         * @param subgroupNumber Placement subgroup
         * 
         * @return builder
         * 
         */
        public Builder subgroupNumber(@Nullable Output<Integer> subgroupNumber) {
            $.subgroupNumber = subgroupNumber;
            return this;
        }

        /**
         * @param subgroupNumber Placement subgroup
         * 
         * @return builder
         * 
         */
        public Builder subgroupNumber(Integer subgroupNumber) {
            return subgroupNumber(Output.of(subgroupNumber));
        }

        /**
         * @param type Disk type: ESSD*PL0: ultra-fast SSD cloud disk with performance level PL0. ESSD*FlexPL: ultra-fast SSD cloud disk with performance level PL1.
         * 
         * @return builder
         * 
         */
        public Builder type(@Nullable Output<String> type) {
            $.type = type;
            return this;
        }

        /**
         * @param type Disk type: ESSD*PL0: ultra-fast SSD cloud disk with performance level PL0. ESSD*FlexPL: ultra-fast SSD cloud disk with performance level PL1.
         * 
         * @return builder
         * 
         */
        public Builder type(String type) {
            return type(Output.of(type));
        }

        public DefaultNodePoolNodeConfigDataVolumeArgs build() {
            return $;
        }
    }

}
