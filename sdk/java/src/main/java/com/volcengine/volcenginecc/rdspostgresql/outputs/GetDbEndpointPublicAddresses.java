// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.volcengine.volcenginecc.rdspostgresql.outputs;

import com.pulumi.core.annotations.CustomType;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.Boolean;
import java.lang.String;
import java.util.Objects;

@CustomType
public final class GetDbEndpointPublicAddresses {
    /**
     * @return Private network address accessible across regions. Note: If this address is unavailable, this field will not be returned.
     * 
     */
    private String crossRegionDomain;
    /**
     * @return Whether public network resolution is enabled. Values: false: Default, private network resolution. true: Both private and public network resolution.
     * 
     */
    private Boolean dnsVisibility;
    /**
     * @return Connection domain name
     * 
     */
    private String domain;
    /**
     * @return New access address prefix. The access address prefix must meet the following rules: consists of lowercase letters, numbers, and hyphens (-). Must contain at least 8 characters. The total length (including suffix) must not exceed 63 characters. Must start with a lowercase letter and end with a lowercase letter or number.
     * 
     */
    private String domainPrefix;
    /**
     * @return Type of private network address. Values: LocalDomain: Local region domain name. CrossRegionDomain: Domain name accessible across regions.
     * 
     */
    private String domainVisibilitySetting;
    /**
     * @return EIP ID, valid only for Public addresses.
     * 
     */
    private String eipId;
    /**
     * @return IP address
     * 
     */
    private String ipAddress;
    /**
     * @return Network address type. Values: Private: private network connection address. Public: public network connection address. Inner: public service zone address.
     * 
     */
    private String networkType;
    /**
     * @return Port number.
     * 
     */
    private String port;
    /**
     * @return Subnet ID
     * 
     */
    private String subnetId;

    private GetDbEndpointPublicAddresses() {}
    /**
     * @return Private network address accessible across regions. Note: If this address is unavailable, this field will not be returned.
     * 
     */
    public String crossRegionDomain() {
        return this.crossRegionDomain;
    }
    /**
     * @return Whether public network resolution is enabled. Values: false: Default, private network resolution. true: Both private and public network resolution.
     * 
     */
    public Boolean dnsVisibility() {
        return this.dnsVisibility;
    }
    /**
     * @return Connection domain name
     * 
     */
    public String domain() {
        return this.domain;
    }
    /**
     * @return New access address prefix. The access address prefix must meet the following rules: consists of lowercase letters, numbers, and hyphens (-). Must contain at least 8 characters. The total length (including suffix) must not exceed 63 characters. Must start with a lowercase letter and end with a lowercase letter or number.
     * 
     */
    public String domainPrefix() {
        return this.domainPrefix;
    }
    /**
     * @return Type of private network address. Values: LocalDomain: Local region domain name. CrossRegionDomain: Domain name accessible across regions.
     * 
     */
    public String domainVisibilitySetting() {
        return this.domainVisibilitySetting;
    }
    /**
     * @return EIP ID, valid only for Public addresses.
     * 
     */
    public String eipId() {
        return this.eipId;
    }
    /**
     * @return IP address
     * 
     */
    public String ipAddress() {
        return this.ipAddress;
    }
    /**
     * @return Network address type. Values: Private: private network connection address. Public: public network connection address. Inner: public service zone address.
     * 
     */
    public String networkType() {
        return this.networkType;
    }
    /**
     * @return Port number.
     * 
     */
    public String port() {
        return this.port;
    }
    /**
     * @return Subnet ID
     * 
     */
    public String subnetId() {
        return this.subnetId;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(GetDbEndpointPublicAddresses defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private String crossRegionDomain;
        private Boolean dnsVisibility;
        private String domain;
        private String domainPrefix;
        private String domainVisibilitySetting;
        private String eipId;
        private String ipAddress;
        private String networkType;
        private String port;
        private String subnetId;
        public Builder() {}
        public Builder(GetDbEndpointPublicAddresses defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.crossRegionDomain = defaults.crossRegionDomain;
    	      this.dnsVisibility = defaults.dnsVisibility;
    	      this.domain = defaults.domain;
    	      this.domainPrefix = defaults.domainPrefix;
    	      this.domainVisibilitySetting = defaults.domainVisibilitySetting;
    	      this.eipId = defaults.eipId;
    	      this.ipAddress = defaults.ipAddress;
    	      this.networkType = defaults.networkType;
    	      this.port = defaults.port;
    	      this.subnetId = defaults.subnetId;
        }

        @CustomType.Setter
        public Builder crossRegionDomain(String crossRegionDomain) {
            if (crossRegionDomain == null) {
              throw new MissingRequiredPropertyException("GetDbEndpointPublicAddresses", "crossRegionDomain");
            }
            this.crossRegionDomain = crossRegionDomain;
            return this;
        }
        @CustomType.Setter
        public Builder dnsVisibility(Boolean dnsVisibility) {
            if (dnsVisibility == null) {
              throw new MissingRequiredPropertyException("GetDbEndpointPublicAddresses", "dnsVisibility");
            }
            this.dnsVisibility = dnsVisibility;
            return this;
        }
        @CustomType.Setter
        public Builder domain(String domain) {
            if (domain == null) {
              throw new MissingRequiredPropertyException("GetDbEndpointPublicAddresses", "domain");
            }
            this.domain = domain;
            return this;
        }
        @CustomType.Setter
        public Builder domainPrefix(String domainPrefix) {
            if (domainPrefix == null) {
              throw new MissingRequiredPropertyException("GetDbEndpointPublicAddresses", "domainPrefix");
            }
            this.domainPrefix = domainPrefix;
            return this;
        }
        @CustomType.Setter
        public Builder domainVisibilitySetting(String domainVisibilitySetting) {
            if (domainVisibilitySetting == null) {
              throw new MissingRequiredPropertyException("GetDbEndpointPublicAddresses", "domainVisibilitySetting");
            }
            this.domainVisibilitySetting = domainVisibilitySetting;
            return this;
        }
        @CustomType.Setter
        public Builder eipId(String eipId) {
            if (eipId == null) {
              throw new MissingRequiredPropertyException("GetDbEndpointPublicAddresses", "eipId");
            }
            this.eipId = eipId;
            return this;
        }
        @CustomType.Setter
        public Builder ipAddress(String ipAddress) {
            if (ipAddress == null) {
              throw new MissingRequiredPropertyException("GetDbEndpointPublicAddresses", "ipAddress");
            }
            this.ipAddress = ipAddress;
            return this;
        }
        @CustomType.Setter
        public Builder networkType(String networkType) {
            if (networkType == null) {
              throw new MissingRequiredPropertyException("GetDbEndpointPublicAddresses", "networkType");
            }
            this.networkType = networkType;
            return this;
        }
        @CustomType.Setter
        public Builder port(String port) {
            if (port == null) {
              throw new MissingRequiredPropertyException("GetDbEndpointPublicAddresses", "port");
            }
            this.port = port;
            return this;
        }
        @CustomType.Setter
        public Builder subnetId(String subnetId) {
            if (subnetId == null) {
              throw new MissingRequiredPropertyException("GetDbEndpointPublicAddresses", "subnetId");
            }
            this.subnetId = subnetId;
            return this;
        }
        public GetDbEndpointPublicAddresses build() {
            final var _resultValue = new GetDbEndpointPublicAddresses();
            _resultValue.crossRegionDomain = crossRegionDomain;
            _resultValue.dnsVisibility = dnsVisibility;
            _resultValue.domain = domain;
            _resultValue.domainPrefix = domainPrefix;
            _resultValue.domainVisibilitySetting = domainVisibilitySetting;
            _resultValue.eipId = eipId;
            _resultValue.ipAddress = ipAddress;
            _resultValue.networkType = networkType;
            _resultValue.port = port;
            _resultValue.subnetId = subnetId;
            return _resultValue;
        }
    }
}
