// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.volcengine.volcenginecc.vke.outputs;

import com.pulumi.core.annotations.CustomType;
import java.lang.Integer;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class DefaultNodePoolNodeConfigPublicAccessConfig {
    /**
     * @return Peak bandwidth for the public IP, unit: Mbps
     * 
     */
    private @Nullable Integer bandwidth;
    /**
     * @return Billing type for public IP: 2: Pay-as-you-go by bandwidth cap. 3: Pay-as-you-go by actual traffic.
     * 
     */
    private @Nullable Integer billingType;
    /**
     * @return Public IP line type. Parameter description: BGP: BGP (multi-line)
     * 
     */
    private @Nullable String isp;

    private DefaultNodePoolNodeConfigPublicAccessConfig() {}
    /**
     * @return Peak bandwidth for the public IP, unit: Mbps
     * 
     */
    public Optional<Integer> bandwidth() {
        return Optional.ofNullable(this.bandwidth);
    }
    /**
     * @return Billing type for public IP: 2: Pay-as-you-go by bandwidth cap. 3: Pay-as-you-go by actual traffic.
     * 
     */
    public Optional<Integer> billingType() {
        return Optional.ofNullable(this.billingType);
    }
    /**
     * @return Public IP line type. Parameter description: BGP: BGP (multi-line)
     * 
     */
    public Optional<String> isp() {
        return Optional.ofNullable(this.isp);
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(DefaultNodePoolNodeConfigPublicAccessConfig defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private @Nullable Integer bandwidth;
        private @Nullable Integer billingType;
        private @Nullable String isp;
        public Builder() {}
        public Builder(DefaultNodePoolNodeConfigPublicAccessConfig defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.bandwidth = defaults.bandwidth;
    	      this.billingType = defaults.billingType;
    	      this.isp = defaults.isp;
        }

        @CustomType.Setter
        public Builder bandwidth(@Nullable Integer bandwidth) {

            this.bandwidth = bandwidth;
            return this;
        }
        @CustomType.Setter
        public Builder billingType(@Nullable Integer billingType) {

            this.billingType = billingType;
            return this;
        }
        @CustomType.Setter
        public Builder isp(@Nullable String isp) {

            this.isp = isp;
            return this;
        }
        public DefaultNodePoolNodeConfigPublicAccessConfig build() {
            final var _resultValue = new DefaultNodePoolNodeConfigPublicAccessConfig();
            _resultValue.bandwidth = bandwidth;
            _resultValue.billingType = billingType;
            _resultValue.isp = isp;
            return _resultValue;
        }
    }
}
