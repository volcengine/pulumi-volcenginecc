// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;
using Pulumi;

namespace Volcengine.Pulumi.Volcenginecc.Vke.Outputs
{

    [OutputType]
    public sealed class GetDefaultNodePoolNodeConfigSecurityResult
    {
        /// <summary>
        /// 节点的访问方式配置。
        /// </summary>
        public readonly Outputs.GetDefaultNodePoolNodeConfigSecurityLoginResult Login;
        /// <summary>
        /// 节点网络所在的安全组 ID 列表。
        /// </summary>
        public readonly ImmutableArray<string> SecurityGroupIds;
        /// <summary>
        /// 节点的安全策略，参数值说明：Hids：主机安全加固。
        /// </summary>
        public readonly ImmutableArray<string> SecurityStrategies;
        /// <summary>
        /// 节点是否启用了安全加固配置，参数值说明：true：已开启。false：未开启。
        /// </summary>
        public readonly bool SecurityStrategyEnabled;

        [OutputConstructor]
        private GetDefaultNodePoolNodeConfigSecurityResult(
            Outputs.GetDefaultNodePoolNodeConfigSecurityLoginResult login,

            ImmutableArray<string> securityGroupIds,

            ImmutableArray<string> securityStrategies,

            bool securityStrategyEnabled)
        {
            Login = login;
            SecurityGroupIds = securityGroupIds;
            SecurityStrategies = securityStrategies;
            SecurityStrategyEnabled = securityStrategyEnabled;
        }
    }
}
