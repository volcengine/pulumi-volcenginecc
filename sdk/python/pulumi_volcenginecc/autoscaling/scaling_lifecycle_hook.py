# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = ['ScalingLifecycleHookArgs', 'ScalingLifecycleHook']

@pulumi.input_type
class ScalingLifecycleHookArgs:
    def __init__(__self__, *,
                 lifecycle_hook_name: pulumi.Input[_builtins.str],
                 lifecycle_hook_policy: pulumi.Input[_builtins.str],
                 lifecycle_hook_timeout: pulumi.Input[_builtins.int],
                 lifecycle_hook_type: pulumi.Input[_builtins.str],
                 scaling_group_id: pulumi.Input[_builtins.str],
                 lifecycle_command: pulumi.Input[Optional['ScalingLifecycleHookLifecycleCommandArgs']] = None):
        """
        The set of arguments for constructing a ScalingLifecycleHook resource.

        :param pulumi.Input[_builtins.str] lifecycle_hook_name: Lifecycle hook name, cannot be modified after creation. Values: Cannot start with a digit, hyphen, or underscore. Can only contain Chinese characters, letters, digits, underscores, and hyphens. Length must be between 1 and 128 characters.
        :param pulumi.Input[_builtins.str] lifecycle_hook_policy: Policy executed after the instance suspension ends. Values: CONTINUE: Continue execution. REJECT: Abort subsequent actions. ROLLBACK: For elastic scale-in activities, the release of ECS instances is rejected and rollback is performed; for elastic scale-out activities, the effect is the same as REJECT.
        :param pulumi.Input[_builtins.int] lifecycle_hook_timeout: Duration for which the ECS instance remains suspended. After timeout, the suspended state ends automatically and scaling actions continue according to the execution policy. Please evaluate the processing time for custom operations and set an appropriate timeout. Values: 30 to 21600 (6 hours), unit: s, must be an integer.
        :param pulumi.Input[_builtins.str] lifecycle_hook_type: Type of scaling activity. When a specified type of scaling activity occurs, the lifecycle hook is triggered and the ECS instance is suspended. Values: SCALE*IN: Elastic scale-in activity. SCALE*OUT: Elastic scale-out activity.
        :param pulumi.Input[_builtins.str] scaling_group_id: Scaling group ID.
        :param pulumi.Input['ScalingLifecycleHookLifecycleCommandArgs'] lifecycle_command: Cloud Assistant command information.
        """
        pulumi.set(__self__, "lifecycle_hook_name", lifecycle_hook_name)
        pulumi.set(__self__, "lifecycle_hook_policy", lifecycle_hook_policy)
        pulumi.set(__self__, "lifecycle_hook_timeout", lifecycle_hook_timeout)
        pulumi.set(__self__, "lifecycle_hook_type", lifecycle_hook_type)
        pulumi.set(__self__, "scaling_group_id", scaling_group_id)
        if lifecycle_command is not None:
            pulumi.set(__self__, "lifecycle_command", lifecycle_command)

    @_builtins.property
    @pulumi.getter(name="lifecycleHookName")
    def lifecycle_hook_name(self) -> pulumi.Input[_builtins.str]:
        """
        Lifecycle hook name, cannot be modified after creation. Values: Cannot start with a digit, hyphen, or underscore. Can only contain Chinese characters, letters, digits, underscores, and hyphens. Length must be between 1 and 128 characters.
        """
        return pulumi.get(self, "lifecycle_hook_name")

    @lifecycle_hook_name.setter
    def lifecycle_hook_name(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "lifecycle_hook_name", value)

    @_builtins.property
    @pulumi.getter(name="lifecycleHookPolicy")
    def lifecycle_hook_policy(self) -> pulumi.Input[_builtins.str]:
        """
        Policy executed after the instance suspension ends. Values: CONTINUE: Continue execution. REJECT: Abort subsequent actions. ROLLBACK: For elastic scale-in activities, the release of ECS instances is rejected and rollback is performed; for elastic scale-out activities, the effect is the same as REJECT.
        """
        return pulumi.get(self, "lifecycle_hook_policy")

    @lifecycle_hook_policy.setter
    def lifecycle_hook_policy(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "lifecycle_hook_policy", value)

    @_builtins.property
    @pulumi.getter(name="lifecycleHookTimeout")
    def lifecycle_hook_timeout(self) -> pulumi.Input[_builtins.int]:
        """
        Duration for which the ECS instance remains suspended. After timeout, the suspended state ends automatically and scaling actions continue according to the execution policy. Please evaluate the processing time for custom operations and set an appropriate timeout. Values: 30 to 21600 (6 hours), unit: s, must be an integer.
        """
        return pulumi.get(self, "lifecycle_hook_timeout")

    @lifecycle_hook_timeout.setter
    def lifecycle_hook_timeout(self, value: pulumi.Input[_builtins.int]):
        pulumi.set(self, "lifecycle_hook_timeout", value)

    @_builtins.property
    @pulumi.getter(name="lifecycleHookType")
    def lifecycle_hook_type(self) -> pulumi.Input[_builtins.str]:
        """
        Type of scaling activity. When a specified type of scaling activity occurs, the lifecycle hook is triggered and the ECS instance is suspended. Values: SCALE*IN: Elastic scale-in activity. SCALE*OUT: Elastic scale-out activity.
        """
        return pulumi.get(self, "lifecycle_hook_type")

    @lifecycle_hook_type.setter
    def lifecycle_hook_type(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "lifecycle_hook_type", value)

    @_builtins.property
    @pulumi.getter(name="scalingGroupId")
    def scaling_group_id(self) -> pulumi.Input[_builtins.str]:
        """
        Scaling group ID.
        """
        return pulumi.get(self, "scaling_group_id")

    @scaling_group_id.setter
    def scaling_group_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "scaling_group_id", value)

    @_builtins.property
    @pulumi.getter(name="lifecycleCommand")
    def lifecycle_command(self) -> pulumi.Input[Optional['ScalingLifecycleHookLifecycleCommandArgs']]:
        """
        Cloud Assistant command information.
        """
        return pulumi.get(self, "lifecycle_command")

    @lifecycle_command.setter
    def lifecycle_command(self, value: pulumi.Input[Optional['ScalingLifecycleHookLifecycleCommandArgs']]):
        pulumi.set(self, "lifecycle_command", value)


@pulumi.input_type
class _ScalingLifecycleHookState:
    def __init__(__self__, *,
                 lifecycle_command: pulumi.Input[Optional['ScalingLifecycleHookLifecycleCommandArgs']] = None,
                 lifecycle_hook_id: pulumi.Input[Optional[_builtins.str]] = None,
                 lifecycle_hook_name: pulumi.Input[Optional[_builtins.str]] = None,
                 lifecycle_hook_policy: pulumi.Input[Optional[_builtins.str]] = None,
                 lifecycle_hook_timeout: pulumi.Input[Optional[_builtins.int]] = None,
                 lifecycle_hook_type: pulumi.Input[Optional[_builtins.str]] = None,
                 scaling_group_id: pulumi.Input[Optional[_builtins.str]] = None):
        """
        Input properties used for looking up and filtering ScalingLifecycleHook resources.

        :param pulumi.Input['ScalingLifecycleHookLifecycleCommandArgs'] lifecycle_command: Cloud Assistant command information.
        :param pulumi.Input[_builtins.str] lifecycle_hook_id: Lifecycle hook ID.
        :param pulumi.Input[_builtins.str] lifecycle_hook_name: Lifecycle hook name, cannot be modified after creation. Values: Cannot start with a digit, hyphen, or underscore. Can only contain Chinese characters, letters, digits, underscores, and hyphens. Length must be between 1 and 128 characters.
        :param pulumi.Input[_builtins.str] lifecycle_hook_policy: Policy executed after the instance suspension ends. Values: CONTINUE: Continue execution. REJECT: Abort subsequent actions. ROLLBACK: For elastic scale-in activities, the release of ECS instances is rejected and rollback is performed; for elastic scale-out activities, the effect is the same as REJECT.
        :param pulumi.Input[_builtins.int] lifecycle_hook_timeout: Duration for which the ECS instance remains suspended. After timeout, the suspended state ends automatically and scaling actions continue according to the execution policy. Please evaluate the processing time for custom operations and set an appropriate timeout. Values: 30 to 21600 (6 hours), unit: s, must be an integer.
        :param pulumi.Input[_builtins.str] lifecycle_hook_type: Type of scaling activity. When a specified type of scaling activity occurs, the lifecycle hook is triggered and the ECS instance is suspended. Values: SCALE*IN: Elastic scale-in activity. SCALE*OUT: Elastic scale-out activity.
        :param pulumi.Input[_builtins.str] scaling_group_id: Scaling group ID.
        """
        if lifecycle_command is not None:
            pulumi.set(__self__, "lifecycle_command", lifecycle_command)
        if lifecycle_hook_id is not None:
            pulumi.set(__self__, "lifecycle_hook_id", lifecycle_hook_id)
        if lifecycle_hook_name is not None:
            pulumi.set(__self__, "lifecycle_hook_name", lifecycle_hook_name)
        if lifecycle_hook_policy is not None:
            pulumi.set(__self__, "lifecycle_hook_policy", lifecycle_hook_policy)
        if lifecycle_hook_timeout is not None:
            pulumi.set(__self__, "lifecycle_hook_timeout", lifecycle_hook_timeout)
        if lifecycle_hook_type is not None:
            pulumi.set(__self__, "lifecycle_hook_type", lifecycle_hook_type)
        if scaling_group_id is not None:
            pulumi.set(__self__, "scaling_group_id", scaling_group_id)

    @_builtins.property
    @pulumi.getter(name="lifecycleCommand")
    def lifecycle_command(self) -> pulumi.Input[Optional['ScalingLifecycleHookLifecycleCommandArgs']]:
        """
        Cloud Assistant command information.
        """
        return pulumi.get(self, "lifecycle_command")

    @lifecycle_command.setter
    def lifecycle_command(self, value: pulumi.Input[Optional['ScalingLifecycleHookLifecycleCommandArgs']]):
        pulumi.set(self, "lifecycle_command", value)

    @_builtins.property
    @pulumi.getter(name="lifecycleHookId")
    def lifecycle_hook_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Lifecycle hook ID.
        """
        return pulumi.get(self, "lifecycle_hook_id")

    @lifecycle_hook_id.setter
    def lifecycle_hook_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "lifecycle_hook_id", value)

    @_builtins.property
    @pulumi.getter(name="lifecycleHookName")
    def lifecycle_hook_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Lifecycle hook name, cannot be modified after creation. Values: Cannot start with a digit, hyphen, or underscore. Can only contain Chinese characters, letters, digits, underscores, and hyphens. Length must be between 1 and 128 characters.
        """
        return pulumi.get(self, "lifecycle_hook_name")

    @lifecycle_hook_name.setter
    def lifecycle_hook_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "lifecycle_hook_name", value)

    @_builtins.property
    @pulumi.getter(name="lifecycleHookPolicy")
    def lifecycle_hook_policy(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Policy executed after the instance suspension ends. Values: CONTINUE: Continue execution. REJECT: Abort subsequent actions. ROLLBACK: For elastic scale-in activities, the release of ECS instances is rejected and rollback is performed; for elastic scale-out activities, the effect is the same as REJECT.
        """
        return pulumi.get(self, "lifecycle_hook_policy")

    @lifecycle_hook_policy.setter
    def lifecycle_hook_policy(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "lifecycle_hook_policy", value)

    @_builtins.property
    @pulumi.getter(name="lifecycleHookTimeout")
    def lifecycle_hook_timeout(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Duration for which the ECS instance remains suspended. After timeout, the suspended state ends automatically and scaling actions continue according to the execution policy. Please evaluate the processing time for custom operations and set an appropriate timeout. Values: 30 to 21600 (6 hours), unit: s, must be an integer.
        """
        return pulumi.get(self, "lifecycle_hook_timeout")

    @lifecycle_hook_timeout.setter
    def lifecycle_hook_timeout(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "lifecycle_hook_timeout", value)

    @_builtins.property
    @pulumi.getter(name="lifecycleHookType")
    def lifecycle_hook_type(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Type of scaling activity. When a specified type of scaling activity occurs, the lifecycle hook is triggered and the ECS instance is suspended. Values: SCALE*IN: Elastic scale-in activity. SCALE*OUT: Elastic scale-out activity.
        """
        return pulumi.get(self, "lifecycle_hook_type")

    @lifecycle_hook_type.setter
    def lifecycle_hook_type(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "lifecycle_hook_type", value)

    @_builtins.property
    @pulumi.getter(name="scalingGroupId")
    def scaling_group_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Scaling group ID.
        """
        return pulumi.get(self, "scaling_group_id")

    @scaling_group_id.setter
    def scaling_group_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "scaling_group_id", value)


@pulumi.type_token("volcenginecc:autoscaling/scalingLifecycleHook:ScalingLifecycleHook")
class ScalingLifecycleHook(pulumi.CustomResource):
    @overload
    def __init__(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 lifecycle_command: pulumi.Input[Optional[Union['ScalingLifecycleHookLifecycleCommandArgs', 'ScalingLifecycleHookLifecycleCommandArgsDict']]] = None,
                 lifecycle_hook_name: pulumi.Input[Optional[_builtins.str]] = None,
                 lifecycle_hook_policy: pulumi.Input[Optional[_builtins.str]] = None,
                 lifecycle_hook_timeout: pulumi.Input[Optional[_builtins.int]] = None,
                 lifecycle_hook_type: pulumi.Input[Optional[_builtins.str]] = None,
                 scaling_group_id: pulumi.Input[Optional[_builtins.str]] = None,
                 __props__=None):
        """
        After the scaling group triggers scaling rules, health checks, and instance count checks, the lifecycle hook can suspend instances that are in the 'joining', 'removing', or 'disabling' state, putting them into the joining/removing suspended state. The instances are now in a waiting state, and you can perform custom operations during this period until the lifecycle hook times out or you manually end the suspension.

        ## Example Usage

        ```python
        import pulumi
        import pulumi_volcenginecc as volcenginecc

        scaling_lifecycle_hook_demo = volcenginecc.autoscaling.ScalingLifecycleHook("ScalingLifecycleHookDemo",
            scaling_group_id="scg-yexxxxxxv6yn56hnvlm",
            lifecycle_hook_name="ccapi-dx-1",
            lifecycle_hook_type="SCALE_IN",
            lifecycle_hook_timeout=1800,
            lifecycle_hook_policy="CONTINUE",
            lifecycle_command={
                "command_id": "cmd-ybv3xxxxxxx51qxnx",
                "parameters": "{\\"KEY_PAIR_ID\\":\\"1024\\",\\"KEY_PAIR_TIMEOUT\\":\\"360\\"}",
            })
        ```

        ## Import

        ```sh
        $ pulumi import volcenginecc:autoscaling/scalingLifecycleHook:ScalingLifecycleHook example "scaling_group_id|lifecycle_hook_id"
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[Union['ScalingLifecycleHookLifecycleCommandArgs', 'ScalingLifecycleHookLifecycleCommandArgsDict']] lifecycle_command: Cloud Assistant command information.
        :param pulumi.Input[_builtins.str] lifecycle_hook_name: Lifecycle hook name, cannot be modified after creation. Values: Cannot start with a digit, hyphen, or underscore. Can only contain Chinese characters, letters, digits, underscores, and hyphens. Length must be between 1 and 128 characters.
        :param pulumi.Input[_builtins.str] lifecycle_hook_policy: Policy executed after the instance suspension ends. Values: CONTINUE: Continue execution. REJECT: Abort subsequent actions. ROLLBACK: For elastic scale-in activities, the release of ECS instances is rejected and rollback is performed; for elastic scale-out activities, the effect is the same as REJECT.
        :param pulumi.Input[_builtins.int] lifecycle_hook_timeout: Duration for which the ECS instance remains suspended. After timeout, the suspended state ends automatically and scaling actions continue according to the execution policy. Please evaluate the processing time for custom operations and set an appropriate timeout. Values: 30 to 21600 (6 hours), unit: s, must be an integer.
        :param pulumi.Input[_builtins.str] lifecycle_hook_type: Type of scaling activity. When a specified type of scaling activity occurs, the lifecycle hook is triggered and the ECS instance is suspended. Values: SCALE*IN: Elastic scale-in activity. SCALE*OUT: Elastic scale-out activity.
        :param pulumi.Input[_builtins.str] scaling_group_id: Scaling group ID.
        """
        ...
    @overload
    def __init__(__self__,
                 resource_name: str,
                 args: ScalingLifecycleHookArgs,
                 opts: Optional[pulumi.ResourceOptions] = None):
        """
        After the scaling group triggers scaling rules, health checks, and instance count checks, the lifecycle hook can suspend instances that are in the 'joining', 'removing', or 'disabling' state, putting them into the joining/removing suspended state. The instances are now in a waiting state, and you can perform custom operations during this period until the lifecycle hook times out or you manually end the suspension.

        ## Example Usage

        ```python
        import pulumi
        import pulumi_volcenginecc as volcenginecc

        scaling_lifecycle_hook_demo = volcenginecc.autoscaling.ScalingLifecycleHook("ScalingLifecycleHookDemo",
            scaling_group_id="scg-yexxxxxxv6yn56hnvlm",
            lifecycle_hook_name="ccapi-dx-1",
            lifecycle_hook_type="SCALE_IN",
            lifecycle_hook_timeout=1800,
            lifecycle_hook_policy="CONTINUE",
            lifecycle_command={
                "command_id": "cmd-ybv3xxxxxxx51qxnx",
                "parameters": "{\\"KEY_PAIR_ID\\":\\"1024\\",\\"KEY_PAIR_TIMEOUT\\":\\"360\\"}",
            })
        ```

        ## Import

        ```sh
        $ pulumi import volcenginecc:autoscaling/scalingLifecycleHook:ScalingLifecycleHook example "scaling_group_id|lifecycle_hook_id"
        ```


        :param str resource_name: The name of the resource.
        :param ScalingLifecycleHookArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        ...
    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(ScalingLifecycleHookArgs, pulumi.ResourceOptions, *args, **kwargs)
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 lifecycle_command: pulumi.Input[Optional[Union['ScalingLifecycleHookLifecycleCommandArgs', 'ScalingLifecycleHookLifecycleCommandArgsDict']]] = None,
                 lifecycle_hook_name: pulumi.Input[Optional[_builtins.str]] = None,
                 lifecycle_hook_policy: pulumi.Input[Optional[_builtins.str]] = None,
                 lifecycle_hook_timeout: pulumi.Input[Optional[_builtins.int]] = None,
                 lifecycle_hook_type: pulumi.Input[Optional[_builtins.str]] = None,
                 scaling_group_id: pulumi.Input[Optional[_builtins.str]] = None,
                 __props__=None):
        opts = pulumi.ResourceOptions.merge(_utilities.get_resource_opts_defaults(), opts)
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError('Expected resource options to be a ResourceOptions instance')
        if opts.id is None:
            if __props__ is not None:
                raise TypeError('__props__ is only valid when passed in combination with a valid opts.id to get an existing resource')
            __props__ = ScalingLifecycleHookArgs.__new__(ScalingLifecycleHookArgs)

            __props__.__dict__["lifecycle_command"] = lifecycle_command
            if lifecycle_hook_name is None and not opts.urn:
                raise TypeError("Missing required property 'lifecycle_hook_name'")
            __props__.__dict__["lifecycle_hook_name"] = lifecycle_hook_name
            if lifecycle_hook_policy is None and not opts.urn:
                raise TypeError("Missing required property 'lifecycle_hook_policy'")
            __props__.__dict__["lifecycle_hook_policy"] = lifecycle_hook_policy
            if lifecycle_hook_timeout is None and not opts.urn:
                raise TypeError("Missing required property 'lifecycle_hook_timeout'")
            __props__.__dict__["lifecycle_hook_timeout"] = lifecycle_hook_timeout
            if lifecycle_hook_type is None and not opts.urn:
                raise TypeError("Missing required property 'lifecycle_hook_type'")
            __props__.__dict__["lifecycle_hook_type"] = lifecycle_hook_type
            if scaling_group_id is None and not opts.urn:
                raise TypeError("Missing required property 'scaling_group_id'")
            __props__.__dict__["scaling_group_id"] = scaling_group_id
            __props__.__dict__["lifecycle_hook_id"] = None
        super(ScalingLifecycleHook, __self__).__init__(
            'volcenginecc:autoscaling/scalingLifecycleHook:ScalingLifecycleHook',
            resource_name,
            __props__,
            opts)

    @staticmethod
    def get(resource_name: str,
            id: pulumi.Input[str],
            opts: Optional[pulumi.ResourceOptions] = None,
            lifecycle_command: pulumi.Input[Optional[Union['ScalingLifecycleHookLifecycleCommandArgs', 'ScalingLifecycleHookLifecycleCommandArgsDict']]] = None,
            lifecycle_hook_id: pulumi.Input[Optional[_builtins.str]] = None,
            lifecycle_hook_name: pulumi.Input[Optional[_builtins.str]] = None,
            lifecycle_hook_policy: pulumi.Input[Optional[_builtins.str]] = None,
            lifecycle_hook_timeout: pulumi.Input[Optional[_builtins.int]] = None,
            lifecycle_hook_type: pulumi.Input[Optional[_builtins.str]] = None,
            scaling_group_id: pulumi.Input[Optional[_builtins.str]] = None) -> 'ScalingLifecycleHook':
        """
        Get an existing ScalingLifecycleHook resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[Union['ScalingLifecycleHookLifecycleCommandArgs', 'ScalingLifecycleHookLifecycleCommandArgsDict']] lifecycle_command: Cloud Assistant command information.
        :param pulumi.Input[_builtins.str] lifecycle_hook_id: Lifecycle hook ID.
        :param pulumi.Input[_builtins.str] lifecycle_hook_name: Lifecycle hook name, cannot be modified after creation. Values: Cannot start with a digit, hyphen, or underscore. Can only contain Chinese characters, letters, digits, underscores, and hyphens. Length must be between 1 and 128 characters.
        :param pulumi.Input[_builtins.str] lifecycle_hook_policy: Policy executed after the instance suspension ends. Values: CONTINUE: Continue execution. REJECT: Abort subsequent actions. ROLLBACK: For elastic scale-in activities, the release of ECS instances is rejected and rollback is performed; for elastic scale-out activities, the effect is the same as REJECT.
        :param pulumi.Input[_builtins.int] lifecycle_hook_timeout: Duration for which the ECS instance remains suspended. After timeout, the suspended state ends automatically and scaling actions continue according to the execution policy. Please evaluate the processing time for custom operations and set an appropriate timeout. Values: 30 to 21600 (6 hours), unit: s, must be an integer.
        :param pulumi.Input[_builtins.str] lifecycle_hook_type: Type of scaling activity. When a specified type of scaling activity occurs, the lifecycle hook is triggered and the ECS instance is suspended. Values: SCALE*IN: Elastic scale-in activity. SCALE*OUT: Elastic scale-out activity.
        :param pulumi.Input[_builtins.str] scaling_group_id: Scaling group ID.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _ScalingLifecycleHookState.__new__(_ScalingLifecycleHookState)

        __props__.__dict__["lifecycle_command"] = lifecycle_command
        __props__.__dict__["lifecycle_hook_id"] = lifecycle_hook_id
        __props__.__dict__["lifecycle_hook_name"] = lifecycle_hook_name
        __props__.__dict__["lifecycle_hook_policy"] = lifecycle_hook_policy
        __props__.__dict__["lifecycle_hook_timeout"] = lifecycle_hook_timeout
        __props__.__dict__["lifecycle_hook_type"] = lifecycle_hook_type
        __props__.__dict__["scaling_group_id"] = scaling_group_id
        return ScalingLifecycleHook(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="lifecycleCommand")
    def lifecycle_command(self) -> pulumi.Output['outputs.ScalingLifecycleHookLifecycleCommand']:
        """
        Cloud Assistant command information.
        """
        return pulumi.get(self, "lifecycle_command")

    @_builtins.property
    @pulumi.getter(name="lifecycleHookId")
    def lifecycle_hook_id(self) -> pulumi.Output[_builtins.str]:
        """
        Lifecycle hook ID.
        """
        return pulumi.get(self, "lifecycle_hook_id")

    @_builtins.property
    @pulumi.getter(name="lifecycleHookName")
    def lifecycle_hook_name(self) -> pulumi.Output[_builtins.str]:
        """
        Lifecycle hook name, cannot be modified after creation. Values: Cannot start with a digit, hyphen, or underscore. Can only contain Chinese characters, letters, digits, underscores, and hyphens. Length must be between 1 and 128 characters.
        """
        return pulumi.get(self, "lifecycle_hook_name")

    @_builtins.property
    @pulumi.getter(name="lifecycleHookPolicy")
    def lifecycle_hook_policy(self) -> pulumi.Output[_builtins.str]:
        """
        Policy executed after the instance suspension ends. Values: CONTINUE: Continue execution. REJECT: Abort subsequent actions. ROLLBACK: For elastic scale-in activities, the release of ECS instances is rejected and rollback is performed; for elastic scale-out activities, the effect is the same as REJECT.
        """
        return pulumi.get(self, "lifecycle_hook_policy")

    @_builtins.property
    @pulumi.getter(name="lifecycleHookTimeout")
    def lifecycle_hook_timeout(self) -> pulumi.Output[_builtins.int]:
        """
        Duration for which the ECS instance remains suspended. After timeout, the suspended state ends automatically and scaling actions continue according to the execution policy. Please evaluate the processing time for custom operations and set an appropriate timeout. Values: 30 to 21600 (6 hours), unit: s, must be an integer.
        """
        return pulumi.get(self, "lifecycle_hook_timeout")

    @_builtins.property
    @pulumi.getter(name="lifecycleHookType")
    def lifecycle_hook_type(self) -> pulumi.Output[_builtins.str]:
        """
        Type of scaling activity. When a specified type of scaling activity occurs, the lifecycle hook is triggered and the ECS instance is suspended. Values: SCALE*IN: Elastic scale-in activity. SCALE*OUT: Elastic scale-out activity.
        """
        return pulumi.get(self, "lifecycle_hook_type")

    @_builtins.property
    @pulumi.getter(name="scalingGroupId")
    def scaling_group_id(self) -> pulumi.Output[_builtins.str]:
        """
        Scaling group ID.
        """
        return pulumi.get(self, "scaling_group_id")

