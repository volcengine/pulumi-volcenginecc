# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = ['InstanceArgs', 'Instance']

@pulumi.input_type
class InstanceArgs:
    def __init__(__self__, *,
                 charge_type: pulumi.Input[_builtins.str],
                 engine_version: pulumi.Input[_builtins.str],
                 master_spec: pulumi.Input[_builtins.str],
                 region_id: pulumi.Input[_builtins.str],
                 rs_count: pulumi.Input[_builtins.int],
                 rs_spec: pulumi.Input[_builtins.str],
                 storage_capacity: pulumi.Input[_builtins.int],
                 storage_type: pulumi.Input[_builtins.str],
                 subnet_id: pulumi.Input[_builtins.str],
                 vpc_id: pulumi.Input[_builtins.str],
                 zone_id: pulumi.Input[_builtins.str],
                 auto_renew: pulumi.Input[Optional[_builtins.bool]] = None,
                 cold_storage: pulumi.Input[Optional[_builtins.int]] = None,
                 deletion_protection: pulumi.Input[Optional[_builtins.str]] = None,
                 enable_auth: pulumi.Input[Optional[_builtins.bool]] = None,
                 enable_cloud_storage: pulumi.Input[Optional[_builtins.bool]] = None,
                 endpoints: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceEndpointArgs']]]] = None,
                 instance_name: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_type: pulumi.Input[Optional[_builtins.str]] = None,
                 multi_az: pulumi.Input[Optional[_builtins.bool]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 purchase_months: pulumi.Input[Optional[_builtins.int]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceTagArgs']]]] = None):
        """
        The set of arguments for constructing a Instance resource.

        :param pulumi.Input[_builtins.str] charge_type: Billing type. Values: PostPaid: pay-as-you-go (postpaid). PrePaid: subscription (prepaid, yearly/monthly).
        :param pulumi.Input[_builtins.str] engine_version: HBase database engine version. Currently, only HBase 2.0 is supported. The default value is HBase_2.0
        :param pulumi.Input[_builtins.str] master_spec: Master node specification code. For information about supported specifications for Master nodes, see Instance Specifications. The number of Master nodes is fixed and cannot be changed.
        :param pulumi.Input[_builtins.str] region_id: Region ID of the instance.
        :param pulumi.Input[_builtins.int] rs_count: Number of RegionServer nodes
        :param pulumi.Input[_builtins.str] rs_spec: RegionServer node specification code.
        :param pulumi.Input[_builtins.int] storage_capacity: Total storage capacity of the instance, unit: GiB.
        :param pulumi.Input[_builtins.str] storage_type: Instance storage type. Values: HdfsHdd: HDD file storage. HdfsSsd: SSD file storage.
        :param pulumi.Input[_builtins.str] subnet_id: Subnet ID of the instance. For single-zone instances, only one subnet ID for the zone is included
        :param pulumi.Input[_builtins.str] vpc_id: Private network ID. You can call the DescribeVpcs API to query information about private networks available for HBase instance creation, including the private network ID.
        :param pulumi.Input[_builtins.str] zone_id: Zone ID of the instance
        :param pulumi.Input[_builtins.bool] auto_renew: Whether auto-renewal is enabled for subscription scenarios. Values: true: auto-renewal enabled. false: auto-renewal not enabled.
        :param pulumi.Input[_builtins.int] cold_storage: Total capacity-type storage space for the instance, in GiB.
        :param pulumi.Input[_builtins.str] deletion_protection: Instance deletion protection switch status. Value range: enabled: enabled. disabled: not enabled. For more information about instance deletion protection, see Instance Deletion Protection.
        :param pulumi.Input[_builtins.bool] enable_auth: Whether login authentication is enabled. Values: true: login authentication enabled. false: login authentication disabled.
        :param pulumi.Input[_builtins.bool] enable_cloud_storage: Whether capacity-type storage is enabled. Values: true: capacity-type storage enabled. false: capacity-type storage not enabled. For details about capacity-type storage, see Hot and Cold Data Separation. The hot and cold data separation feature is supported only when MultiAZ (deployment mode) is set to false (single availability zone deployment).
        :param pulumi.Input[Sequence[pulumi.Input['InstanceEndpointArgs']]] endpoints: Connection address information list
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] instance_name: Instance name
        :param pulumi.Input[_builtins.str] instance_type: Instance type. Currently, only Standard is supported. The default value is Standard
        :param pulumi.Input[_builtins.bool] multi_az: Instance deployment mode. Values: true: multi-availability zone deployment. false: single availability zone deployment. Currently, only single availability zone deployment is supported.
        :param pulumi.Input[_builtins.str] project_name: Project name of the instance
        :param pulumi.Input[_builtins.int] purchase_months: Purchase duration, in months. The value range is: 1, 2, 3, 4, 5, 6, 7, 8, 9, 12, 24, 36. When ChargeType is PrePaid, this parameter is required
        :param pulumi.Input[Sequence[pulumi.Input['InstanceTagArgs']]] tags: Array of tag keys and tag values to bind. Supports passing multiple sets of tag key-value objects at once, separated by commas (,). Up to 20 tag key-value pairs can be passed at a time, and up to 50 tags can be bound to a single instance. Tag keys and values must comply with the setting rules. For details, see Tag Setting Rules
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        pulumi.set(__self__, "charge_type", charge_type)
        pulumi.set(__self__, "engine_version", engine_version)
        pulumi.set(__self__, "master_spec", master_spec)
        pulumi.set(__self__, "region_id", region_id)
        pulumi.set(__self__, "rs_count", rs_count)
        pulumi.set(__self__, "rs_spec", rs_spec)
        pulumi.set(__self__, "storage_capacity", storage_capacity)
        pulumi.set(__self__, "storage_type", storage_type)
        pulumi.set(__self__, "subnet_id", subnet_id)
        pulumi.set(__self__, "vpc_id", vpc_id)
        pulumi.set(__self__, "zone_id", zone_id)
        if auto_renew is not None:
            pulumi.set(__self__, "auto_renew", auto_renew)
        if cold_storage is not None:
            pulumi.set(__self__, "cold_storage", cold_storage)
        if deletion_protection is not None:
            pulumi.set(__self__, "deletion_protection", deletion_protection)
        if enable_auth is not None:
            pulumi.set(__self__, "enable_auth", enable_auth)
        if enable_cloud_storage is not None:
            pulumi.set(__self__, "enable_cloud_storage", enable_cloud_storage)
        if endpoints is not None:
            pulumi.set(__self__, "endpoints", endpoints)
        if instance_name is not None:
            pulumi.set(__self__, "instance_name", instance_name)
        if instance_type is not None:
            pulumi.set(__self__, "instance_type", instance_type)
        if multi_az is not None:
            pulumi.set(__self__, "multi_az", multi_az)
        if project_name is not None:
            pulumi.set(__self__, "project_name", project_name)
        if purchase_months is not None:
            pulumi.set(__self__, "purchase_months", purchase_months)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)

    @_builtins.property
    @pulumi.getter(name="chargeType")
    def charge_type(self) -> pulumi.Input[_builtins.str]:
        """
        Billing type. Values: PostPaid: pay-as-you-go (postpaid). PrePaid: subscription (prepaid, yearly/monthly).
        """
        return pulumi.get(self, "charge_type")

    @charge_type.setter
    def charge_type(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "charge_type", value)

    @_builtins.property
    @pulumi.getter(name="engineVersion")
    def engine_version(self) -> pulumi.Input[_builtins.str]:
        """
        HBase database engine version. Currently, only HBase 2.0 is supported. The default value is HBase_2.0
        """
        return pulumi.get(self, "engine_version")

    @engine_version.setter
    def engine_version(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "engine_version", value)

    @_builtins.property
    @pulumi.getter(name="masterSpec")
    def master_spec(self) -> pulumi.Input[_builtins.str]:
        """
        Master node specification code. For information about supported specifications for Master nodes, see Instance Specifications. The number of Master nodes is fixed and cannot be changed.
        """
        return pulumi.get(self, "master_spec")

    @master_spec.setter
    def master_spec(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "master_spec", value)

    @_builtins.property
    @pulumi.getter(name="regionId")
    def region_id(self) -> pulumi.Input[_builtins.str]:
        """
        Region ID of the instance.
        """
        return pulumi.get(self, "region_id")

    @region_id.setter
    def region_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "region_id", value)

    @_builtins.property
    @pulumi.getter(name="rsCount")
    def rs_count(self) -> pulumi.Input[_builtins.int]:
        """
        Number of RegionServer nodes
        """
        return pulumi.get(self, "rs_count")

    @rs_count.setter
    def rs_count(self, value: pulumi.Input[_builtins.int]):
        pulumi.set(self, "rs_count", value)

    @_builtins.property
    @pulumi.getter(name="rsSpec")
    def rs_spec(self) -> pulumi.Input[_builtins.str]:
        """
        RegionServer node specification code.
        """
        return pulumi.get(self, "rs_spec")

    @rs_spec.setter
    def rs_spec(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "rs_spec", value)

    @_builtins.property
    @pulumi.getter(name="storageCapacity")
    def storage_capacity(self) -> pulumi.Input[_builtins.int]:
        """
        Total storage capacity of the instance, unit: GiB.
        """
        return pulumi.get(self, "storage_capacity")

    @storage_capacity.setter
    def storage_capacity(self, value: pulumi.Input[_builtins.int]):
        pulumi.set(self, "storage_capacity", value)

    @_builtins.property
    @pulumi.getter(name="storageType")
    def storage_type(self) -> pulumi.Input[_builtins.str]:
        """
        Instance storage type. Values: HdfsHdd: HDD file storage. HdfsSsd: SSD file storage.
        """
        return pulumi.get(self, "storage_type")

    @storage_type.setter
    def storage_type(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "storage_type", value)

    @_builtins.property
    @pulumi.getter(name="subnetId")
    def subnet_id(self) -> pulumi.Input[_builtins.str]:
        """
        Subnet ID of the instance. For single-zone instances, only one subnet ID for the zone is included
        """
        return pulumi.get(self, "subnet_id")

    @subnet_id.setter
    def subnet_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "subnet_id", value)

    @_builtins.property
    @pulumi.getter(name="vpcId")
    def vpc_id(self) -> pulumi.Input[_builtins.str]:
        """
        Private network ID. You can call the DescribeVpcs API to query information about private networks available for HBase instance creation, including the private network ID.
        """
        return pulumi.get(self, "vpc_id")

    @vpc_id.setter
    def vpc_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "vpc_id", value)

    @_builtins.property
    @pulumi.getter(name="zoneId")
    def zone_id(self) -> pulumi.Input[_builtins.str]:
        """
        Zone ID of the instance
        """
        return pulumi.get(self, "zone_id")

    @zone_id.setter
    def zone_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "zone_id", value)

    @_builtins.property
    @pulumi.getter(name="autoRenew")
    def auto_renew(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Whether auto-renewal is enabled for subscription scenarios. Values: true: auto-renewal enabled. false: auto-renewal not enabled.
        """
        return pulumi.get(self, "auto_renew")

    @auto_renew.setter
    def auto_renew(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "auto_renew", value)

    @_builtins.property
    @pulumi.getter(name="coldStorage")
    def cold_storage(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Total capacity-type storage space for the instance, in GiB.
        """
        return pulumi.get(self, "cold_storage")

    @cold_storage.setter
    def cold_storage(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "cold_storage", value)

    @_builtins.property
    @pulumi.getter(name="deletionProtection")
    def deletion_protection(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance deletion protection switch status. Value range: enabled: enabled. disabled: not enabled. For more information about instance deletion protection, see Instance Deletion Protection.
        """
        return pulumi.get(self, "deletion_protection")

    @deletion_protection.setter
    def deletion_protection(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "deletion_protection", value)

    @_builtins.property
    @pulumi.getter(name="enableAuth")
    def enable_auth(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Whether login authentication is enabled. Values: true: login authentication enabled. false: login authentication disabled.
        """
        return pulumi.get(self, "enable_auth")

    @enable_auth.setter
    def enable_auth(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "enable_auth", value)

    @_builtins.property
    @pulumi.getter(name="enableCloudStorage")
    def enable_cloud_storage(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Whether capacity-type storage is enabled. Values: true: capacity-type storage enabled. false: capacity-type storage not enabled. For details about capacity-type storage, see Hot and Cold Data Separation. The hot and cold data separation feature is supported only when MultiAZ (deployment mode) is set to false (single availability zone deployment).
        """
        return pulumi.get(self, "enable_cloud_storage")

    @enable_cloud_storage.setter
    def enable_cloud_storage(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "enable_cloud_storage", value)

    @_builtins.property
    @pulumi.getter
    def endpoints(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['InstanceEndpointArgs']]]]:
        """
        Connection address information list
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "endpoints")

    @endpoints.setter
    def endpoints(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceEndpointArgs']]]]):
        pulumi.set(self, "endpoints", value)

    @_builtins.property
    @pulumi.getter(name="instanceName")
    def instance_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance name
        """
        return pulumi.get(self, "instance_name")

    @instance_name.setter
    def instance_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "instance_name", value)

    @_builtins.property
    @pulumi.getter(name="instanceType")
    def instance_type(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance type. Currently, only Standard is supported. The default value is Standard
        """
        return pulumi.get(self, "instance_type")

    @instance_type.setter
    def instance_type(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "instance_type", value)

    @_builtins.property
    @pulumi.getter(name="multiAz")
    def multi_az(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Instance deployment mode. Values: true: multi-availability zone deployment. false: single availability zone deployment. Currently, only single availability zone deployment is supported.
        """
        return pulumi.get(self, "multi_az")

    @multi_az.setter
    def multi_az(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "multi_az", value)

    @_builtins.property
    @pulumi.getter(name="projectName")
    def project_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Project name of the instance
        """
        return pulumi.get(self, "project_name")

    @project_name.setter
    def project_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "project_name", value)

    @_builtins.property
    @pulumi.getter(name="purchaseMonths")
    def purchase_months(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Purchase duration, in months. The value range is: 1, 2, 3, 4, 5, 6, 7, 8, 9, 12, 24, 36. When ChargeType is PrePaid, this parameter is required
        """
        return pulumi.get(self, "purchase_months")

    @purchase_months.setter
    def purchase_months(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "purchase_months", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['InstanceTagArgs']]]]:
        """
        Array of tag keys and tag values to bind. Supports passing multiple sets of tag key-value objects at once, separated by commas (,). Up to 20 tag key-value pairs can be passed at a time, and up to 50 tags can be bound to a single instance. Tag keys and values must comply with the setting rules. For details, see Tag Setting Rules
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceTagArgs']]]]):
        pulumi.set(self, "tags", value)


@pulumi.input_type
class _InstanceState:
    def __init__(__self__, *,
                 auto_renew: pulumi.Input[Optional[_builtins.bool]] = None,
                 charge_status: pulumi.Input[Optional[_builtins.str]] = None,
                 charge_type: pulumi.Input[Optional[_builtins.str]] = None,
                 cold_storage: pulumi.Input[Optional[_builtins.int]] = None,
                 create_time: pulumi.Input[Optional[_builtins.str]] = None,
                 deletion_protection: pulumi.Input[Optional[_builtins.str]] = None,
                 enable_auth: pulumi.Input[Optional[_builtins.bool]] = None,
                 enable_cloud_storage: pulumi.Input[Optional[_builtins.bool]] = None,
                 endpoints: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceEndpointArgs']]]] = None,
                 engine_version: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_id: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_name: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_type: pulumi.Input[Optional[_builtins.str]] = None,
                 master_count: pulumi.Input[Optional[_builtins.int]] = None,
                 master_spec: pulumi.Input[Optional[_builtins.str]] = None,
                 multi_az: pulumi.Input[Optional[_builtins.bool]] = None,
                 primary_subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
                 primary_zone_id: pulumi.Input[Optional[_builtins.str]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 purchase_months: pulumi.Input[Optional[_builtins.int]] = None,
                 region_id: pulumi.Input[Optional[_builtins.str]] = None,
                 rs_count: pulumi.Input[Optional[_builtins.int]] = None,
                 rs_spec: pulumi.Input[Optional[_builtins.str]] = None,
                 standby_subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
                 standby_zone_id: pulumi.Input[Optional[_builtins.str]] = None,
                 status: pulumi.Input[Optional[_builtins.str]] = None,
                 storage_capacity: pulumi.Input[Optional[_builtins.int]] = None,
                 storage_type: pulumi.Input[Optional[_builtins.str]] = None,
                 subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceTagArgs']]]] = None,
                 used_cold_storage: pulumi.Input[Optional[_builtins.int]] = None,
                 used_storage: pulumi.Input[Optional[_builtins.float]] = None,
                 vpc_id: pulumi.Input[Optional[_builtins.str]] = None,
                 vpc_name: pulumi.Input[Optional[_builtins.str]] = None,
                 zone_id: pulumi.Input[Optional[_builtins.str]] = None,
                 zone_name: pulumi.Input[Optional[_builtins.str]] = None):
        """
        Input properties used for looking up and filtering Instance resources.

        :param pulumi.Input[_builtins.bool] auto_renew: Whether auto-renewal is enabled for subscription scenarios. Values: true: auto-renewal enabled. false: auto-renewal not enabled.
        :param pulumi.Input[_builtins.str] charge_status: Billing status. Values: Normal: normal. Overdue: overdue. Shutdown: shutdown.
        :param pulumi.Input[_builtins.str] charge_type: Billing type. Values: PostPaid: pay-as-you-go (postpaid). PrePaid: subscription (prepaid, yearly/monthly).
        :param pulumi.Input[_builtins.int] cold_storage: Total capacity-type storage space for the instance, in GiB.
        :param pulumi.Input[_builtins.str] create_time: Instance creation time (UTC).
        :param pulumi.Input[_builtins.str] deletion_protection: Instance deletion protection switch status. Value range: enabled: enabled. disabled: not enabled. For more information about instance deletion protection, see Instance Deletion Protection.
        :param pulumi.Input[_builtins.bool] enable_auth: Whether login authentication is enabled. Values: true: login authentication enabled. false: login authentication disabled.
        :param pulumi.Input[_builtins.bool] enable_cloud_storage: Whether capacity-type storage is enabled. Values: true: capacity-type storage enabled. false: capacity-type storage not enabled. For details about capacity-type storage, see Hot and Cold Data Separation. The hot and cold data separation feature is supported only when MultiAZ (deployment mode) is set to false (single availability zone deployment).
        :param pulumi.Input[Sequence[pulumi.Input['InstanceEndpointArgs']]] endpoints: Connection address information list
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] engine_version: HBase database engine version. Currently, only HBase 2.0 is supported. The default value is HBase_2.0
        :param pulumi.Input[_builtins.str] instance_id: Instance ID
        :param pulumi.Input[_builtins.str] instance_name: Instance name
        :param pulumi.Input[_builtins.str] instance_type: Instance type. Currently, only Standard is supported. The default value is Standard
        :param pulumi.Input[_builtins.int] master_count: Number of Master nodes. For single-zone deployment, 2 Master nodes are included by default
        :param pulumi.Input[_builtins.str] master_spec: Master node specification code. For information about supported specifications for Master nodes, see Instance Specifications. The number of Master nodes is fixed and cannot be changed.
        :param pulumi.Input[_builtins.bool] multi_az: Instance deployment mode. Values: true: multi-availability zone deployment. false: single availability zone deployment. Currently, only single availability zone deployment is supported.
        :param pulumi.Input[_builtins.str] primary_subnet_id: Real-time primary availability zone subnet ID.
        :param pulumi.Input[_builtins.str] primary_zone_id: Real-time primary zone ID
        :param pulumi.Input[_builtins.str] project_name: Project name of the instance
        :param pulumi.Input[_builtins.int] purchase_months: Purchase duration, in months. The value range is: 1, 2, 3, 4, 5, 6, 7, 8, 9, 12, 24, 36. When ChargeType is PrePaid, this parameter is required
        :param pulumi.Input[_builtins.str] region_id: Region ID of the instance.
        :param pulumi.Input[_builtins.int] rs_count: Number of RegionServer nodes
        :param pulumi.Input[_builtins.str] rs_spec: RegionServer node specification code.
        :param pulumi.Input[_builtins.str] standby_subnet_id: Real-time standby availability zone subnet ID.
        :param pulumi.Input[_builtins.str] standby_zone_id: Real-time standby zone ID
        :param pulumi.Input[_builtins.str] status: Current status of the instance. For more details about instance status, see Instance Status Description
        :param pulumi.Input[_builtins.int] storage_capacity: Total storage capacity of the instance, unit: GiB.
        :param pulumi.Input[_builtins.str] storage_type: Instance storage type. Values: HdfsHdd: HDD file storage. HdfsSsd: SSD file storage.
        :param pulumi.Input[_builtins.str] subnet_id: Subnet ID of the instance. For single-zone instances, only one subnet ID for the zone is included
        :param pulumi.Input[Sequence[pulumi.Input['InstanceTagArgs']]] tags: Array of tag keys and tag values to bind. Supports passing multiple sets of tag key-value objects at once, separated by commas (,). Up to 20 tag key-value pairs can be passed at a time, and up to 50 tags can be bound to a single instance. Tag keys and values must comply with the setting rules. For details, see Tag Setting Rules
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.int] used_cold_storage: Capacity-type storage space used by the instance, in MiB
        :param pulumi.Input[_builtins.float] used_storage: Storage capacity used by the primary instance, in MiB
        :param pulumi.Input[_builtins.str] vpc_id: Private network ID. You can call the DescribeVpcs API to query information about private networks available for HBase instance creation, including the private network ID.
        :param pulumi.Input[_builtins.str] vpc_name: Name of the private network to which the instance belongs.
        :param pulumi.Input[_builtins.str] zone_id: Zone ID of the instance
        :param pulumi.Input[_builtins.str] zone_name: Name of the availability zone to which the instance belongs.
        """
        if auto_renew is not None:
            pulumi.set(__self__, "auto_renew", auto_renew)
        if charge_status is not None:
            pulumi.set(__self__, "charge_status", charge_status)
        if charge_type is not None:
            pulumi.set(__self__, "charge_type", charge_type)
        if cold_storage is not None:
            pulumi.set(__self__, "cold_storage", cold_storage)
        if create_time is not None:
            pulumi.set(__self__, "create_time", create_time)
        if deletion_protection is not None:
            pulumi.set(__self__, "deletion_protection", deletion_protection)
        if enable_auth is not None:
            pulumi.set(__self__, "enable_auth", enable_auth)
        if enable_cloud_storage is not None:
            pulumi.set(__self__, "enable_cloud_storage", enable_cloud_storage)
        if endpoints is not None:
            pulumi.set(__self__, "endpoints", endpoints)
        if engine_version is not None:
            pulumi.set(__self__, "engine_version", engine_version)
        if instance_id is not None:
            pulumi.set(__self__, "instance_id", instance_id)
        if instance_name is not None:
            pulumi.set(__self__, "instance_name", instance_name)
        if instance_type is not None:
            pulumi.set(__self__, "instance_type", instance_type)
        if master_count is not None:
            pulumi.set(__self__, "master_count", master_count)
        if master_spec is not None:
            pulumi.set(__self__, "master_spec", master_spec)
        if multi_az is not None:
            pulumi.set(__self__, "multi_az", multi_az)
        if primary_subnet_id is not None:
            pulumi.set(__self__, "primary_subnet_id", primary_subnet_id)
        if primary_zone_id is not None:
            pulumi.set(__self__, "primary_zone_id", primary_zone_id)
        if project_name is not None:
            pulumi.set(__self__, "project_name", project_name)
        if purchase_months is not None:
            pulumi.set(__self__, "purchase_months", purchase_months)
        if region_id is not None:
            pulumi.set(__self__, "region_id", region_id)
        if rs_count is not None:
            pulumi.set(__self__, "rs_count", rs_count)
        if rs_spec is not None:
            pulumi.set(__self__, "rs_spec", rs_spec)
        if standby_subnet_id is not None:
            pulumi.set(__self__, "standby_subnet_id", standby_subnet_id)
        if standby_zone_id is not None:
            pulumi.set(__self__, "standby_zone_id", standby_zone_id)
        if status is not None:
            pulumi.set(__self__, "status", status)
        if storage_capacity is not None:
            pulumi.set(__self__, "storage_capacity", storage_capacity)
        if storage_type is not None:
            pulumi.set(__self__, "storage_type", storage_type)
        if subnet_id is not None:
            pulumi.set(__self__, "subnet_id", subnet_id)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)
        if used_cold_storage is not None:
            pulumi.set(__self__, "used_cold_storage", used_cold_storage)
        if used_storage is not None:
            pulumi.set(__self__, "used_storage", used_storage)
        if vpc_id is not None:
            pulumi.set(__self__, "vpc_id", vpc_id)
        if vpc_name is not None:
            pulumi.set(__self__, "vpc_name", vpc_name)
        if zone_id is not None:
            pulumi.set(__self__, "zone_id", zone_id)
        if zone_name is not None:
            pulumi.set(__self__, "zone_name", zone_name)

    @_builtins.property
    @pulumi.getter(name="autoRenew")
    def auto_renew(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Whether auto-renewal is enabled for subscription scenarios. Values: true: auto-renewal enabled. false: auto-renewal not enabled.
        """
        return pulumi.get(self, "auto_renew")

    @auto_renew.setter
    def auto_renew(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "auto_renew", value)

    @_builtins.property
    @pulumi.getter(name="chargeStatus")
    def charge_status(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Billing status. Values: Normal: normal. Overdue: overdue. Shutdown: shutdown.
        """
        return pulumi.get(self, "charge_status")

    @charge_status.setter
    def charge_status(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "charge_status", value)

    @_builtins.property
    @pulumi.getter(name="chargeType")
    def charge_type(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Billing type. Values: PostPaid: pay-as-you-go (postpaid). PrePaid: subscription (prepaid, yearly/monthly).
        """
        return pulumi.get(self, "charge_type")

    @charge_type.setter
    def charge_type(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "charge_type", value)

    @_builtins.property
    @pulumi.getter(name="coldStorage")
    def cold_storage(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Total capacity-type storage space for the instance, in GiB.
        """
        return pulumi.get(self, "cold_storage")

    @cold_storage.setter
    def cold_storage(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "cold_storage", value)

    @_builtins.property
    @pulumi.getter(name="createTime")
    def create_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance creation time (UTC).
        """
        return pulumi.get(self, "create_time")

    @create_time.setter
    def create_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "create_time", value)

    @_builtins.property
    @pulumi.getter(name="deletionProtection")
    def deletion_protection(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance deletion protection switch status. Value range: enabled: enabled. disabled: not enabled. For more information about instance deletion protection, see Instance Deletion Protection.
        """
        return pulumi.get(self, "deletion_protection")

    @deletion_protection.setter
    def deletion_protection(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "deletion_protection", value)

    @_builtins.property
    @pulumi.getter(name="enableAuth")
    def enable_auth(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Whether login authentication is enabled. Values: true: login authentication enabled. false: login authentication disabled.
        """
        return pulumi.get(self, "enable_auth")

    @enable_auth.setter
    def enable_auth(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "enable_auth", value)

    @_builtins.property
    @pulumi.getter(name="enableCloudStorage")
    def enable_cloud_storage(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Whether capacity-type storage is enabled. Values: true: capacity-type storage enabled. false: capacity-type storage not enabled. For details about capacity-type storage, see Hot and Cold Data Separation. The hot and cold data separation feature is supported only when MultiAZ (deployment mode) is set to false (single availability zone deployment).
        """
        return pulumi.get(self, "enable_cloud_storage")

    @enable_cloud_storage.setter
    def enable_cloud_storage(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "enable_cloud_storage", value)

    @_builtins.property
    @pulumi.getter
    def endpoints(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['InstanceEndpointArgs']]]]:
        """
        Connection address information list
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "endpoints")

    @endpoints.setter
    def endpoints(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceEndpointArgs']]]]):
        pulumi.set(self, "endpoints", value)

    @_builtins.property
    @pulumi.getter(name="engineVersion")
    def engine_version(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        HBase database engine version. Currently, only HBase 2.0 is supported. The default value is HBase_2.0
        """
        return pulumi.get(self, "engine_version")

    @engine_version.setter
    def engine_version(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "engine_version", value)

    @_builtins.property
    @pulumi.getter(name="instanceId")
    def instance_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance ID
        """
        return pulumi.get(self, "instance_id")

    @instance_id.setter
    def instance_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "instance_id", value)

    @_builtins.property
    @pulumi.getter(name="instanceName")
    def instance_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance name
        """
        return pulumi.get(self, "instance_name")

    @instance_name.setter
    def instance_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "instance_name", value)

    @_builtins.property
    @pulumi.getter(name="instanceType")
    def instance_type(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance type. Currently, only Standard is supported. The default value is Standard
        """
        return pulumi.get(self, "instance_type")

    @instance_type.setter
    def instance_type(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "instance_type", value)

    @_builtins.property
    @pulumi.getter(name="masterCount")
    def master_count(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Number of Master nodes. For single-zone deployment, 2 Master nodes are included by default
        """
        return pulumi.get(self, "master_count")

    @master_count.setter
    def master_count(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "master_count", value)

    @_builtins.property
    @pulumi.getter(name="masterSpec")
    def master_spec(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Master node specification code. For information about supported specifications for Master nodes, see Instance Specifications. The number of Master nodes is fixed and cannot be changed.
        """
        return pulumi.get(self, "master_spec")

    @master_spec.setter
    def master_spec(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "master_spec", value)

    @_builtins.property
    @pulumi.getter(name="multiAz")
    def multi_az(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Instance deployment mode. Values: true: multi-availability zone deployment. false: single availability zone deployment. Currently, only single availability zone deployment is supported.
        """
        return pulumi.get(self, "multi_az")

    @multi_az.setter
    def multi_az(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "multi_az", value)

    @_builtins.property
    @pulumi.getter(name="primarySubnetId")
    def primary_subnet_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Real-time primary availability zone subnet ID.
        """
        return pulumi.get(self, "primary_subnet_id")

    @primary_subnet_id.setter
    def primary_subnet_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "primary_subnet_id", value)

    @_builtins.property
    @pulumi.getter(name="primaryZoneId")
    def primary_zone_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Real-time primary zone ID
        """
        return pulumi.get(self, "primary_zone_id")

    @primary_zone_id.setter
    def primary_zone_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "primary_zone_id", value)

    @_builtins.property
    @pulumi.getter(name="projectName")
    def project_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Project name of the instance
        """
        return pulumi.get(self, "project_name")

    @project_name.setter
    def project_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "project_name", value)

    @_builtins.property
    @pulumi.getter(name="purchaseMonths")
    def purchase_months(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Purchase duration, in months. The value range is: 1, 2, 3, 4, 5, 6, 7, 8, 9, 12, 24, 36. When ChargeType is PrePaid, this parameter is required
        """
        return pulumi.get(self, "purchase_months")

    @purchase_months.setter
    def purchase_months(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "purchase_months", value)

    @_builtins.property
    @pulumi.getter(name="regionId")
    def region_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Region ID of the instance.
        """
        return pulumi.get(self, "region_id")

    @region_id.setter
    def region_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "region_id", value)

    @_builtins.property
    @pulumi.getter(name="rsCount")
    def rs_count(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Number of RegionServer nodes
        """
        return pulumi.get(self, "rs_count")

    @rs_count.setter
    def rs_count(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "rs_count", value)

    @_builtins.property
    @pulumi.getter(name="rsSpec")
    def rs_spec(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        RegionServer node specification code.
        """
        return pulumi.get(self, "rs_spec")

    @rs_spec.setter
    def rs_spec(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "rs_spec", value)

    @_builtins.property
    @pulumi.getter(name="standbySubnetId")
    def standby_subnet_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Real-time standby availability zone subnet ID.
        """
        return pulumi.get(self, "standby_subnet_id")

    @standby_subnet_id.setter
    def standby_subnet_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "standby_subnet_id", value)

    @_builtins.property
    @pulumi.getter(name="standbyZoneId")
    def standby_zone_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Real-time standby zone ID
        """
        return pulumi.get(self, "standby_zone_id")

    @standby_zone_id.setter
    def standby_zone_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "standby_zone_id", value)

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Current status of the instance. For more details about instance status, see Instance Status Description
        """
        return pulumi.get(self, "status")

    @status.setter
    def status(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "status", value)

    @_builtins.property
    @pulumi.getter(name="storageCapacity")
    def storage_capacity(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Total storage capacity of the instance, unit: GiB.
        """
        return pulumi.get(self, "storage_capacity")

    @storage_capacity.setter
    def storage_capacity(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "storage_capacity", value)

    @_builtins.property
    @pulumi.getter(name="storageType")
    def storage_type(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance storage type. Values: HdfsHdd: HDD file storage. HdfsSsd: SSD file storage.
        """
        return pulumi.get(self, "storage_type")

    @storage_type.setter
    def storage_type(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "storage_type", value)

    @_builtins.property
    @pulumi.getter(name="subnetId")
    def subnet_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Subnet ID of the instance. For single-zone instances, only one subnet ID for the zone is included
        """
        return pulumi.get(self, "subnet_id")

    @subnet_id.setter
    def subnet_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "subnet_id", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['InstanceTagArgs']]]]:
        """
        Array of tag keys and tag values to bind. Supports passing multiple sets of tag key-value objects at once, separated by commas (,). Up to 20 tag key-value pairs can be passed at a time, and up to 50 tags can be bound to a single instance. Tag keys and values must comply with the setting rules. For details, see Tag Setting Rules
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceTagArgs']]]]):
        pulumi.set(self, "tags", value)

    @_builtins.property
    @pulumi.getter(name="usedColdStorage")
    def used_cold_storage(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Capacity-type storage space used by the instance, in MiB
        """
        return pulumi.get(self, "used_cold_storage")

    @used_cold_storage.setter
    def used_cold_storage(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "used_cold_storage", value)

    @_builtins.property
    @pulumi.getter(name="usedStorage")
    def used_storage(self) -> pulumi.Input[Optional[_builtins.float]]:
        """
        Storage capacity used by the primary instance, in MiB
        """
        return pulumi.get(self, "used_storage")

    @used_storage.setter
    def used_storage(self, value: pulumi.Input[Optional[_builtins.float]]):
        pulumi.set(self, "used_storage", value)

    @_builtins.property
    @pulumi.getter(name="vpcId")
    def vpc_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Private network ID. You can call the DescribeVpcs API to query information about private networks available for HBase instance creation, including the private network ID.
        """
        return pulumi.get(self, "vpc_id")

    @vpc_id.setter
    def vpc_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "vpc_id", value)

    @_builtins.property
    @pulumi.getter(name="vpcName")
    def vpc_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Name of the private network to which the instance belongs.
        """
        return pulumi.get(self, "vpc_name")

    @vpc_name.setter
    def vpc_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "vpc_name", value)

    @_builtins.property
    @pulumi.getter(name="zoneId")
    def zone_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Zone ID of the instance
        """
        return pulumi.get(self, "zone_id")

    @zone_id.setter
    def zone_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "zone_id", value)

    @_builtins.property
    @pulumi.getter(name="zoneName")
    def zone_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Name of the availability zone to which the instance belongs.
        """
        return pulumi.get(self, "zone_name")

    @zone_name.setter
    def zone_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "zone_name", value)


@pulumi.type_token("volcenginecc:hbase/instance:Instance")
class Instance(pulumi.CustomResource):
    @overload
    def __init__(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 auto_renew: pulumi.Input[Optional[_builtins.bool]] = None,
                 charge_type: pulumi.Input[Optional[_builtins.str]] = None,
                 cold_storage: pulumi.Input[Optional[_builtins.int]] = None,
                 deletion_protection: pulumi.Input[Optional[_builtins.str]] = None,
                 enable_auth: pulumi.Input[Optional[_builtins.bool]] = None,
                 enable_cloud_storage: pulumi.Input[Optional[_builtins.bool]] = None,
                 endpoints: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InstanceEndpointArgs', 'InstanceEndpointArgsDict']]]]] = None,
                 engine_version: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_name: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_type: pulumi.Input[Optional[_builtins.str]] = None,
                 master_spec: pulumi.Input[Optional[_builtins.str]] = None,
                 multi_az: pulumi.Input[Optional[_builtins.bool]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 purchase_months: pulumi.Input[Optional[_builtins.int]] = None,
                 region_id: pulumi.Input[Optional[_builtins.str]] = None,
                 rs_count: pulumi.Input[Optional[_builtins.int]] = None,
                 rs_spec: pulumi.Input[Optional[_builtins.str]] = None,
                 storage_capacity: pulumi.Input[Optional[_builtins.int]] = None,
                 storage_type: pulumi.Input[Optional[_builtins.str]] = None,
                 subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InstanceTagArgs', 'InstanceTagArgsDict']]]]] = None,
                 vpc_id: pulumi.Input[Optional[_builtins.str]] = None,
                 zone_id: pulumi.Input[Optional[_builtins.str]] = None,
                 __props__=None):
        """
        Table Database HBase Edition is a fully managed NoSQL service based on Apache HBase, compatible with the standard HBase access protocol, offering advantages such as low-cost storage and high scalability throughput.

        ## Example Usage

        ```python
        import pulumi
        import pulumi_volcenginecc as volcenginecc

        h_base_instance_demo = volcenginecc.hbase.Instance("HBaseInstanceDemo",
            region_id="cn-beijing",
            multi_az=False,
            zone_id="cn-beijing-a",
            instance_name="HBaseInstanceDemo",
            engine_version="HBase_2.0",
            instance_type="Standard",
            master_spec="hbase.x1.medium",
            rs_count=2,
            rs_spec="hbase.x1.large",
            storage_capacity=20000,
            vpc_id="vpc-rrco37ovjq4gv0x5xxxxx",
            project_name="default",
            charge_type="PrePaid",
            purchase_months=1,
            auto_renew=False,
            subnet_id="subnet-rrwqhg3qzxfkv0x57xxxxx",
            deletion_protection="disabled",
            enable_cloud_storage=False,
            tags=[{
                "key": "env",
                "value": "test",
            }],
            storage_type="HdfsSsd",
            enable_auth=True)
        ```

        ## Import

        ```sh
        $ pulumi import volcenginecc:hbase/instance:Instance example "instance_id"
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.bool] auto_renew: Whether auto-renewal is enabled for subscription scenarios. Values: true: auto-renewal enabled. false: auto-renewal not enabled.
        :param pulumi.Input[_builtins.str] charge_type: Billing type. Values: PostPaid: pay-as-you-go (postpaid). PrePaid: subscription (prepaid, yearly/monthly).
        :param pulumi.Input[_builtins.int] cold_storage: Total capacity-type storage space for the instance, in GiB.
        :param pulumi.Input[_builtins.str] deletion_protection: Instance deletion protection switch status. Value range: enabled: enabled. disabled: not enabled. For more information about instance deletion protection, see Instance Deletion Protection.
        :param pulumi.Input[_builtins.bool] enable_auth: Whether login authentication is enabled. Values: true: login authentication enabled. false: login authentication disabled.
        :param pulumi.Input[_builtins.bool] enable_cloud_storage: Whether capacity-type storage is enabled. Values: true: capacity-type storage enabled. false: capacity-type storage not enabled. For details about capacity-type storage, see Hot and Cold Data Separation. The hot and cold data separation feature is supported only when MultiAZ (deployment mode) is set to false (single availability zone deployment).
        :param pulumi.Input[Sequence[pulumi.Input[Union['InstanceEndpointArgs', 'InstanceEndpointArgsDict']]]] endpoints: Connection address information list
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] engine_version: HBase database engine version. Currently, only HBase 2.0 is supported. The default value is HBase_2.0
        :param pulumi.Input[_builtins.str] instance_name: Instance name
        :param pulumi.Input[_builtins.str] instance_type: Instance type. Currently, only Standard is supported. The default value is Standard
        :param pulumi.Input[_builtins.str] master_spec: Master node specification code. For information about supported specifications for Master nodes, see Instance Specifications. The number of Master nodes is fixed and cannot be changed.
        :param pulumi.Input[_builtins.bool] multi_az: Instance deployment mode. Values: true: multi-availability zone deployment. false: single availability zone deployment. Currently, only single availability zone deployment is supported.
        :param pulumi.Input[_builtins.str] project_name: Project name of the instance
        :param pulumi.Input[_builtins.int] purchase_months: Purchase duration, in months. The value range is: 1, 2, 3, 4, 5, 6, 7, 8, 9, 12, 24, 36. When ChargeType is PrePaid, this parameter is required
        :param pulumi.Input[_builtins.str] region_id: Region ID of the instance.
        :param pulumi.Input[_builtins.int] rs_count: Number of RegionServer nodes
        :param pulumi.Input[_builtins.str] rs_spec: RegionServer node specification code.
        :param pulumi.Input[_builtins.int] storage_capacity: Total storage capacity of the instance, unit: GiB.
        :param pulumi.Input[_builtins.str] storage_type: Instance storage type. Values: HdfsHdd: HDD file storage. HdfsSsd: SSD file storage.
        :param pulumi.Input[_builtins.str] subnet_id: Subnet ID of the instance. For single-zone instances, only one subnet ID for the zone is included
        :param pulumi.Input[Sequence[pulumi.Input[Union['InstanceTagArgs', 'InstanceTagArgsDict']]]] tags: Array of tag keys and tag values to bind. Supports passing multiple sets of tag key-value objects at once, separated by commas (,). Up to 20 tag key-value pairs can be passed at a time, and up to 50 tags can be bound to a single instance. Tag keys and values must comply with the setting rules. For details, see Tag Setting Rules
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] vpc_id: Private network ID. You can call the DescribeVpcs API to query information about private networks available for HBase instance creation, including the private network ID.
        :param pulumi.Input[_builtins.str] zone_id: Zone ID of the instance
        """
        ...
    @overload
    def __init__(__self__,
                 resource_name: str,
                 args: InstanceArgs,
                 opts: Optional[pulumi.ResourceOptions] = None):
        """
        Table Database HBase Edition is a fully managed NoSQL service based on Apache HBase, compatible with the standard HBase access protocol, offering advantages such as low-cost storage and high scalability throughput.

        ## Example Usage

        ```python
        import pulumi
        import pulumi_volcenginecc as volcenginecc

        h_base_instance_demo = volcenginecc.hbase.Instance("HBaseInstanceDemo",
            region_id="cn-beijing",
            multi_az=False,
            zone_id="cn-beijing-a",
            instance_name="HBaseInstanceDemo",
            engine_version="HBase_2.0",
            instance_type="Standard",
            master_spec="hbase.x1.medium",
            rs_count=2,
            rs_spec="hbase.x1.large",
            storage_capacity=20000,
            vpc_id="vpc-rrco37ovjq4gv0x5xxxxx",
            project_name="default",
            charge_type="PrePaid",
            purchase_months=1,
            auto_renew=False,
            subnet_id="subnet-rrwqhg3qzxfkv0x57xxxxx",
            deletion_protection="disabled",
            enable_cloud_storage=False,
            tags=[{
                "key": "env",
                "value": "test",
            }],
            storage_type="HdfsSsd",
            enable_auth=True)
        ```

        ## Import

        ```sh
        $ pulumi import volcenginecc:hbase/instance:Instance example "instance_id"
        ```


        :param str resource_name: The name of the resource.
        :param InstanceArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        ...
    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(InstanceArgs, pulumi.ResourceOptions, *args, **kwargs)
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 auto_renew: pulumi.Input[Optional[_builtins.bool]] = None,
                 charge_type: pulumi.Input[Optional[_builtins.str]] = None,
                 cold_storage: pulumi.Input[Optional[_builtins.int]] = None,
                 deletion_protection: pulumi.Input[Optional[_builtins.str]] = None,
                 enable_auth: pulumi.Input[Optional[_builtins.bool]] = None,
                 enable_cloud_storage: pulumi.Input[Optional[_builtins.bool]] = None,
                 endpoints: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InstanceEndpointArgs', 'InstanceEndpointArgsDict']]]]] = None,
                 engine_version: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_name: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_type: pulumi.Input[Optional[_builtins.str]] = None,
                 master_spec: pulumi.Input[Optional[_builtins.str]] = None,
                 multi_az: pulumi.Input[Optional[_builtins.bool]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 purchase_months: pulumi.Input[Optional[_builtins.int]] = None,
                 region_id: pulumi.Input[Optional[_builtins.str]] = None,
                 rs_count: pulumi.Input[Optional[_builtins.int]] = None,
                 rs_spec: pulumi.Input[Optional[_builtins.str]] = None,
                 storage_capacity: pulumi.Input[Optional[_builtins.int]] = None,
                 storage_type: pulumi.Input[Optional[_builtins.str]] = None,
                 subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InstanceTagArgs', 'InstanceTagArgsDict']]]]] = None,
                 vpc_id: pulumi.Input[Optional[_builtins.str]] = None,
                 zone_id: pulumi.Input[Optional[_builtins.str]] = None,
                 __props__=None):
        opts = pulumi.ResourceOptions.merge(_utilities.get_resource_opts_defaults(), opts)
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError('Expected resource options to be a ResourceOptions instance')
        if opts.id is None:
            if __props__ is not None:
                raise TypeError('__props__ is only valid when passed in combination with a valid opts.id to get an existing resource')
            __props__ = InstanceArgs.__new__(InstanceArgs)

            __props__.__dict__["auto_renew"] = auto_renew
            if charge_type is None and not opts.urn:
                raise TypeError("Missing required property 'charge_type'")
            __props__.__dict__["charge_type"] = charge_type
            __props__.__dict__["cold_storage"] = cold_storage
            __props__.__dict__["deletion_protection"] = deletion_protection
            __props__.__dict__["enable_auth"] = enable_auth
            __props__.__dict__["enable_cloud_storage"] = enable_cloud_storage
            __props__.__dict__["endpoints"] = endpoints
            if engine_version is None and not opts.urn:
                raise TypeError("Missing required property 'engine_version'")
            __props__.__dict__["engine_version"] = engine_version
            __props__.__dict__["instance_name"] = instance_name
            __props__.__dict__["instance_type"] = instance_type
            if master_spec is None and not opts.urn:
                raise TypeError("Missing required property 'master_spec'")
            __props__.__dict__["master_spec"] = master_spec
            __props__.__dict__["multi_az"] = multi_az
            __props__.__dict__["project_name"] = project_name
            __props__.__dict__["purchase_months"] = purchase_months
            if region_id is None and not opts.urn:
                raise TypeError("Missing required property 'region_id'")
            __props__.__dict__["region_id"] = region_id
            if rs_count is None and not opts.urn:
                raise TypeError("Missing required property 'rs_count'")
            __props__.__dict__["rs_count"] = rs_count
            if rs_spec is None and not opts.urn:
                raise TypeError("Missing required property 'rs_spec'")
            __props__.__dict__["rs_spec"] = rs_spec
            if storage_capacity is None and not opts.urn:
                raise TypeError("Missing required property 'storage_capacity'")
            __props__.__dict__["storage_capacity"] = storage_capacity
            if storage_type is None and not opts.urn:
                raise TypeError("Missing required property 'storage_type'")
            __props__.__dict__["storage_type"] = storage_type
            if subnet_id is None and not opts.urn:
                raise TypeError("Missing required property 'subnet_id'")
            __props__.__dict__["subnet_id"] = subnet_id
            __props__.__dict__["tags"] = tags
            if vpc_id is None and not opts.urn:
                raise TypeError("Missing required property 'vpc_id'")
            __props__.__dict__["vpc_id"] = vpc_id
            if zone_id is None and not opts.urn:
                raise TypeError("Missing required property 'zone_id'")
            __props__.__dict__["zone_id"] = zone_id
            __props__.__dict__["charge_status"] = None
            __props__.__dict__["create_time"] = None
            __props__.__dict__["instance_id"] = None
            __props__.__dict__["master_count"] = None
            __props__.__dict__["primary_subnet_id"] = None
            __props__.__dict__["primary_zone_id"] = None
            __props__.__dict__["standby_subnet_id"] = None
            __props__.__dict__["standby_zone_id"] = None
            __props__.__dict__["status"] = None
            __props__.__dict__["used_cold_storage"] = None
            __props__.__dict__["used_storage"] = None
            __props__.__dict__["vpc_name"] = None
            __props__.__dict__["zone_name"] = None
        super(Instance, __self__).__init__(
            'volcenginecc:hbase/instance:Instance',
            resource_name,
            __props__,
            opts)

    @staticmethod
    def get(resource_name: str,
            id: pulumi.Input[str],
            opts: Optional[pulumi.ResourceOptions] = None,
            auto_renew: pulumi.Input[Optional[_builtins.bool]] = None,
            charge_status: pulumi.Input[Optional[_builtins.str]] = None,
            charge_type: pulumi.Input[Optional[_builtins.str]] = None,
            cold_storage: pulumi.Input[Optional[_builtins.int]] = None,
            create_time: pulumi.Input[Optional[_builtins.str]] = None,
            deletion_protection: pulumi.Input[Optional[_builtins.str]] = None,
            enable_auth: pulumi.Input[Optional[_builtins.bool]] = None,
            enable_cloud_storage: pulumi.Input[Optional[_builtins.bool]] = None,
            endpoints: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InstanceEndpointArgs', 'InstanceEndpointArgsDict']]]]] = None,
            engine_version: pulumi.Input[Optional[_builtins.str]] = None,
            instance_id: pulumi.Input[Optional[_builtins.str]] = None,
            instance_name: pulumi.Input[Optional[_builtins.str]] = None,
            instance_type: pulumi.Input[Optional[_builtins.str]] = None,
            master_count: pulumi.Input[Optional[_builtins.int]] = None,
            master_spec: pulumi.Input[Optional[_builtins.str]] = None,
            multi_az: pulumi.Input[Optional[_builtins.bool]] = None,
            primary_subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
            primary_zone_id: pulumi.Input[Optional[_builtins.str]] = None,
            project_name: pulumi.Input[Optional[_builtins.str]] = None,
            purchase_months: pulumi.Input[Optional[_builtins.int]] = None,
            region_id: pulumi.Input[Optional[_builtins.str]] = None,
            rs_count: pulumi.Input[Optional[_builtins.int]] = None,
            rs_spec: pulumi.Input[Optional[_builtins.str]] = None,
            standby_subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
            standby_zone_id: pulumi.Input[Optional[_builtins.str]] = None,
            status: pulumi.Input[Optional[_builtins.str]] = None,
            storage_capacity: pulumi.Input[Optional[_builtins.int]] = None,
            storage_type: pulumi.Input[Optional[_builtins.str]] = None,
            subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
            tags: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InstanceTagArgs', 'InstanceTagArgsDict']]]]] = None,
            used_cold_storage: pulumi.Input[Optional[_builtins.int]] = None,
            used_storage: pulumi.Input[Optional[_builtins.float]] = None,
            vpc_id: pulumi.Input[Optional[_builtins.str]] = None,
            vpc_name: pulumi.Input[Optional[_builtins.str]] = None,
            zone_id: pulumi.Input[Optional[_builtins.str]] = None,
            zone_name: pulumi.Input[Optional[_builtins.str]] = None) -> 'Instance':
        """
        Get an existing Instance resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.bool] auto_renew: Whether auto-renewal is enabled for subscription scenarios. Values: true: auto-renewal enabled. false: auto-renewal not enabled.
        :param pulumi.Input[_builtins.str] charge_status: Billing status. Values: Normal: normal. Overdue: overdue. Shutdown: shutdown.
        :param pulumi.Input[_builtins.str] charge_type: Billing type. Values: PostPaid: pay-as-you-go (postpaid). PrePaid: subscription (prepaid, yearly/monthly).
        :param pulumi.Input[_builtins.int] cold_storage: Total capacity-type storage space for the instance, in GiB.
        :param pulumi.Input[_builtins.str] create_time: Instance creation time (UTC).
        :param pulumi.Input[_builtins.str] deletion_protection: Instance deletion protection switch status. Value range: enabled: enabled. disabled: not enabled. For more information about instance deletion protection, see Instance Deletion Protection.
        :param pulumi.Input[_builtins.bool] enable_auth: Whether login authentication is enabled. Values: true: login authentication enabled. false: login authentication disabled.
        :param pulumi.Input[_builtins.bool] enable_cloud_storage: Whether capacity-type storage is enabled. Values: true: capacity-type storage enabled. false: capacity-type storage not enabled. For details about capacity-type storage, see Hot and Cold Data Separation. The hot and cold data separation feature is supported only when MultiAZ (deployment mode) is set to false (single availability zone deployment).
        :param pulumi.Input[Sequence[pulumi.Input[Union['InstanceEndpointArgs', 'InstanceEndpointArgsDict']]]] endpoints: Connection address information list
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] engine_version: HBase database engine version. Currently, only HBase 2.0 is supported. The default value is HBase_2.0
        :param pulumi.Input[_builtins.str] instance_id: Instance ID
        :param pulumi.Input[_builtins.str] instance_name: Instance name
        :param pulumi.Input[_builtins.str] instance_type: Instance type. Currently, only Standard is supported. The default value is Standard
        :param pulumi.Input[_builtins.int] master_count: Number of Master nodes. For single-zone deployment, 2 Master nodes are included by default
        :param pulumi.Input[_builtins.str] master_spec: Master node specification code. For information about supported specifications for Master nodes, see Instance Specifications. The number of Master nodes is fixed and cannot be changed.
        :param pulumi.Input[_builtins.bool] multi_az: Instance deployment mode. Values: true: multi-availability zone deployment. false: single availability zone deployment. Currently, only single availability zone deployment is supported.
        :param pulumi.Input[_builtins.str] primary_subnet_id: Real-time primary availability zone subnet ID.
        :param pulumi.Input[_builtins.str] primary_zone_id: Real-time primary zone ID
        :param pulumi.Input[_builtins.str] project_name: Project name of the instance
        :param pulumi.Input[_builtins.int] purchase_months: Purchase duration, in months. The value range is: 1, 2, 3, 4, 5, 6, 7, 8, 9, 12, 24, 36. When ChargeType is PrePaid, this parameter is required
        :param pulumi.Input[_builtins.str] region_id: Region ID of the instance.
        :param pulumi.Input[_builtins.int] rs_count: Number of RegionServer nodes
        :param pulumi.Input[_builtins.str] rs_spec: RegionServer node specification code.
        :param pulumi.Input[_builtins.str] standby_subnet_id: Real-time standby availability zone subnet ID.
        :param pulumi.Input[_builtins.str] standby_zone_id: Real-time standby zone ID
        :param pulumi.Input[_builtins.str] status: Current status of the instance. For more details about instance status, see Instance Status Description
        :param pulumi.Input[_builtins.int] storage_capacity: Total storage capacity of the instance, unit: GiB.
        :param pulumi.Input[_builtins.str] storage_type: Instance storage type. Values: HdfsHdd: HDD file storage. HdfsSsd: SSD file storage.
        :param pulumi.Input[_builtins.str] subnet_id: Subnet ID of the instance. For single-zone instances, only one subnet ID for the zone is included
        :param pulumi.Input[Sequence[pulumi.Input[Union['InstanceTagArgs', 'InstanceTagArgsDict']]]] tags: Array of tag keys and tag values to bind. Supports passing multiple sets of tag key-value objects at once, separated by commas (,). Up to 20 tag key-value pairs can be passed at a time, and up to 50 tags can be bound to a single instance. Tag keys and values must comply with the setting rules. For details, see Tag Setting Rules
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.int] used_cold_storage: Capacity-type storage space used by the instance, in MiB
        :param pulumi.Input[_builtins.float] used_storage: Storage capacity used by the primary instance, in MiB
        :param pulumi.Input[_builtins.str] vpc_id: Private network ID. You can call the DescribeVpcs API to query information about private networks available for HBase instance creation, including the private network ID.
        :param pulumi.Input[_builtins.str] vpc_name: Name of the private network to which the instance belongs.
        :param pulumi.Input[_builtins.str] zone_id: Zone ID of the instance
        :param pulumi.Input[_builtins.str] zone_name: Name of the availability zone to which the instance belongs.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _InstanceState.__new__(_InstanceState)

        __props__.__dict__["auto_renew"] = auto_renew
        __props__.__dict__["charge_status"] = charge_status
        __props__.__dict__["charge_type"] = charge_type
        __props__.__dict__["cold_storage"] = cold_storage
        __props__.__dict__["create_time"] = create_time
        __props__.__dict__["deletion_protection"] = deletion_protection
        __props__.__dict__["enable_auth"] = enable_auth
        __props__.__dict__["enable_cloud_storage"] = enable_cloud_storage
        __props__.__dict__["endpoints"] = endpoints
        __props__.__dict__["engine_version"] = engine_version
        __props__.__dict__["instance_id"] = instance_id
        __props__.__dict__["instance_name"] = instance_name
        __props__.__dict__["instance_type"] = instance_type
        __props__.__dict__["master_count"] = master_count
        __props__.__dict__["master_spec"] = master_spec
        __props__.__dict__["multi_az"] = multi_az
        __props__.__dict__["primary_subnet_id"] = primary_subnet_id
        __props__.__dict__["primary_zone_id"] = primary_zone_id
        __props__.__dict__["project_name"] = project_name
        __props__.__dict__["purchase_months"] = purchase_months
        __props__.__dict__["region_id"] = region_id
        __props__.__dict__["rs_count"] = rs_count
        __props__.__dict__["rs_spec"] = rs_spec
        __props__.__dict__["standby_subnet_id"] = standby_subnet_id
        __props__.__dict__["standby_zone_id"] = standby_zone_id
        __props__.__dict__["status"] = status
        __props__.__dict__["storage_capacity"] = storage_capacity
        __props__.__dict__["storage_type"] = storage_type
        __props__.__dict__["subnet_id"] = subnet_id
        __props__.__dict__["tags"] = tags
        __props__.__dict__["used_cold_storage"] = used_cold_storage
        __props__.__dict__["used_storage"] = used_storage
        __props__.__dict__["vpc_id"] = vpc_id
        __props__.__dict__["vpc_name"] = vpc_name
        __props__.__dict__["zone_id"] = zone_id
        __props__.__dict__["zone_name"] = zone_name
        return Instance(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="autoRenew")
    def auto_renew(self) -> pulumi.Output[_builtins.bool]:
        """
        Whether auto-renewal is enabled for subscription scenarios. Values: true: auto-renewal enabled. false: auto-renewal not enabled.
        """
        return pulumi.get(self, "auto_renew")

    @_builtins.property
    @pulumi.getter(name="chargeStatus")
    def charge_status(self) -> pulumi.Output[_builtins.str]:
        """
        Billing status. Values: Normal: normal. Overdue: overdue. Shutdown: shutdown.
        """
        return pulumi.get(self, "charge_status")

    @_builtins.property
    @pulumi.getter(name="chargeType")
    def charge_type(self) -> pulumi.Output[_builtins.str]:
        """
        Billing type. Values: PostPaid: pay-as-you-go (postpaid). PrePaid: subscription (prepaid, yearly/monthly).
        """
        return pulumi.get(self, "charge_type")

    @_builtins.property
    @pulumi.getter(name="coldStorage")
    def cold_storage(self) -> pulumi.Output[_builtins.int]:
        """
        Total capacity-type storage space for the instance, in GiB.
        """
        return pulumi.get(self, "cold_storage")

    @_builtins.property
    @pulumi.getter(name="createTime")
    def create_time(self) -> pulumi.Output[_builtins.str]:
        """
        Instance creation time (UTC).
        """
        return pulumi.get(self, "create_time")

    @_builtins.property
    @pulumi.getter(name="deletionProtection")
    def deletion_protection(self) -> pulumi.Output[_builtins.str]:
        """
        Instance deletion protection switch status. Value range: enabled: enabled. disabled: not enabled. For more information about instance deletion protection, see Instance Deletion Protection.
        """
        return pulumi.get(self, "deletion_protection")

    @_builtins.property
    @pulumi.getter(name="enableAuth")
    def enable_auth(self) -> pulumi.Output[_builtins.bool]:
        """
        Whether login authentication is enabled. Values: true: login authentication enabled. false: login authentication disabled.
        """
        return pulumi.get(self, "enable_auth")

    @_builtins.property
    @pulumi.getter(name="enableCloudStorage")
    def enable_cloud_storage(self) -> pulumi.Output[_builtins.bool]:
        """
        Whether capacity-type storage is enabled. Values: true: capacity-type storage enabled. false: capacity-type storage not enabled. For details about capacity-type storage, see Hot and Cold Data Separation. The hot and cold data separation feature is supported only when MultiAZ (deployment mode) is set to false (single availability zone deployment).
        """
        return pulumi.get(self, "enable_cloud_storage")

    @_builtins.property
    @pulumi.getter
    def endpoints(self) -> pulumi.Output[Sequence['outputs.InstanceEndpoint']]:
        """
        Connection address information list
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "endpoints")

    @_builtins.property
    @pulumi.getter(name="engineVersion")
    def engine_version(self) -> pulumi.Output[_builtins.str]:
        """
        HBase database engine version. Currently, only HBase 2.0 is supported. The default value is HBase_2.0
        """
        return pulumi.get(self, "engine_version")

    @_builtins.property
    @pulumi.getter(name="instanceId")
    def instance_id(self) -> pulumi.Output[_builtins.str]:
        """
        Instance ID
        """
        return pulumi.get(self, "instance_id")

    @_builtins.property
    @pulumi.getter(name="instanceName")
    def instance_name(self) -> pulumi.Output[_builtins.str]:
        """
        Instance name
        """
        return pulumi.get(self, "instance_name")

    @_builtins.property
    @pulumi.getter(name="instanceType")
    def instance_type(self) -> pulumi.Output[_builtins.str]:
        """
        Instance type. Currently, only Standard is supported. The default value is Standard
        """
        return pulumi.get(self, "instance_type")

    @_builtins.property
    @pulumi.getter(name="masterCount")
    def master_count(self) -> pulumi.Output[_builtins.int]:
        """
        Number of Master nodes. For single-zone deployment, 2 Master nodes are included by default
        """
        return pulumi.get(self, "master_count")

    @_builtins.property
    @pulumi.getter(name="masterSpec")
    def master_spec(self) -> pulumi.Output[_builtins.str]:
        """
        Master node specification code. For information about supported specifications for Master nodes, see Instance Specifications. The number of Master nodes is fixed and cannot be changed.
        """
        return pulumi.get(self, "master_spec")

    @_builtins.property
    @pulumi.getter(name="multiAz")
    def multi_az(self) -> pulumi.Output[_builtins.bool]:
        """
        Instance deployment mode. Values: true: multi-availability zone deployment. false: single availability zone deployment. Currently, only single availability zone deployment is supported.
        """
        return pulumi.get(self, "multi_az")

    @_builtins.property
    @pulumi.getter(name="primarySubnetId")
    def primary_subnet_id(self) -> pulumi.Output[_builtins.str]:
        """
        Real-time primary availability zone subnet ID.
        """
        return pulumi.get(self, "primary_subnet_id")

    @_builtins.property
    @pulumi.getter(name="primaryZoneId")
    def primary_zone_id(self) -> pulumi.Output[_builtins.str]:
        """
        Real-time primary zone ID
        """
        return pulumi.get(self, "primary_zone_id")

    @_builtins.property
    @pulumi.getter(name="projectName")
    def project_name(self) -> pulumi.Output[_builtins.str]:
        """
        Project name of the instance
        """
        return pulumi.get(self, "project_name")

    @_builtins.property
    @pulumi.getter(name="purchaseMonths")
    def purchase_months(self) -> pulumi.Output[_builtins.int]:
        """
        Purchase duration, in months. The value range is: 1, 2, 3, 4, 5, 6, 7, 8, 9, 12, 24, 36. When ChargeType is PrePaid, this parameter is required
        """
        return pulumi.get(self, "purchase_months")

    @_builtins.property
    @pulumi.getter(name="regionId")
    def region_id(self) -> pulumi.Output[_builtins.str]:
        """
        Region ID of the instance.
        """
        return pulumi.get(self, "region_id")

    @_builtins.property
    @pulumi.getter(name="rsCount")
    def rs_count(self) -> pulumi.Output[_builtins.int]:
        """
        Number of RegionServer nodes
        """
        return pulumi.get(self, "rs_count")

    @_builtins.property
    @pulumi.getter(name="rsSpec")
    def rs_spec(self) -> pulumi.Output[_builtins.str]:
        """
        RegionServer node specification code.
        """
        return pulumi.get(self, "rs_spec")

    @_builtins.property
    @pulumi.getter(name="standbySubnetId")
    def standby_subnet_id(self) -> pulumi.Output[_builtins.str]:
        """
        Real-time standby availability zone subnet ID.
        """
        return pulumi.get(self, "standby_subnet_id")

    @_builtins.property
    @pulumi.getter(name="standbyZoneId")
    def standby_zone_id(self) -> pulumi.Output[_builtins.str]:
        """
        Real-time standby zone ID
        """
        return pulumi.get(self, "standby_zone_id")

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Output[_builtins.str]:
        """
        Current status of the instance. For more details about instance status, see Instance Status Description
        """
        return pulumi.get(self, "status")

    @_builtins.property
    @pulumi.getter(name="storageCapacity")
    def storage_capacity(self) -> pulumi.Output[_builtins.int]:
        """
        Total storage capacity of the instance, unit: GiB.
        """
        return pulumi.get(self, "storage_capacity")

    @_builtins.property
    @pulumi.getter(name="storageType")
    def storage_type(self) -> pulumi.Output[_builtins.str]:
        """
        Instance storage type. Values: HdfsHdd: HDD file storage. HdfsSsd: SSD file storage.
        """
        return pulumi.get(self, "storage_type")

    @_builtins.property
    @pulumi.getter(name="subnetId")
    def subnet_id(self) -> pulumi.Output[_builtins.str]:
        """
        Subnet ID of the instance. For single-zone instances, only one subnet ID for the zone is included
        """
        return pulumi.get(self, "subnet_id")

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Output[Sequence['outputs.InstanceTag']]:
        """
        Array of tag keys and tag values to bind. Supports passing multiple sets of tag key-value objects at once, separated by commas (,). Up to 20 tag key-value pairs can be passed at a time, and up to 50 tags can be bound to a single instance. Tag keys and values must comply with the setting rules. For details, see Tag Setting Rules
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "tags")

    @_builtins.property
    @pulumi.getter(name="usedColdStorage")
    def used_cold_storage(self) -> pulumi.Output[_builtins.int]:
        """
        Capacity-type storage space used by the instance, in MiB
        """
        return pulumi.get(self, "used_cold_storage")

    @_builtins.property
    @pulumi.getter(name="usedStorage")
    def used_storage(self) -> pulumi.Output[_builtins.float]:
        """
        Storage capacity used by the primary instance, in MiB
        """
        return pulumi.get(self, "used_storage")

    @_builtins.property
    @pulumi.getter(name="vpcId")
    def vpc_id(self) -> pulumi.Output[_builtins.str]:
        """
        Private network ID. You can call the DescribeVpcs API to query information about private networks available for HBase instance creation, including the private network ID.
        """
        return pulumi.get(self, "vpc_id")

    @_builtins.property
    @pulumi.getter(name="vpcName")
    def vpc_name(self) -> pulumi.Output[_builtins.str]:
        """
        Name of the private network to which the instance belongs.
        """
        return pulumi.get(self, "vpc_name")

    @_builtins.property
    @pulumi.getter(name="zoneId")
    def zone_id(self) -> pulumi.Output[_builtins.str]:
        """
        Zone ID of the instance
        """
        return pulumi.get(self, "zone_id")

    @_builtins.property
    @pulumi.getter(name="zoneName")
    def zone_name(self) -> pulumi.Output[_builtins.str]:
        """
        Name of the availability zone to which the instance belongs.
        """
        return pulumi.get(self, "zone_name")

