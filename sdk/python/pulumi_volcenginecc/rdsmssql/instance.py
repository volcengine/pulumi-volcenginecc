# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = ['InstanceArgs', 'Instance']

@pulumi.input_type
class InstanceArgs:
    def __init__(__self__, *,
                 db_engine_version: pulumi.Input[_builtins.str],
                 instance_type: pulumi.Input[_builtins.str],
                 node_spec: pulumi.Input[_builtins.str],
                 storage_space: pulumi.Input[_builtins.int],
                 subnet_id: pulumi.Input[_builtins.str],
                 super_account_password: pulumi.Input[_builtins.str],
                 time_zone: pulumi.Input[_builtins.str],
                 vpc_id: pulumi.Input[_builtins.str],
                 zone_id: pulumi.Input[_builtins.str],
                 advanced_features: pulumi.Input[Optional[_builtins.str]] = None,
                 allow_list_ids: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]] = None,
                 charge_info: pulumi.Input[Optional['InstanceChargeInfoArgs']] = None,
                 instance_name: pulumi.Input[Optional[_builtins.str]] = None,
                 maintenance_time: pulumi.Input[Optional[_builtins.str]] = None,
                 master_node_id: pulumi.Input[Optional[_builtins.str]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 server_collation: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceTagArgs']]]] = None):
        """
        The set of arguments for constructing a Instance resource.

        :param pulumi.Input[_builtins.str] db_engine_version: Compatible version. Values: SQLServer*2019*Std: SQL Server 2019 Standard Edition. SQLServer*2019*Ent: SQL Server 2019 Enterprise Edition. SQLServer*2019*Web: SQL Server 2019 Web Edition. SQLServer*2022*Std: SQL Server 2022 Standard Edition. SQLServer*2022*Ent: SQL Server 2022 Enterprise Edition. SQLServer*2022*Web: SQL Server 2022 Web Edition.
        :param pulumi.Input[_builtins.str] instance_type: Instance type. Available values: HA (High Availability), Basic, Cluster. Supported types vary by engine version: 2019 Std supports HA/Basic; 2019 Ent supports Cluster/Basic; 2019 Web supports Basic.
        :param pulumi.Input[_builtins.str] node_spec: Instance specification code.
        :param pulumi.Input[_builtins.int] storage_space: Storage size, in GiB. Range: 20 GiB ~ 4000 GiB, increment: 10 GiB.
        :param pulumi.Input[_builtins.str] subnet_id: Subnet ID. If the primary and standby nodes are in different availability zones, their subnets must be set to the corresponding private network subnets for each zone. Use a semicolon (;) to separate multiple zones.
        :param pulumi.Input[_builtins.str] super_account_password: Password for the high-privilege account. Password rules: 8–32 characters; must include at least three of the following: uppercase letters, lowercase letters, numbers, special characters. Allowed special characters: !@#$%^&*()_+-=.
        :param pulumi.Input[_builtins.str] time_zone: Time zone.
        :param pulumi.Input[_builtins.str] vpc_id: VPC ID
        :param pulumi.Input[_builtins.str] zone_id: Availability zone of the node. If the primary and standby nodes are in different availability zones, use a semicolon (;) to separate them. The first is the primary node's zone by default, the second is the standby node's zone.
        :param pulumi.Input[_builtins.str] advanced_features: Advanced instance features to modify. This API currently requires allowlisting. To use it, contact technical support. Example value: {"Key":"Value"}
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] allow_list_ids: Allowlist ID list permitted for access.
        :param pulumi.Input['InstanceChargeInfoArgs'] charge_info: Payment method.
        :param pulumi.Input[_builtins.str] instance_name: Instance name. Naming rules: Cannot start with a digit or hyphen. Can only contain Chinese characters, letters, digits, underscores, and hyphens. Length must be between 1 and 128 characters.
        :param pulumi.Input[_builtins.str] maintenance_time: Instance maintenance window. Format: HH:mmZ-HH:mmZ (UTC time). Default: UTC18:00Z-21:59Z (Beijing time 02:00-05:59). Note: To ensure cloud database stability, the system performs maintenance on instances periodically. It is recommended to set the maintenance window during off-peak hours to avoid business impact. During the maintenance window, the instance may experience 1–2 brief disconnections. Ensure your application supports automatic reconnection. The minimum maintenance window is 1 hour, the maximum is 24 hours, and cross-day maintenance windows are not allowed.
        :param pulumi.Input[_builtins.str] master_node_id: Primary node ID. This parameter is required when manually switching master and standby nodes.
        :param pulumi.Input[_builtins.str] project_name: Project.
        :param pulumi.Input[_builtins.str] server_collation: Instance collation. Default: Chinese*PRC*CI*AS. Most native collations are supported. Available values: Latin1*General*CI*AS, Latin1*General*CS*AS, SQL*Latin1*General*CP1*CI*AS, SQL*Latin1*General*CP1*CS*AS, Chinese*PRC*CI*AS, Chinese*PRC*CS*AS, Chinese*PRC*BIN, Japanese*CI*AS, Japanese*CS*AS, Chinese*Taiwan*Stroke*CI*AS, Chinese*Taiwan*Stroke*CS*AS, Thai*CI*AS, Chinese*PRC*CI*AI, Chinese*PRC*BIN2, Cyrillic*General*CI_AS.
        :param pulumi.Input[Sequence[pulumi.Input['InstanceTagArgs']]] tags: Tag list.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        pulumi.set(__self__, "db_engine_version", db_engine_version)
        pulumi.set(__self__, "instance_type", instance_type)
        pulumi.set(__self__, "node_spec", node_spec)
        pulumi.set(__self__, "storage_space", storage_space)
        pulumi.set(__self__, "subnet_id", subnet_id)
        pulumi.set(__self__, "super_account_password", super_account_password)
        pulumi.set(__self__, "time_zone", time_zone)
        pulumi.set(__self__, "vpc_id", vpc_id)
        pulumi.set(__self__, "zone_id", zone_id)
        if advanced_features is not None:
            pulumi.set(__self__, "advanced_features", advanced_features)
        if allow_list_ids is not None:
            pulumi.set(__self__, "allow_list_ids", allow_list_ids)
        if charge_info is not None:
            pulumi.set(__self__, "charge_info", charge_info)
        if instance_name is not None:
            pulumi.set(__self__, "instance_name", instance_name)
        if maintenance_time is not None:
            pulumi.set(__self__, "maintenance_time", maintenance_time)
        if master_node_id is not None:
            pulumi.set(__self__, "master_node_id", master_node_id)
        if project_name is not None:
            pulumi.set(__self__, "project_name", project_name)
        if server_collation is not None:
            pulumi.set(__self__, "server_collation", server_collation)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)

    @_builtins.property
    @pulumi.getter(name="dbEngineVersion")
    def db_engine_version(self) -> pulumi.Input[_builtins.str]:
        """
        Compatible version. Values: SQLServer*2019*Std: SQL Server 2019 Standard Edition. SQLServer*2019*Ent: SQL Server 2019 Enterprise Edition. SQLServer*2019*Web: SQL Server 2019 Web Edition. SQLServer*2022*Std: SQL Server 2022 Standard Edition. SQLServer*2022*Ent: SQL Server 2022 Enterprise Edition. SQLServer*2022*Web: SQL Server 2022 Web Edition.
        """
        return pulumi.get(self, "db_engine_version")

    @db_engine_version.setter
    def db_engine_version(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "db_engine_version", value)

    @_builtins.property
    @pulumi.getter(name="instanceType")
    def instance_type(self) -> pulumi.Input[_builtins.str]:
        """
        Instance type. Available values: HA (High Availability), Basic, Cluster. Supported types vary by engine version: 2019 Std supports HA/Basic; 2019 Ent supports Cluster/Basic; 2019 Web supports Basic.
        """
        return pulumi.get(self, "instance_type")

    @instance_type.setter
    def instance_type(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "instance_type", value)

    @_builtins.property
    @pulumi.getter(name="nodeSpec")
    def node_spec(self) -> pulumi.Input[_builtins.str]:
        """
        Instance specification code.
        """
        return pulumi.get(self, "node_spec")

    @node_spec.setter
    def node_spec(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "node_spec", value)

    @_builtins.property
    @pulumi.getter(name="storageSpace")
    def storage_space(self) -> pulumi.Input[_builtins.int]:
        """
        Storage size, in GiB. Range: 20 GiB ~ 4000 GiB, increment: 10 GiB.
        """
        return pulumi.get(self, "storage_space")

    @storage_space.setter
    def storage_space(self, value: pulumi.Input[_builtins.int]):
        pulumi.set(self, "storage_space", value)

    @_builtins.property
    @pulumi.getter(name="subnetId")
    def subnet_id(self) -> pulumi.Input[_builtins.str]:
        """
        Subnet ID. If the primary and standby nodes are in different availability zones, their subnets must be set to the corresponding private network subnets for each zone. Use a semicolon (;) to separate multiple zones.
        """
        return pulumi.get(self, "subnet_id")

    @subnet_id.setter
    def subnet_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "subnet_id", value)

    @_builtins.property
    @pulumi.getter(name="superAccountPassword")
    def super_account_password(self) -> pulumi.Input[_builtins.str]:
        """
        Password for the high-privilege account. Password rules: 8–32 characters; must include at least three of the following: uppercase letters, lowercase letters, numbers, special characters. Allowed special characters: !@#$%^&*()_+-=.
        """
        return pulumi.get(self, "super_account_password")

    @super_account_password.setter
    def super_account_password(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "super_account_password", value)

    @_builtins.property
    @pulumi.getter(name="timeZone")
    def time_zone(self) -> pulumi.Input[_builtins.str]:
        """
        Time zone.
        """
        return pulumi.get(self, "time_zone")

    @time_zone.setter
    def time_zone(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "time_zone", value)

    @_builtins.property
    @pulumi.getter(name="vpcId")
    def vpc_id(self) -> pulumi.Input[_builtins.str]:
        """
        VPC ID
        """
        return pulumi.get(self, "vpc_id")

    @vpc_id.setter
    def vpc_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "vpc_id", value)

    @_builtins.property
    @pulumi.getter(name="zoneId")
    def zone_id(self) -> pulumi.Input[_builtins.str]:
        """
        Availability zone of the node. If the primary and standby nodes are in different availability zones, use a semicolon (;) to separate them. The first is the primary node's zone by default, the second is the standby node's zone.
        """
        return pulumi.get(self, "zone_id")

    @zone_id.setter
    def zone_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "zone_id", value)

    @_builtins.property
    @pulumi.getter(name="advancedFeatures")
    def advanced_features(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Advanced instance features to modify. This API currently requires allowlisting. To use it, contact technical support. Example value: {"Key":"Value"}
        """
        return pulumi.get(self, "advanced_features")

    @advanced_features.setter
    def advanced_features(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "advanced_features", value)

    @_builtins.property
    @pulumi.getter(name="allowListIds")
    def allow_list_ids(self) -> pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]]:
        """
        Allowlist ID list permitted for access.
        """
        return pulumi.get(self, "allow_list_ids")

    @allow_list_ids.setter
    def allow_list_ids(self, value: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]]):
        pulumi.set(self, "allow_list_ids", value)

    @_builtins.property
    @pulumi.getter(name="chargeInfo")
    def charge_info(self) -> pulumi.Input[Optional['InstanceChargeInfoArgs']]:
        """
        Payment method.
        """
        return pulumi.get(self, "charge_info")

    @charge_info.setter
    def charge_info(self, value: pulumi.Input[Optional['InstanceChargeInfoArgs']]):
        pulumi.set(self, "charge_info", value)

    @_builtins.property
    @pulumi.getter(name="instanceName")
    def instance_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance name. Naming rules: Cannot start with a digit or hyphen. Can only contain Chinese characters, letters, digits, underscores, and hyphens. Length must be between 1 and 128 characters.
        """
        return pulumi.get(self, "instance_name")

    @instance_name.setter
    def instance_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "instance_name", value)

    @_builtins.property
    @pulumi.getter(name="maintenanceTime")
    def maintenance_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance maintenance window. Format: HH:mmZ-HH:mmZ (UTC time). Default: UTC18:00Z-21:59Z (Beijing time 02:00-05:59). Note: To ensure cloud database stability, the system performs maintenance on instances periodically. It is recommended to set the maintenance window during off-peak hours to avoid business impact. During the maintenance window, the instance may experience 1–2 brief disconnections. Ensure your application supports automatic reconnection. The minimum maintenance window is 1 hour, the maximum is 24 hours, and cross-day maintenance windows are not allowed.
        """
        return pulumi.get(self, "maintenance_time")

    @maintenance_time.setter
    def maintenance_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "maintenance_time", value)

    @_builtins.property
    @pulumi.getter(name="masterNodeId")
    def master_node_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Primary node ID. This parameter is required when manually switching master and standby nodes.
        """
        return pulumi.get(self, "master_node_id")

    @master_node_id.setter
    def master_node_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "master_node_id", value)

    @_builtins.property
    @pulumi.getter(name="projectName")
    def project_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Project.
        """
        return pulumi.get(self, "project_name")

    @project_name.setter
    def project_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "project_name", value)

    @_builtins.property
    @pulumi.getter(name="serverCollation")
    def server_collation(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance collation. Default: Chinese*PRC*CI*AS. Most native collations are supported. Available values: Latin1*General*CI*AS, Latin1*General*CS*AS, SQL*Latin1*General*CP1*CI*AS, SQL*Latin1*General*CP1*CS*AS, Chinese*PRC*CI*AS, Chinese*PRC*CS*AS, Chinese*PRC*BIN, Japanese*CI*AS, Japanese*CS*AS, Chinese*Taiwan*Stroke*CI*AS, Chinese*Taiwan*Stroke*CS*AS, Thai*CI*AS, Chinese*PRC*CI*AI, Chinese*PRC*BIN2, Cyrillic*General*CI_AS.
        """
        return pulumi.get(self, "server_collation")

    @server_collation.setter
    def server_collation(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "server_collation", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['InstanceTagArgs']]]]:
        """
        Tag list.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceTagArgs']]]]):
        pulumi.set(self, "tags", value)


@pulumi.input_type
class _InstanceState:
    def __init__(__self__, *,
                 advanced_features: pulumi.Input[Optional[_builtins.str]] = None,
                 allow_list_ids: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]] = None,
                 backup_use: pulumi.Input[Optional[_builtins.float]] = None,
                 charge_info: pulumi.Input[Optional['InstanceChargeInfoArgs']] = None,
                 connection_infos: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceConnectionInfoArgs']]]] = None,
                 created_time: pulumi.Input[Optional[_builtins.str]] = None,
                 db_engine_version: pulumi.Input[Optional[_builtins.str]] = None,
                 inner_version: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_category: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_id: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_name: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_status: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_type: pulumi.Input[Optional[_builtins.str]] = None,
                 maintenance_time: pulumi.Input[Optional[_builtins.str]] = None,
                 master_node_id: pulumi.Input[Optional[_builtins.str]] = None,
                 memory: pulumi.Input[Optional[_builtins.int]] = None,
                 node_detail_infos: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceNodeDetailInfoArgs']]]] = None,
                 node_spec: pulumi.Input[Optional[_builtins.str]] = None,
                 port: pulumi.Input[Optional[_builtins.str]] = None,
                 primary_instance_id: pulumi.Input[Optional[_builtins.str]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 read_only_number: pulumi.Input[Optional[_builtins.int]] = None,
                 server_collation: pulumi.Input[Optional[_builtins.str]] = None,
                 slow_query_enable: pulumi.Input[Optional[_builtins.bool]] = None,
                 slow_query_time: pulumi.Input[Optional[_builtins.str]] = None,
                 storage_space: pulumi.Input[Optional[_builtins.int]] = None,
                 storage_type: pulumi.Input[Optional[_builtins.str]] = None,
                 storage_use: pulumi.Input[Optional[_builtins.float]] = None,
                 subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
                 super_account_password: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceTagArgs']]]] = None,
                 time_zone: pulumi.Input[Optional[_builtins.str]] = None,
                 updated_time: pulumi.Input[Optional[_builtins.str]] = None,
                 vcpu: pulumi.Input[Optional[_builtins.int]] = None,
                 vpc_id: pulumi.Input[Optional[_builtins.str]] = None,
                 zone_id: pulumi.Input[Optional[_builtins.str]] = None):
        """
        Input properties used for looking up and filtering Instance resources.

        :param pulumi.Input[_builtins.str] advanced_features: Advanced instance features to modify. This API currently requires allowlisting. To use it, contact technical support. Example value: {"Key":"Value"}
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] allow_list_ids: Allowlist ID list permitted for access.
        :param pulumi.Input[_builtins.float] backup_use: Used backup space for the instance, in GiB.
        :param pulumi.Input['InstanceChargeInfoArgs'] charge_info: Payment method.
        :param pulumi.Input[Sequence[pulumi.Input['InstanceConnectionInfoArgs']]] connection_infos: Connection information.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] created_time: Creation time.
        :param pulumi.Input[_builtins.str] db_engine_version: Compatible version. Values: SQLServer*2019*Std: SQL Server 2019 Standard Edition. SQLServer*2019*Ent: SQL Server 2019 Enterprise Edition. SQLServer*2019*Web: SQL Server 2019 Web Edition. SQLServer*2022*Std: SQL Server 2022 Standard Edition. SQLServer*2022*Ent: SQL Server 2022 Enterprise Edition. SQLServer*2022*Web: SQL Server 2022 Web Edition.
        :param pulumi.Input[_builtins.str] inner_version: Kernel version number.
        :param pulumi.Input[_builtins.str] instance_category: Instance category. Values: Primary: primary instance. ReadOnly: read-only instance.
        :param pulumi.Input[_builtins.str] instance_id: Instance ID.
        :param pulumi.Input[_builtins.str] instance_name: Instance name. Naming rules: Cannot start with a digit or hyphen. Can only contain Chinese characters, letters, digits, underscores, and hyphens. Length must be between 1 and 128 characters.
        :param pulumi.Input[_builtins.str] instance_status: Instance status. Values: Running: running. Creating: creating. CreateFailed: creation failed. Deleting: deleting. Restarting: restarting. Updating: updating. MasterChanging: primary node switching. Error: error.
        :param pulumi.Input[_builtins.str] instance_type: Instance type. Available values: HA (High Availability), Basic, Cluster. Supported types vary by engine version: 2019 Std supports HA/Basic; 2019 Ent supports Cluster/Basic; 2019 Web supports Basic.
        :param pulumi.Input[_builtins.str] maintenance_time: Instance maintenance window. Format: HH:mmZ-HH:mmZ (UTC time). Default: UTC18:00Z-21:59Z (Beijing time 02:00-05:59). Note: To ensure cloud database stability, the system performs maintenance on instances periodically. It is recommended to set the maintenance window during off-peak hours to avoid business impact. During the maintenance window, the instance may experience 1–2 brief disconnections. Ensure your application supports automatic reconnection. The minimum maintenance window is 1 hour, the maximum is 24 hours, and cross-day maintenance windows are not allowed.
        :param pulumi.Input[_builtins.str] master_node_id: Primary node ID. This parameter is required when manually switching master and standby nodes.
        :param pulumi.Input[_builtins.int] memory: Memory size, in GiB.
        :param pulumi.Input[Sequence[pulumi.Input['InstanceNodeDetailInfoArgs']]] node_detail_infos: Instance node information.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] node_spec: Instance specification code.
        :param pulumi.Input[_builtins.str] port: Port.
        :param pulumi.Input[_builtins.str] primary_instance_id: Primary instance ID. If this field is empty, this instance is the primary instance.
        :param pulumi.Input[_builtins.str] project_name: Project.
        :param pulumi.Input[_builtins.int] read_only_number: Number of read-only instances.
        :param pulumi.Input[_builtins.str] server_collation: Instance collation. Default: Chinese*PRC*CI*AS. Most native collations are supported. Available values: Latin1*General*CI*AS, Latin1*General*CS*AS, SQL*Latin1*General*CP1*CI*AS, SQL*Latin1*General*CP1*CS*AS, Chinese*PRC*CI*AS, Chinese*PRC*CS*AS, Chinese*PRC*BIN, Japanese*CI*AS, Japanese*CS*AS, Chinese*Taiwan*Stroke*CI*AS, Chinese*Taiwan*Stroke*CS*AS, Thai*CI*AS, Chinese*PRC*CI*AI, Chinese*PRC*BIN2, Cyrillic*General*CI_AS.
        :param pulumi.Input[_builtins.bool] slow_query_enable: Indicates whether slow logs are enabled. Available values: true: slow logs enabled. false: slow logs disabled.
        :param pulumi.Input[_builtins.str] slow_query_time: Slow log threshold. Range: [1~10], unit: seconds (s).
        :param pulumi.Input[_builtins.int] storage_space: Storage size, in GiB. Range: 20 GiB ~ 4000 GiB, increment: 10 GiB.
        :param pulumi.Input[_builtins.str] storage_type: Instance storage type.
        :param pulumi.Input[_builtins.float] storage_use: Used storage space for the instance. Unit: GiB.
        :param pulumi.Input[_builtins.str] subnet_id: Subnet ID. If the primary and standby nodes are in different availability zones, their subnets must be set to the corresponding private network subnets for each zone. Use a semicolon (;) to separate multiple zones.
        :param pulumi.Input[_builtins.str] super_account_password: Password for the high-privilege account. Password rules: 8–32 characters; must include at least three of the following: uppercase letters, lowercase letters, numbers, special characters. Allowed special characters: !@#$%^&*()_+-=.
        :param pulumi.Input[Sequence[pulumi.Input['InstanceTagArgs']]] tags: Tag list.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] time_zone: Time zone.
        :param pulumi.Input[_builtins.str] updated_time: Update time.
        :param pulumi.Input[_builtins.int] vcpu: CPU size. For example: 1 means 1 core.
        :param pulumi.Input[_builtins.str] vpc_id: VPC ID
        :param pulumi.Input[_builtins.str] zone_id: Availability zone of the node. If the primary and standby nodes are in different availability zones, use a semicolon (;) to separate them. The first is the primary node's zone by default, the second is the standby node's zone.
        """
        if advanced_features is not None:
            pulumi.set(__self__, "advanced_features", advanced_features)
        if allow_list_ids is not None:
            pulumi.set(__self__, "allow_list_ids", allow_list_ids)
        if backup_use is not None:
            pulumi.set(__self__, "backup_use", backup_use)
        if charge_info is not None:
            pulumi.set(__self__, "charge_info", charge_info)
        if connection_infos is not None:
            pulumi.set(__self__, "connection_infos", connection_infos)
        if created_time is not None:
            pulumi.set(__self__, "created_time", created_time)
        if db_engine_version is not None:
            pulumi.set(__self__, "db_engine_version", db_engine_version)
        if inner_version is not None:
            pulumi.set(__self__, "inner_version", inner_version)
        if instance_category is not None:
            pulumi.set(__self__, "instance_category", instance_category)
        if instance_id is not None:
            pulumi.set(__self__, "instance_id", instance_id)
        if instance_name is not None:
            pulumi.set(__self__, "instance_name", instance_name)
        if instance_status is not None:
            pulumi.set(__self__, "instance_status", instance_status)
        if instance_type is not None:
            pulumi.set(__self__, "instance_type", instance_type)
        if maintenance_time is not None:
            pulumi.set(__self__, "maintenance_time", maintenance_time)
        if master_node_id is not None:
            pulumi.set(__self__, "master_node_id", master_node_id)
        if memory is not None:
            pulumi.set(__self__, "memory", memory)
        if node_detail_infos is not None:
            pulumi.set(__self__, "node_detail_infos", node_detail_infos)
        if node_spec is not None:
            pulumi.set(__self__, "node_spec", node_spec)
        if port is not None:
            pulumi.set(__self__, "port", port)
        if primary_instance_id is not None:
            pulumi.set(__self__, "primary_instance_id", primary_instance_id)
        if project_name is not None:
            pulumi.set(__self__, "project_name", project_name)
        if read_only_number is not None:
            pulumi.set(__self__, "read_only_number", read_only_number)
        if server_collation is not None:
            pulumi.set(__self__, "server_collation", server_collation)
        if slow_query_enable is not None:
            pulumi.set(__self__, "slow_query_enable", slow_query_enable)
        if slow_query_time is not None:
            pulumi.set(__self__, "slow_query_time", slow_query_time)
        if storage_space is not None:
            pulumi.set(__self__, "storage_space", storage_space)
        if storage_type is not None:
            pulumi.set(__self__, "storage_type", storage_type)
        if storage_use is not None:
            pulumi.set(__self__, "storage_use", storage_use)
        if subnet_id is not None:
            pulumi.set(__self__, "subnet_id", subnet_id)
        if super_account_password is not None:
            pulumi.set(__self__, "super_account_password", super_account_password)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)
        if time_zone is not None:
            pulumi.set(__self__, "time_zone", time_zone)
        if updated_time is not None:
            pulumi.set(__self__, "updated_time", updated_time)
        if vcpu is not None:
            pulumi.set(__self__, "vcpu", vcpu)
        if vpc_id is not None:
            pulumi.set(__self__, "vpc_id", vpc_id)
        if zone_id is not None:
            pulumi.set(__self__, "zone_id", zone_id)

    @_builtins.property
    @pulumi.getter(name="advancedFeatures")
    def advanced_features(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Advanced instance features to modify. This API currently requires allowlisting. To use it, contact technical support. Example value: {"Key":"Value"}
        """
        return pulumi.get(self, "advanced_features")

    @advanced_features.setter
    def advanced_features(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "advanced_features", value)

    @_builtins.property
    @pulumi.getter(name="allowListIds")
    def allow_list_ids(self) -> pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]]:
        """
        Allowlist ID list permitted for access.
        """
        return pulumi.get(self, "allow_list_ids")

    @allow_list_ids.setter
    def allow_list_ids(self, value: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]]):
        pulumi.set(self, "allow_list_ids", value)

    @_builtins.property
    @pulumi.getter(name="backupUse")
    def backup_use(self) -> pulumi.Input[Optional[_builtins.float]]:
        """
        Used backup space for the instance, in GiB.
        """
        return pulumi.get(self, "backup_use")

    @backup_use.setter
    def backup_use(self, value: pulumi.Input[Optional[_builtins.float]]):
        pulumi.set(self, "backup_use", value)

    @_builtins.property
    @pulumi.getter(name="chargeInfo")
    def charge_info(self) -> pulumi.Input[Optional['InstanceChargeInfoArgs']]:
        """
        Payment method.
        """
        return pulumi.get(self, "charge_info")

    @charge_info.setter
    def charge_info(self, value: pulumi.Input[Optional['InstanceChargeInfoArgs']]):
        pulumi.set(self, "charge_info", value)

    @_builtins.property
    @pulumi.getter(name="connectionInfos")
    def connection_infos(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['InstanceConnectionInfoArgs']]]]:
        """
        Connection information.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "connection_infos")

    @connection_infos.setter
    def connection_infos(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceConnectionInfoArgs']]]]):
        pulumi.set(self, "connection_infos", value)

    @_builtins.property
    @pulumi.getter(name="createdTime")
    def created_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Creation time.
        """
        return pulumi.get(self, "created_time")

    @created_time.setter
    def created_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "created_time", value)

    @_builtins.property
    @pulumi.getter(name="dbEngineVersion")
    def db_engine_version(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Compatible version. Values: SQLServer*2019*Std: SQL Server 2019 Standard Edition. SQLServer*2019*Ent: SQL Server 2019 Enterprise Edition. SQLServer*2019*Web: SQL Server 2019 Web Edition. SQLServer*2022*Std: SQL Server 2022 Standard Edition. SQLServer*2022*Ent: SQL Server 2022 Enterprise Edition. SQLServer*2022*Web: SQL Server 2022 Web Edition.
        """
        return pulumi.get(self, "db_engine_version")

    @db_engine_version.setter
    def db_engine_version(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "db_engine_version", value)

    @_builtins.property
    @pulumi.getter(name="innerVersion")
    def inner_version(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Kernel version number.
        """
        return pulumi.get(self, "inner_version")

    @inner_version.setter
    def inner_version(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "inner_version", value)

    @_builtins.property
    @pulumi.getter(name="instanceCategory")
    def instance_category(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance category. Values: Primary: primary instance. ReadOnly: read-only instance.
        """
        return pulumi.get(self, "instance_category")

    @instance_category.setter
    def instance_category(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "instance_category", value)

    @_builtins.property
    @pulumi.getter(name="instanceId")
    def instance_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance ID.
        """
        return pulumi.get(self, "instance_id")

    @instance_id.setter
    def instance_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "instance_id", value)

    @_builtins.property
    @pulumi.getter(name="instanceName")
    def instance_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance name. Naming rules: Cannot start with a digit or hyphen. Can only contain Chinese characters, letters, digits, underscores, and hyphens. Length must be between 1 and 128 characters.
        """
        return pulumi.get(self, "instance_name")

    @instance_name.setter
    def instance_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "instance_name", value)

    @_builtins.property
    @pulumi.getter(name="instanceStatus")
    def instance_status(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance status. Values: Running: running. Creating: creating. CreateFailed: creation failed. Deleting: deleting. Restarting: restarting. Updating: updating. MasterChanging: primary node switching. Error: error.
        """
        return pulumi.get(self, "instance_status")

    @instance_status.setter
    def instance_status(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "instance_status", value)

    @_builtins.property
    @pulumi.getter(name="instanceType")
    def instance_type(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance type. Available values: HA (High Availability), Basic, Cluster. Supported types vary by engine version: 2019 Std supports HA/Basic; 2019 Ent supports Cluster/Basic; 2019 Web supports Basic.
        """
        return pulumi.get(self, "instance_type")

    @instance_type.setter
    def instance_type(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "instance_type", value)

    @_builtins.property
    @pulumi.getter(name="maintenanceTime")
    def maintenance_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance maintenance window. Format: HH:mmZ-HH:mmZ (UTC time). Default: UTC18:00Z-21:59Z (Beijing time 02:00-05:59). Note: To ensure cloud database stability, the system performs maintenance on instances periodically. It is recommended to set the maintenance window during off-peak hours to avoid business impact. During the maintenance window, the instance may experience 1–2 brief disconnections. Ensure your application supports automatic reconnection. The minimum maintenance window is 1 hour, the maximum is 24 hours, and cross-day maintenance windows are not allowed.
        """
        return pulumi.get(self, "maintenance_time")

    @maintenance_time.setter
    def maintenance_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "maintenance_time", value)

    @_builtins.property
    @pulumi.getter(name="masterNodeId")
    def master_node_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Primary node ID. This parameter is required when manually switching master and standby nodes.
        """
        return pulumi.get(self, "master_node_id")

    @master_node_id.setter
    def master_node_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "master_node_id", value)

    @_builtins.property
    @pulumi.getter
    def memory(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Memory size, in GiB.
        """
        return pulumi.get(self, "memory")

    @memory.setter
    def memory(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "memory", value)

    @_builtins.property
    @pulumi.getter(name="nodeDetailInfos")
    def node_detail_infos(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['InstanceNodeDetailInfoArgs']]]]:
        """
        Instance node information.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "node_detail_infos")

    @node_detail_infos.setter
    def node_detail_infos(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceNodeDetailInfoArgs']]]]):
        pulumi.set(self, "node_detail_infos", value)

    @_builtins.property
    @pulumi.getter(name="nodeSpec")
    def node_spec(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance specification code.
        """
        return pulumi.get(self, "node_spec")

    @node_spec.setter
    def node_spec(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "node_spec", value)

    @_builtins.property
    @pulumi.getter
    def port(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Port.
        """
        return pulumi.get(self, "port")

    @port.setter
    def port(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "port", value)

    @_builtins.property
    @pulumi.getter(name="primaryInstanceId")
    def primary_instance_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Primary instance ID. If this field is empty, this instance is the primary instance.
        """
        return pulumi.get(self, "primary_instance_id")

    @primary_instance_id.setter
    def primary_instance_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "primary_instance_id", value)

    @_builtins.property
    @pulumi.getter(name="projectName")
    def project_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Project.
        """
        return pulumi.get(self, "project_name")

    @project_name.setter
    def project_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "project_name", value)

    @_builtins.property
    @pulumi.getter(name="readOnlyNumber")
    def read_only_number(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Number of read-only instances.
        """
        return pulumi.get(self, "read_only_number")

    @read_only_number.setter
    def read_only_number(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "read_only_number", value)

    @_builtins.property
    @pulumi.getter(name="serverCollation")
    def server_collation(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance collation. Default: Chinese*PRC*CI*AS. Most native collations are supported. Available values: Latin1*General*CI*AS, Latin1*General*CS*AS, SQL*Latin1*General*CP1*CI*AS, SQL*Latin1*General*CP1*CS*AS, Chinese*PRC*CI*AS, Chinese*PRC*CS*AS, Chinese*PRC*BIN, Japanese*CI*AS, Japanese*CS*AS, Chinese*Taiwan*Stroke*CI*AS, Chinese*Taiwan*Stroke*CS*AS, Thai*CI*AS, Chinese*PRC*CI*AI, Chinese*PRC*BIN2, Cyrillic*General*CI_AS.
        """
        return pulumi.get(self, "server_collation")

    @server_collation.setter
    def server_collation(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "server_collation", value)

    @_builtins.property
    @pulumi.getter(name="slowQueryEnable")
    def slow_query_enable(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Indicates whether slow logs are enabled. Available values: true: slow logs enabled. false: slow logs disabled.
        """
        return pulumi.get(self, "slow_query_enable")

    @slow_query_enable.setter
    def slow_query_enable(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "slow_query_enable", value)

    @_builtins.property
    @pulumi.getter(name="slowQueryTime")
    def slow_query_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Slow log threshold. Range: [1~10], unit: seconds (s).
        """
        return pulumi.get(self, "slow_query_time")

    @slow_query_time.setter
    def slow_query_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "slow_query_time", value)

    @_builtins.property
    @pulumi.getter(name="storageSpace")
    def storage_space(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Storage size, in GiB. Range: 20 GiB ~ 4000 GiB, increment: 10 GiB.
        """
        return pulumi.get(self, "storage_space")

    @storage_space.setter
    def storage_space(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "storage_space", value)

    @_builtins.property
    @pulumi.getter(name="storageType")
    def storage_type(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Instance storage type.
        """
        return pulumi.get(self, "storage_type")

    @storage_type.setter
    def storage_type(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "storage_type", value)

    @_builtins.property
    @pulumi.getter(name="storageUse")
    def storage_use(self) -> pulumi.Input[Optional[_builtins.float]]:
        """
        Used storage space for the instance. Unit: GiB.
        """
        return pulumi.get(self, "storage_use")

    @storage_use.setter
    def storage_use(self, value: pulumi.Input[Optional[_builtins.float]]):
        pulumi.set(self, "storage_use", value)

    @_builtins.property
    @pulumi.getter(name="subnetId")
    def subnet_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Subnet ID. If the primary and standby nodes are in different availability zones, their subnets must be set to the corresponding private network subnets for each zone. Use a semicolon (;) to separate multiple zones.
        """
        return pulumi.get(self, "subnet_id")

    @subnet_id.setter
    def subnet_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "subnet_id", value)

    @_builtins.property
    @pulumi.getter(name="superAccountPassword")
    def super_account_password(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Password for the high-privilege account. Password rules: 8–32 characters; must include at least three of the following: uppercase letters, lowercase letters, numbers, special characters. Allowed special characters: !@#$%^&*()_+-=.
        """
        return pulumi.get(self, "super_account_password")

    @super_account_password.setter
    def super_account_password(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "super_account_password", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['InstanceTagArgs']]]]:
        """
        Tag list.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['InstanceTagArgs']]]]):
        pulumi.set(self, "tags", value)

    @_builtins.property
    @pulumi.getter(name="timeZone")
    def time_zone(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Time zone.
        """
        return pulumi.get(self, "time_zone")

    @time_zone.setter
    def time_zone(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "time_zone", value)

    @_builtins.property
    @pulumi.getter(name="updatedTime")
    def updated_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Update time.
        """
        return pulumi.get(self, "updated_time")

    @updated_time.setter
    def updated_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "updated_time", value)

    @_builtins.property
    @pulumi.getter
    def vcpu(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        CPU size. For example: 1 means 1 core.
        """
        return pulumi.get(self, "vcpu")

    @vcpu.setter
    def vcpu(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "vcpu", value)

    @_builtins.property
    @pulumi.getter(name="vpcId")
    def vpc_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        VPC ID
        """
        return pulumi.get(self, "vpc_id")

    @vpc_id.setter
    def vpc_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "vpc_id", value)

    @_builtins.property
    @pulumi.getter(name="zoneId")
    def zone_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Availability zone of the node. If the primary and standby nodes are in different availability zones, use a semicolon (;) to separate them. The first is the primary node's zone by default, the second is the standby node's zone.
        """
        return pulumi.get(self, "zone_id")

    @zone_id.setter
    def zone_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "zone_id", value)


@pulumi.type_token("volcenginecc:rdsmssql/instance:Instance")
class Instance(pulumi.CustomResource):
    @overload
    def __init__(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 advanced_features: pulumi.Input[Optional[_builtins.str]] = None,
                 allow_list_ids: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]] = None,
                 charge_info: pulumi.Input[Optional[Union['InstanceChargeInfoArgs', 'InstanceChargeInfoArgsDict']]] = None,
                 db_engine_version: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_name: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_type: pulumi.Input[Optional[_builtins.str]] = None,
                 maintenance_time: pulumi.Input[Optional[_builtins.str]] = None,
                 master_node_id: pulumi.Input[Optional[_builtins.str]] = None,
                 node_spec: pulumi.Input[Optional[_builtins.str]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 server_collation: pulumi.Input[Optional[_builtins.str]] = None,
                 storage_space: pulumi.Input[Optional[_builtins.int]] = None,
                 subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
                 super_account_password: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InstanceTagArgs', 'InstanceTagArgsDict']]]]] = None,
                 time_zone: pulumi.Input[Optional[_builtins.str]] = None,
                 vpc_id: pulumi.Input[Optional[_builtins.str]] = None,
                 zone_id: pulumi.Input[Optional[_builtins.str]] = None,
                 __props__=None):
        """
        Cloud Database SQL Server Edition is a stable, reliable, and scalable online database service built on the world-class commercial database product Microsoft SQL Server. Creating an instance is the first step to using Cloud Database SQL Server Edition

        ## Example Usage

        ```python
        import pulumi
        import pulumi_volcenginecc as volcenginecc

        r_dsms_sql_instance_demo = volcenginecc.rdsmssql.Instance("RDSMsSQLInstanceDemo",
            node_spec="rds.mssql.3il.x8.medium.s1",
            zone_id="cn-beijing-a",
            subnet_id="subnet-1a0zgr5e7hslc8nvepxxxxx",
            db_engine_version="SQLServer_2019_Std",
            instance_type="Basic",
            storage_space=20,
            vpc_id="vpc-3nqt4kq87xn2893xxxxx",
            instance_name="RDSMsSQLInstanceDemo",
            super_account_password="********",
            server_collation="Chinese_PRC_CI_AS",
            time_zone="China Standard Time",
            charge_info={
                "charge_type": "PostPaid",
            },
            project_name="default",
            maintenance_time="18:00Z-21:59Z",
            allow_list_ids=["acl-03f197e136c843b29e47de74e9xxxxx"],
            tags=[{
                "key": "env",
                "value": "test",
            }])
        ```

        ## Import

        ```sh
        $ pulumi import volcenginecc:rdsmssql/instance:Instance example "instance_id"
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] advanced_features: Advanced instance features to modify. This API currently requires allowlisting. To use it, contact technical support. Example value: {"Key":"Value"}
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] allow_list_ids: Allowlist ID list permitted for access.
        :param pulumi.Input[Union['InstanceChargeInfoArgs', 'InstanceChargeInfoArgsDict']] charge_info: Payment method.
        :param pulumi.Input[_builtins.str] db_engine_version: Compatible version. Values: SQLServer*2019*Std: SQL Server 2019 Standard Edition. SQLServer*2019*Ent: SQL Server 2019 Enterprise Edition. SQLServer*2019*Web: SQL Server 2019 Web Edition. SQLServer*2022*Std: SQL Server 2022 Standard Edition. SQLServer*2022*Ent: SQL Server 2022 Enterprise Edition. SQLServer*2022*Web: SQL Server 2022 Web Edition.
        :param pulumi.Input[_builtins.str] instance_name: Instance name. Naming rules: Cannot start with a digit or hyphen. Can only contain Chinese characters, letters, digits, underscores, and hyphens. Length must be between 1 and 128 characters.
        :param pulumi.Input[_builtins.str] instance_type: Instance type. Available values: HA (High Availability), Basic, Cluster. Supported types vary by engine version: 2019 Std supports HA/Basic; 2019 Ent supports Cluster/Basic; 2019 Web supports Basic.
        :param pulumi.Input[_builtins.str] maintenance_time: Instance maintenance window. Format: HH:mmZ-HH:mmZ (UTC time). Default: UTC18:00Z-21:59Z (Beijing time 02:00-05:59). Note: To ensure cloud database stability, the system performs maintenance on instances periodically. It is recommended to set the maintenance window during off-peak hours to avoid business impact. During the maintenance window, the instance may experience 1–2 brief disconnections. Ensure your application supports automatic reconnection. The minimum maintenance window is 1 hour, the maximum is 24 hours, and cross-day maintenance windows are not allowed.
        :param pulumi.Input[_builtins.str] master_node_id: Primary node ID. This parameter is required when manually switching master and standby nodes.
        :param pulumi.Input[_builtins.str] node_spec: Instance specification code.
        :param pulumi.Input[_builtins.str] project_name: Project.
        :param pulumi.Input[_builtins.str] server_collation: Instance collation. Default: Chinese*PRC*CI*AS. Most native collations are supported. Available values: Latin1*General*CI*AS, Latin1*General*CS*AS, SQL*Latin1*General*CP1*CI*AS, SQL*Latin1*General*CP1*CS*AS, Chinese*PRC*CI*AS, Chinese*PRC*CS*AS, Chinese*PRC*BIN, Japanese*CI*AS, Japanese*CS*AS, Chinese*Taiwan*Stroke*CI*AS, Chinese*Taiwan*Stroke*CS*AS, Thai*CI*AS, Chinese*PRC*CI*AI, Chinese*PRC*BIN2, Cyrillic*General*CI_AS.
        :param pulumi.Input[_builtins.int] storage_space: Storage size, in GiB. Range: 20 GiB ~ 4000 GiB, increment: 10 GiB.
        :param pulumi.Input[_builtins.str] subnet_id: Subnet ID. If the primary and standby nodes are in different availability zones, their subnets must be set to the corresponding private network subnets for each zone. Use a semicolon (;) to separate multiple zones.
        :param pulumi.Input[_builtins.str] super_account_password: Password for the high-privilege account. Password rules: 8–32 characters; must include at least three of the following: uppercase letters, lowercase letters, numbers, special characters. Allowed special characters: !@#$%^&*()_+-=.
        :param pulumi.Input[Sequence[pulumi.Input[Union['InstanceTagArgs', 'InstanceTagArgsDict']]]] tags: Tag list.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] time_zone: Time zone.
        :param pulumi.Input[_builtins.str] vpc_id: VPC ID
        :param pulumi.Input[_builtins.str] zone_id: Availability zone of the node. If the primary and standby nodes are in different availability zones, use a semicolon (;) to separate them. The first is the primary node's zone by default, the second is the standby node's zone.
        """
        ...
    @overload
    def __init__(__self__,
                 resource_name: str,
                 args: InstanceArgs,
                 opts: Optional[pulumi.ResourceOptions] = None):
        """
        Cloud Database SQL Server Edition is a stable, reliable, and scalable online database service built on the world-class commercial database product Microsoft SQL Server. Creating an instance is the first step to using Cloud Database SQL Server Edition

        ## Example Usage

        ```python
        import pulumi
        import pulumi_volcenginecc as volcenginecc

        r_dsms_sql_instance_demo = volcenginecc.rdsmssql.Instance("RDSMsSQLInstanceDemo",
            node_spec="rds.mssql.3il.x8.medium.s1",
            zone_id="cn-beijing-a",
            subnet_id="subnet-1a0zgr5e7hslc8nvepxxxxx",
            db_engine_version="SQLServer_2019_Std",
            instance_type="Basic",
            storage_space=20,
            vpc_id="vpc-3nqt4kq87xn2893xxxxx",
            instance_name="RDSMsSQLInstanceDemo",
            super_account_password="********",
            server_collation="Chinese_PRC_CI_AS",
            time_zone="China Standard Time",
            charge_info={
                "charge_type": "PostPaid",
            },
            project_name="default",
            maintenance_time="18:00Z-21:59Z",
            allow_list_ids=["acl-03f197e136c843b29e47de74e9xxxxx"],
            tags=[{
                "key": "env",
                "value": "test",
            }])
        ```

        ## Import

        ```sh
        $ pulumi import volcenginecc:rdsmssql/instance:Instance example "instance_id"
        ```


        :param str resource_name: The name of the resource.
        :param InstanceArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        ...
    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(InstanceArgs, pulumi.ResourceOptions, *args, **kwargs)
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 advanced_features: pulumi.Input[Optional[_builtins.str]] = None,
                 allow_list_ids: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]] = None,
                 charge_info: pulumi.Input[Optional[Union['InstanceChargeInfoArgs', 'InstanceChargeInfoArgsDict']]] = None,
                 db_engine_version: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_name: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_type: pulumi.Input[Optional[_builtins.str]] = None,
                 maintenance_time: pulumi.Input[Optional[_builtins.str]] = None,
                 master_node_id: pulumi.Input[Optional[_builtins.str]] = None,
                 node_spec: pulumi.Input[Optional[_builtins.str]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 server_collation: pulumi.Input[Optional[_builtins.str]] = None,
                 storage_space: pulumi.Input[Optional[_builtins.int]] = None,
                 subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
                 super_account_password: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InstanceTagArgs', 'InstanceTagArgsDict']]]]] = None,
                 time_zone: pulumi.Input[Optional[_builtins.str]] = None,
                 vpc_id: pulumi.Input[Optional[_builtins.str]] = None,
                 zone_id: pulumi.Input[Optional[_builtins.str]] = None,
                 __props__=None):
        opts = pulumi.ResourceOptions.merge(_utilities.get_resource_opts_defaults(), opts)
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError('Expected resource options to be a ResourceOptions instance')
        if opts.id is None:
            if __props__ is not None:
                raise TypeError('__props__ is only valid when passed in combination with a valid opts.id to get an existing resource')
            __props__ = InstanceArgs.__new__(InstanceArgs)

            __props__.__dict__["advanced_features"] = advanced_features
            __props__.__dict__["allow_list_ids"] = allow_list_ids
            __props__.__dict__["charge_info"] = charge_info
            if db_engine_version is None and not opts.urn:
                raise TypeError("Missing required property 'db_engine_version'")
            __props__.__dict__["db_engine_version"] = db_engine_version
            __props__.__dict__["instance_name"] = instance_name
            if instance_type is None and not opts.urn:
                raise TypeError("Missing required property 'instance_type'")
            __props__.__dict__["instance_type"] = instance_type
            __props__.__dict__["maintenance_time"] = maintenance_time
            __props__.__dict__["master_node_id"] = master_node_id
            if node_spec is None and not opts.urn:
                raise TypeError("Missing required property 'node_spec'")
            __props__.__dict__["node_spec"] = node_spec
            __props__.__dict__["project_name"] = project_name
            __props__.__dict__["server_collation"] = server_collation
            if storage_space is None and not opts.urn:
                raise TypeError("Missing required property 'storage_space'")
            __props__.__dict__["storage_space"] = storage_space
            if subnet_id is None and not opts.urn:
                raise TypeError("Missing required property 'subnet_id'")
            __props__.__dict__["subnet_id"] = subnet_id
            if super_account_password is None and not opts.urn:
                raise TypeError("Missing required property 'super_account_password'")
            __props__.__dict__["super_account_password"] = super_account_password
            __props__.__dict__["tags"] = tags
            if time_zone is None and not opts.urn:
                raise TypeError("Missing required property 'time_zone'")
            __props__.__dict__["time_zone"] = time_zone
            if vpc_id is None and not opts.urn:
                raise TypeError("Missing required property 'vpc_id'")
            __props__.__dict__["vpc_id"] = vpc_id
            if zone_id is None and not opts.urn:
                raise TypeError("Missing required property 'zone_id'")
            __props__.__dict__["zone_id"] = zone_id
            __props__.__dict__["backup_use"] = None
            __props__.__dict__["connection_infos"] = None
            __props__.__dict__["created_time"] = None
            __props__.__dict__["inner_version"] = None
            __props__.__dict__["instance_category"] = None
            __props__.__dict__["instance_id"] = None
            __props__.__dict__["instance_status"] = None
            __props__.__dict__["memory"] = None
            __props__.__dict__["node_detail_infos"] = None
            __props__.__dict__["port"] = None
            __props__.__dict__["primary_instance_id"] = None
            __props__.__dict__["read_only_number"] = None
            __props__.__dict__["slow_query_enable"] = None
            __props__.__dict__["slow_query_time"] = None
            __props__.__dict__["storage_type"] = None
            __props__.__dict__["storage_use"] = None
            __props__.__dict__["updated_time"] = None
            __props__.__dict__["vcpu"] = None
        super(Instance, __self__).__init__(
            'volcenginecc:rdsmssql/instance:Instance',
            resource_name,
            __props__,
            opts)

    @staticmethod
    def get(resource_name: str,
            id: pulumi.Input[str],
            opts: Optional[pulumi.ResourceOptions] = None,
            advanced_features: pulumi.Input[Optional[_builtins.str]] = None,
            allow_list_ids: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]] = None,
            backup_use: pulumi.Input[Optional[_builtins.float]] = None,
            charge_info: pulumi.Input[Optional[Union['InstanceChargeInfoArgs', 'InstanceChargeInfoArgsDict']]] = None,
            connection_infos: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InstanceConnectionInfoArgs', 'InstanceConnectionInfoArgsDict']]]]] = None,
            created_time: pulumi.Input[Optional[_builtins.str]] = None,
            db_engine_version: pulumi.Input[Optional[_builtins.str]] = None,
            inner_version: pulumi.Input[Optional[_builtins.str]] = None,
            instance_category: pulumi.Input[Optional[_builtins.str]] = None,
            instance_id: pulumi.Input[Optional[_builtins.str]] = None,
            instance_name: pulumi.Input[Optional[_builtins.str]] = None,
            instance_status: pulumi.Input[Optional[_builtins.str]] = None,
            instance_type: pulumi.Input[Optional[_builtins.str]] = None,
            maintenance_time: pulumi.Input[Optional[_builtins.str]] = None,
            master_node_id: pulumi.Input[Optional[_builtins.str]] = None,
            memory: pulumi.Input[Optional[_builtins.int]] = None,
            node_detail_infos: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InstanceNodeDetailInfoArgs', 'InstanceNodeDetailInfoArgsDict']]]]] = None,
            node_spec: pulumi.Input[Optional[_builtins.str]] = None,
            port: pulumi.Input[Optional[_builtins.str]] = None,
            primary_instance_id: pulumi.Input[Optional[_builtins.str]] = None,
            project_name: pulumi.Input[Optional[_builtins.str]] = None,
            read_only_number: pulumi.Input[Optional[_builtins.int]] = None,
            server_collation: pulumi.Input[Optional[_builtins.str]] = None,
            slow_query_enable: pulumi.Input[Optional[_builtins.bool]] = None,
            slow_query_time: pulumi.Input[Optional[_builtins.str]] = None,
            storage_space: pulumi.Input[Optional[_builtins.int]] = None,
            storage_type: pulumi.Input[Optional[_builtins.str]] = None,
            storage_use: pulumi.Input[Optional[_builtins.float]] = None,
            subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
            super_account_password: pulumi.Input[Optional[_builtins.str]] = None,
            tags: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InstanceTagArgs', 'InstanceTagArgsDict']]]]] = None,
            time_zone: pulumi.Input[Optional[_builtins.str]] = None,
            updated_time: pulumi.Input[Optional[_builtins.str]] = None,
            vcpu: pulumi.Input[Optional[_builtins.int]] = None,
            vpc_id: pulumi.Input[Optional[_builtins.str]] = None,
            zone_id: pulumi.Input[Optional[_builtins.str]] = None) -> 'Instance':
        """
        Get an existing Instance resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] advanced_features: Advanced instance features to modify. This API currently requires allowlisting. To use it, contact technical support. Example value: {"Key":"Value"}
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] allow_list_ids: Allowlist ID list permitted for access.
        :param pulumi.Input[_builtins.float] backup_use: Used backup space for the instance, in GiB.
        :param pulumi.Input[Union['InstanceChargeInfoArgs', 'InstanceChargeInfoArgsDict']] charge_info: Payment method.
        :param pulumi.Input[Sequence[pulumi.Input[Union['InstanceConnectionInfoArgs', 'InstanceConnectionInfoArgsDict']]]] connection_infos: Connection information.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] created_time: Creation time.
        :param pulumi.Input[_builtins.str] db_engine_version: Compatible version. Values: SQLServer*2019*Std: SQL Server 2019 Standard Edition. SQLServer*2019*Ent: SQL Server 2019 Enterprise Edition. SQLServer*2019*Web: SQL Server 2019 Web Edition. SQLServer*2022*Std: SQL Server 2022 Standard Edition. SQLServer*2022*Ent: SQL Server 2022 Enterprise Edition. SQLServer*2022*Web: SQL Server 2022 Web Edition.
        :param pulumi.Input[_builtins.str] inner_version: Kernel version number.
        :param pulumi.Input[_builtins.str] instance_category: Instance category. Values: Primary: primary instance. ReadOnly: read-only instance.
        :param pulumi.Input[_builtins.str] instance_id: Instance ID.
        :param pulumi.Input[_builtins.str] instance_name: Instance name. Naming rules: Cannot start with a digit or hyphen. Can only contain Chinese characters, letters, digits, underscores, and hyphens. Length must be between 1 and 128 characters.
        :param pulumi.Input[_builtins.str] instance_status: Instance status. Values: Running: running. Creating: creating. CreateFailed: creation failed. Deleting: deleting. Restarting: restarting. Updating: updating. MasterChanging: primary node switching. Error: error.
        :param pulumi.Input[_builtins.str] instance_type: Instance type. Available values: HA (High Availability), Basic, Cluster. Supported types vary by engine version: 2019 Std supports HA/Basic; 2019 Ent supports Cluster/Basic; 2019 Web supports Basic.
        :param pulumi.Input[_builtins.str] maintenance_time: Instance maintenance window. Format: HH:mmZ-HH:mmZ (UTC time). Default: UTC18:00Z-21:59Z (Beijing time 02:00-05:59). Note: To ensure cloud database stability, the system performs maintenance on instances periodically. It is recommended to set the maintenance window during off-peak hours to avoid business impact. During the maintenance window, the instance may experience 1–2 brief disconnections. Ensure your application supports automatic reconnection. The minimum maintenance window is 1 hour, the maximum is 24 hours, and cross-day maintenance windows are not allowed.
        :param pulumi.Input[_builtins.str] master_node_id: Primary node ID. This parameter is required when manually switching master and standby nodes.
        :param pulumi.Input[_builtins.int] memory: Memory size, in GiB.
        :param pulumi.Input[Sequence[pulumi.Input[Union['InstanceNodeDetailInfoArgs', 'InstanceNodeDetailInfoArgsDict']]]] node_detail_infos: Instance node information.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] node_spec: Instance specification code.
        :param pulumi.Input[_builtins.str] port: Port.
        :param pulumi.Input[_builtins.str] primary_instance_id: Primary instance ID. If this field is empty, this instance is the primary instance.
        :param pulumi.Input[_builtins.str] project_name: Project.
        :param pulumi.Input[_builtins.int] read_only_number: Number of read-only instances.
        :param pulumi.Input[_builtins.str] server_collation: Instance collation. Default: Chinese*PRC*CI*AS. Most native collations are supported. Available values: Latin1*General*CI*AS, Latin1*General*CS*AS, SQL*Latin1*General*CP1*CI*AS, SQL*Latin1*General*CP1*CS*AS, Chinese*PRC*CI*AS, Chinese*PRC*CS*AS, Chinese*PRC*BIN, Japanese*CI*AS, Japanese*CS*AS, Chinese*Taiwan*Stroke*CI*AS, Chinese*Taiwan*Stroke*CS*AS, Thai*CI*AS, Chinese*PRC*CI*AI, Chinese*PRC*BIN2, Cyrillic*General*CI_AS.
        :param pulumi.Input[_builtins.bool] slow_query_enable: Indicates whether slow logs are enabled. Available values: true: slow logs enabled. false: slow logs disabled.
        :param pulumi.Input[_builtins.str] slow_query_time: Slow log threshold. Range: [1~10], unit: seconds (s).
        :param pulumi.Input[_builtins.int] storage_space: Storage size, in GiB. Range: 20 GiB ~ 4000 GiB, increment: 10 GiB.
        :param pulumi.Input[_builtins.str] storage_type: Instance storage type.
        :param pulumi.Input[_builtins.float] storage_use: Used storage space for the instance. Unit: GiB.
        :param pulumi.Input[_builtins.str] subnet_id: Subnet ID. If the primary and standby nodes are in different availability zones, their subnets must be set to the corresponding private network subnets for each zone. Use a semicolon (;) to separate multiple zones.
        :param pulumi.Input[_builtins.str] super_account_password: Password for the high-privilege account. Password rules: 8–32 characters; must include at least three of the following: uppercase letters, lowercase letters, numbers, special characters. Allowed special characters: !@#$%^&*()_+-=.
        :param pulumi.Input[Sequence[pulumi.Input[Union['InstanceTagArgs', 'InstanceTagArgsDict']]]] tags: Tag list.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] time_zone: Time zone.
        :param pulumi.Input[_builtins.str] updated_time: Update time.
        :param pulumi.Input[_builtins.int] vcpu: CPU size. For example: 1 means 1 core.
        :param pulumi.Input[_builtins.str] vpc_id: VPC ID
        :param pulumi.Input[_builtins.str] zone_id: Availability zone of the node. If the primary and standby nodes are in different availability zones, use a semicolon (;) to separate them. The first is the primary node's zone by default, the second is the standby node's zone.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _InstanceState.__new__(_InstanceState)

        __props__.__dict__["advanced_features"] = advanced_features
        __props__.__dict__["allow_list_ids"] = allow_list_ids
        __props__.__dict__["backup_use"] = backup_use
        __props__.__dict__["charge_info"] = charge_info
        __props__.__dict__["connection_infos"] = connection_infos
        __props__.__dict__["created_time"] = created_time
        __props__.__dict__["db_engine_version"] = db_engine_version
        __props__.__dict__["inner_version"] = inner_version
        __props__.__dict__["instance_category"] = instance_category
        __props__.__dict__["instance_id"] = instance_id
        __props__.__dict__["instance_name"] = instance_name
        __props__.__dict__["instance_status"] = instance_status
        __props__.__dict__["instance_type"] = instance_type
        __props__.__dict__["maintenance_time"] = maintenance_time
        __props__.__dict__["master_node_id"] = master_node_id
        __props__.__dict__["memory"] = memory
        __props__.__dict__["node_detail_infos"] = node_detail_infos
        __props__.__dict__["node_spec"] = node_spec
        __props__.__dict__["port"] = port
        __props__.__dict__["primary_instance_id"] = primary_instance_id
        __props__.__dict__["project_name"] = project_name
        __props__.__dict__["read_only_number"] = read_only_number
        __props__.__dict__["server_collation"] = server_collation
        __props__.__dict__["slow_query_enable"] = slow_query_enable
        __props__.__dict__["slow_query_time"] = slow_query_time
        __props__.__dict__["storage_space"] = storage_space
        __props__.__dict__["storage_type"] = storage_type
        __props__.__dict__["storage_use"] = storage_use
        __props__.__dict__["subnet_id"] = subnet_id
        __props__.__dict__["super_account_password"] = super_account_password
        __props__.__dict__["tags"] = tags
        __props__.__dict__["time_zone"] = time_zone
        __props__.__dict__["updated_time"] = updated_time
        __props__.__dict__["vcpu"] = vcpu
        __props__.__dict__["vpc_id"] = vpc_id
        __props__.__dict__["zone_id"] = zone_id
        return Instance(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="advancedFeatures")
    def advanced_features(self) -> pulumi.Output[_builtins.str]:
        """
        Advanced instance features to modify. This API currently requires allowlisting. To use it, contact technical support. Example value: {"Key":"Value"}
        """
        return pulumi.get(self, "advanced_features")

    @_builtins.property
    @pulumi.getter(name="allowListIds")
    def allow_list_ids(self) -> pulumi.Output[Sequence[_builtins.str]]:
        """
        Allowlist ID list permitted for access.
        """
        return pulumi.get(self, "allow_list_ids")

    @_builtins.property
    @pulumi.getter(name="backupUse")
    def backup_use(self) -> pulumi.Output[_builtins.float]:
        """
        Used backup space for the instance, in GiB.
        """
        return pulumi.get(self, "backup_use")

    @_builtins.property
    @pulumi.getter(name="chargeInfo")
    def charge_info(self) -> pulumi.Output['outputs.InstanceChargeInfo']:
        """
        Payment method.
        """
        return pulumi.get(self, "charge_info")

    @_builtins.property
    @pulumi.getter(name="connectionInfos")
    def connection_infos(self) -> pulumi.Output[Sequence['outputs.InstanceConnectionInfo']]:
        """
        Connection information.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "connection_infos")

    @_builtins.property
    @pulumi.getter(name="createdTime")
    def created_time(self) -> pulumi.Output[_builtins.str]:
        """
        Creation time.
        """
        return pulumi.get(self, "created_time")

    @_builtins.property
    @pulumi.getter(name="dbEngineVersion")
    def db_engine_version(self) -> pulumi.Output[_builtins.str]:
        """
        Compatible version. Values: SQLServer*2019*Std: SQL Server 2019 Standard Edition. SQLServer*2019*Ent: SQL Server 2019 Enterprise Edition. SQLServer*2019*Web: SQL Server 2019 Web Edition. SQLServer*2022*Std: SQL Server 2022 Standard Edition. SQLServer*2022*Ent: SQL Server 2022 Enterprise Edition. SQLServer*2022*Web: SQL Server 2022 Web Edition.
        """
        return pulumi.get(self, "db_engine_version")

    @_builtins.property
    @pulumi.getter(name="innerVersion")
    def inner_version(self) -> pulumi.Output[_builtins.str]:
        """
        Kernel version number.
        """
        return pulumi.get(self, "inner_version")

    @_builtins.property
    @pulumi.getter(name="instanceCategory")
    def instance_category(self) -> pulumi.Output[_builtins.str]:
        """
        Instance category. Values: Primary: primary instance. ReadOnly: read-only instance.
        """
        return pulumi.get(self, "instance_category")

    @_builtins.property
    @pulumi.getter(name="instanceId")
    def instance_id(self) -> pulumi.Output[_builtins.str]:
        """
        Instance ID.
        """
        return pulumi.get(self, "instance_id")

    @_builtins.property
    @pulumi.getter(name="instanceName")
    def instance_name(self) -> pulumi.Output[_builtins.str]:
        """
        Instance name. Naming rules: Cannot start with a digit or hyphen. Can only contain Chinese characters, letters, digits, underscores, and hyphens. Length must be between 1 and 128 characters.
        """
        return pulumi.get(self, "instance_name")

    @_builtins.property
    @pulumi.getter(name="instanceStatus")
    def instance_status(self) -> pulumi.Output[_builtins.str]:
        """
        Instance status. Values: Running: running. Creating: creating. CreateFailed: creation failed. Deleting: deleting. Restarting: restarting. Updating: updating. MasterChanging: primary node switching. Error: error.
        """
        return pulumi.get(self, "instance_status")

    @_builtins.property
    @pulumi.getter(name="instanceType")
    def instance_type(self) -> pulumi.Output[_builtins.str]:
        """
        Instance type. Available values: HA (High Availability), Basic, Cluster. Supported types vary by engine version: 2019 Std supports HA/Basic; 2019 Ent supports Cluster/Basic; 2019 Web supports Basic.
        """
        return pulumi.get(self, "instance_type")

    @_builtins.property
    @pulumi.getter(name="maintenanceTime")
    def maintenance_time(self) -> pulumi.Output[_builtins.str]:
        """
        Instance maintenance window. Format: HH:mmZ-HH:mmZ (UTC time). Default: UTC18:00Z-21:59Z (Beijing time 02:00-05:59). Note: To ensure cloud database stability, the system performs maintenance on instances periodically. It is recommended to set the maintenance window during off-peak hours to avoid business impact. During the maintenance window, the instance may experience 1–2 brief disconnections. Ensure your application supports automatic reconnection. The minimum maintenance window is 1 hour, the maximum is 24 hours, and cross-day maintenance windows are not allowed.
        """
        return pulumi.get(self, "maintenance_time")

    @_builtins.property
    @pulumi.getter(name="masterNodeId")
    def master_node_id(self) -> pulumi.Output[_builtins.str]:
        """
        Primary node ID. This parameter is required when manually switching master and standby nodes.
        """
        return pulumi.get(self, "master_node_id")

    @_builtins.property
    @pulumi.getter
    def memory(self) -> pulumi.Output[_builtins.int]:
        """
        Memory size, in GiB.
        """
        return pulumi.get(self, "memory")

    @_builtins.property
    @pulumi.getter(name="nodeDetailInfos")
    def node_detail_infos(self) -> pulumi.Output[Sequence['outputs.InstanceNodeDetailInfo']]:
        """
        Instance node information.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "node_detail_infos")

    @_builtins.property
    @pulumi.getter(name="nodeSpec")
    def node_spec(self) -> pulumi.Output[_builtins.str]:
        """
        Instance specification code.
        """
        return pulumi.get(self, "node_spec")

    @_builtins.property
    @pulumi.getter
    def port(self) -> pulumi.Output[_builtins.str]:
        """
        Port.
        """
        return pulumi.get(self, "port")

    @_builtins.property
    @pulumi.getter(name="primaryInstanceId")
    def primary_instance_id(self) -> pulumi.Output[_builtins.str]:
        """
        Primary instance ID. If this field is empty, this instance is the primary instance.
        """
        return pulumi.get(self, "primary_instance_id")

    @_builtins.property
    @pulumi.getter(name="projectName")
    def project_name(self) -> pulumi.Output[_builtins.str]:
        """
        Project.
        """
        return pulumi.get(self, "project_name")

    @_builtins.property
    @pulumi.getter(name="readOnlyNumber")
    def read_only_number(self) -> pulumi.Output[_builtins.int]:
        """
        Number of read-only instances.
        """
        return pulumi.get(self, "read_only_number")

    @_builtins.property
    @pulumi.getter(name="serverCollation")
    def server_collation(self) -> pulumi.Output[_builtins.str]:
        """
        Instance collation. Default: Chinese*PRC*CI*AS. Most native collations are supported. Available values: Latin1*General*CI*AS, Latin1*General*CS*AS, SQL*Latin1*General*CP1*CI*AS, SQL*Latin1*General*CP1*CS*AS, Chinese*PRC*CI*AS, Chinese*PRC*CS*AS, Chinese*PRC*BIN, Japanese*CI*AS, Japanese*CS*AS, Chinese*Taiwan*Stroke*CI*AS, Chinese*Taiwan*Stroke*CS*AS, Thai*CI*AS, Chinese*PRC*CI*AI, Chinese*PRC*BIN2, Cyrillic*General*CI_AS.
        """
        return pulumi.get(self, "server_collation")

    @_builtins.property
    @pulumi.getter(name="slowQueryEnable")
    def slow_query_enable(self) -> pulumi.Output[_builtins.bool]:
        """
        Indicates whether slow logs are enabled. Available values: true: slow logs enabled. false: slow logs disabled.
        """
        return pulumi.get(self, "slow_query_enable")

    @_builtins.property
    @pulumi.getter(name="slowQueryTime")
    def slow_query_time(self) -> pulumi.Output[_builtins.str]:
        """
        Slow log threshold. Range: [1~10], unit: seconds (s).
        """
        return pulumi.get(self, "slow_query_time")

    @_builtins.property
    @pulumi.getter(name="storageSpace")
    def storage_space(self) -> pulumi.Output[_builtins.int]:
        """
        Storage size, in GiB. Range: 20 GiB ~ 4000 GiB, increment: 10 GiB.
        """
        return pulumi.get(self, "storage_space")

    @_builtins.property
    @pulumi.getter(name="storageType")
    def storage_type(self) -> pulumi.Output[_builtins.str]:
        """
        Instance storage type.
        """
        return pulumi.get(self, "storage_type")

    @_builtins.property
    @pulumi.getter(name="storageUse")
    def storage_use(self) -> pulumi.Output[_builtins.float]:
        """
        Used storage space for the instance. Unit: GiB.
        """
        return pulumi.get(self, "storage_use")

    @_builtins.property
    @pulumi.getter(name="subnetId")
    def subnet_id(self) -> pulumi.Output[_builtins.str]:
        """
        Subnet ID. If the primary and standby nodes are in different availability zones, their subnets must be set to the corresponding private network subnets for each zone. Use a semicolon (;) to separate multiple zones.
        """
        return pulumi.get(self, "subnet_id")

    @_builtins.property
    @pulumi.getter(name="superAccountPassword")
    def super_account_password(self) -> pulumi.Output[_builtins.str]:
        """
        Password for the high-privilege account. Password rules: 8–32 characters; must include at least three of the following: uppercase letters, lowercase letters, numbers, special characters. Allowed special characters: !@#$%^&*()_+-=.
        """
        return pulumi.get(self, "super_account_password")

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Output[Sequence['outputs.InstanceTag']]:
        """
        Tag list.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "tags")

    @_builtins.property
    @pulumi.getter(name="timeZone")
    def time_zone(self) -> pulumi.Output[_builtins.str]:
        """
        Time zone.
        """
        return pulumi.get(self, "time_zone")

    @_builtins.property
    @pulumi.getter(name="updatedTime")
    def updated_time(self) -> pulumi.Output[_builtins.str]:
        """
        Update time.
        """
        return pulumi.get(self, "updated_time")

    @_builtins.property
    @pulumi.getter
    def vcpu(self) -> pulumi.Output[_builtins.int]:
        """
        CPU size. For example: 1 means 1 core.
        """
        return pulumi.get(self, "vcpu")

    @_builtins.property
    @pulumi.getter(name="vpcId")
    def vpc_id(self) -> pulumi.Output[_builtins.str]:
        """
        VPC ID
        """
        return pulumi.get(self, "vpc_id")

    @_builtins.property
    @pulumi.getter(name="zoneId")
    def zone_id(self) -> pulumi.Output[_builtins.str]:
        """
        Availability zone of the node. If the primary and standby nodes are in different availability zones, use a semicolon (;) to separate them. The first is the primary node's zone by default, the second is the standby node's zone.
        """
        return pulumi.get(self, "zone_id")

