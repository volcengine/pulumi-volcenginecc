# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities

__all__ = [
    'BackupPlanPolicyArgs',
    'BackupPlanPolicyArgsDict',
    'BackupPlanResourceListArgs',
    'BackupPlanResourceListArgsDict',
    'BackupPlanResourceListMetaInformationArgs',
    'BackupPlanResourceListMetaInformationArgsDict',
    'BackupResourceMetaInformationArgs',
    'BackupResourceMetaInformationArgsDict',
    'BackupResourcePlanArgs',
    'BackupResourcePlanArgsDict',
    'VaultTagArgs',
    'VaultTagArgsDict',
]

class BackupPlanPolicyArgsDict(TypedDict):
    account_id: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Account ID that created this policy
    """
    backup_type: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Backup type. Options: FULL: Full backup. INCREMENTAL: Incremental backup
    """
    created_time: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Creation Time
    """
    crontab: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Specifies the backup schedule. Supports standard crontab expressions
    """
    enable_policy: NotRequired[pulumi.Input[Optional[_builtins.bool]]]
    """
    Whether the policy is enabled. Options: true: Enable policy. false: Disable policy
    """
    name: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Policy Name
    """
    plan_number: NotRequired[pulumi.Input[Optional[_builtins.int]]]
    """
    Number of backup plans associated with this policy
    """
    policy_id: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Backup Policy ID
    """
    retention_day: NotRequired[pulumi.Input[Optional[_builtins.int]]]
    """
    Retention time for recovery points. -1 means retain indefinitely
    """
    updated_time: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Update Time
    """

@pulumi.input_type
class BackupPlanPolicyArgs:
    def __init__(__self__, *,
                 account_id: pulumi.Input[Optional[_builtins.str]] = None,
                 backup_type: pulumi.Input[Optional[_builtins.str]] = None,
                 created_time: pulumi.Input[Optional[_builtins.str]] = None,
                 crontab: pulumi.Input[Optional[_builtins.str]] = None,
                 enable_policy: pulumi.Input[Optional[_builtins.bool]] = None,
                 name: pulumi.Input[Optional[_builtins.str]] = None,
                 plan_number: pulumi.Input[Optional[_builtins.int]] = None,
                 policy_id: pulumi.Input[Optional[_builtins.str]] = None,
                 retention_day: pulumi.Input[Optional[_builtins.int]] = None,
                 updated_time: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.str] account_id: Account ID that created this policy
        :param pulumi.Input[_builtins.str] backup_type: Backup type. Options: FULL: Full backup. INCREMENTAL: Incremental backup
        :param pulumi.Input[_builtins.str] created_time: Creation Time
        :param pulumi.Input[_builtins.str] crontab: Specifies the backup schedule. Supports standard crontab expressions
        :param pulumi.Input[_builtins.bool] enable_policy: Whether the policy is enabled. Options: true: Enable policy. false: Disable policy
        :param pulumi.Input[_builtins.str] name: Policy Name
        :param pulumi.Input[_builtins.int] plan_number: Number of backup plans associated with this policy
        :param pulumi.Input[_builtins.str] policy_id: Backup Policy ID
        :param pulumi.Input[_builtins.int] retention_day: Retention time for recovery points. -1 means retain indefinitely
        :param pulumi.Input[_builtins.str] updated_time: Update Time
        """
        if account_id is not None:
            pulumi.set(__self__, "account_id", account_id)
        if backup_type is not None:
            pulumi.set(__self__, "backup_type", backup_type)
        if created_time is not None:
            pulumi.set(__self__, "created_time", created_time)
        if crontab is not None:
            pulumi.set(__self__, "crontab", crontab)
        if enable_policy is not None:
            pulumi.set(__self__, "enable_policy", enable_policy)
        if name is not None:
            pulumi.set(__self__, "name", name)
        if plan_number is not None:
            pulumi.set(__self__, "plan_number", plan_number)
        if policy_id is not None:
            pulumi.set(__self__, "policy_id", policy_id)
        if retention_day is not None:
            pulumi.set(__self__, "retention_day", retention_day)
        if updated_time is not None:
            pulumi.set(__self__, "updated_time", updated_time)

    @_builtins.property
    @pulumi.getter(name="accountId")
    def account_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Account ID that created this policy
        """
        return pulumi.get(self, "account_id")

    @account_id.setter
    def account_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "account_id", value)

    @_builtins.property
    @pulumi.getter(name="backupType")
    def backup_type(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Backup type. Options: FULL: Full backup. INCREMENTAL: Incremental backup
        """
        return pulumi.get(self, "backup_type")

    @backup_type.setter
    def backup_type(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "backup_type", value)

    @_builtins.property
    @pulumi.getter(name="createdTime")
    def created_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Creation Time
        """
        return pulumi.get(self, "created_time")

    @created_time.setter
    def created_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "created_time", value)

    @_builtins.property
    @pulumi.getter
    def crontab(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Specifies the backup schedule. Supports standard crontab expressions
        """
        return pulumi.get(self, "crontab")

    @crontab.setter
    def crontab(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "crontab", value)

    @_builtins.property
    @pulumi.getter(name="enablePolicy")
    def enable_policy(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Whether the policy is enabled. Options: true: Enable policy. false: Disable policy
        """
        return pulumi.get(self, "enable_policy")

    @enable_policy.setter
    def enable_policy(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "enable_policy", value)

    @_builtins.property
    @pulumi.getter
    def name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Policy Name
        """
        return pulumi.get(self, "name")

    @name.setter
    def name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "name", value)

    @_builtins.property
    @pulumi.getter(name="planNumber")
    def plan_number(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Number of backup plans associated with this policy
        """
        return pulumi.get(self, "plan_number")

    @plan_number.setter
    def plan_number(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "plan_number", value)

    @_builtins.property
    @pulumi.getter(name="policyId")
    def policy_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Backup Policy ID
        """
        return pulumi.get(self, "policy_id")

    @policy_id.setter
    def policy_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "policy_id", value)

    @_builtins.property
    @pulumi.getter(name="retentionDay")
    def retention_day(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Retention time for recovery points. -1 means retain indefinitely
        """
        return pulumi.get(self, "retention_day")

    @retention_day.setter
    def retention_day(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "retention_day", value)

    @_builtins.property
    @pulumi.getter(name="updatedTime")
    def updated_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Update Time
        """
        return pulumi.get(self, "updated_time")

    @updated_time.setter
    def updated_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "updated_time", value)


class BackupPlanResourceListArgsDict(TypedDict):
    meta_information: pulumi.Input['BackupPlanResourceListMetaInformationArgsDict']
    """
    Resource meta information, used to store additional configuration when creating a backup plan
    """
    resource_id: pulumi.Input[_builtins.str]
    """
    Backup Source ID
    """
    resource_type: pulumi.Input[_builtins.str]
    """
    Backup source type. Options: ECS: ECS full machine backup. vePFS: vePFS file system backup
    """

@pulumi.input_type
class BackupPlanResourceListArgs:
    def __init__(__self__, *,
                 meta_information: pulumi.Input['BackupPlanResourceListMetaInformationArgs'],
                 resource_id: pulumi.Input[_builtins.str],
                 resource_type: pulumi.Input[_builtins.str]):
        """
        :param pulumi.Input['BackupPlanResourceListMetaInformationArgs'] meta_information: Resource meta information, used to store additional configuration when creating a backup plan
        :param pulumi.Input[_builtins.str] resource_id: Backup Source ID
        :param pulumi.Input[_builtins.str] resource_type: Backup source type. Options: ECS: ECS full machine backup. vePFS: vePFS file system backup
        """
        pulumi.set(__self__, "meta_information", meta_information)
        pulumi.set(__self__, "resource_id", resource_id)
        pulumi.set(__self__, "resource_type", resource_type)

    @_builtins.property
    @pulumi.getter(name="metaInformation")
    def meta_information(self) -> pulumi.Input['BackupPlanResourceListMetaInformationArgs']:
        """
        Resource meta information, used to store additional configuration when creating a backup plan
        """
        return pulumi.get(self, "meta_information")

    @meta_information.setter
    def meta_information(self, value: pulumi.Input['BackupPlanResourceListMetaInformationArgs']):
        pulumi.set(self, "meta_information", value)

    @_builtins.property
    @pulumi.getter(name="resourceId")
    def resource_id(self) -> pulumi.Input[_builtins.str]:
        """
        Backup Source ID
        """
        return pulumi.get(self, "resource_id")

    @resource_id.setter
    def resource_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "resource_id", value)

    @_builtins.property
    @pulumi.getter(name="resourceType")
    def resource_type(self) -> pulumi.Input[_builtins.str]:
        """
        Backup source type. Options: ECS: ECS full machine backup. vePFS: vePFS file system backup
        """
        return pulumi.get(self, "resource_type")

    @resource_type.setter
    def resource_type(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "resource_type", value)


class BackupPlanResourceListMetaInformationArgsDict(TypedDict):
    ecs_meta: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    ECS metadata, which is a JSON string
    """
    vepfs_meta: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    vePFS backup source metadata, which is a JSON string
    """

@pulumi.input_type
class BackupPlanResourceListMetaInformationArgs:
    def __init__(__self__, *,
                 ecs_meta: pulumi.Input[Optional[_builtins.str]] = None,
                 vepfs_meta: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.str] ecs_meta: ECS metadata, which is a JSON string
        :param pulumi.Input[_builtins.str] vepfs_meta: vePFS backup source metadata, which is a JSON string
        """
        if ecs_meta is not None:
            pulumi.set(__self__, "ecs_meta", ecs_meta)
        if vepfs_meta is not None:
            pulumi.set(__self__, "vepfs_meta", vepfs_meta)

    @_builtins.property
    @pulumi.getter(name="ecsMeta")
    def ecs_meta(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        ECS metadata, which is a JSON string
        """
        return pulumi.get(self, "ecs_meta")

    @ecs_meta.setter
    def ecs_meta(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "ecs_meta", value)

    @_builtins.property
    @pulumi.getter(name="vepfsMeta")
    def vepfs_meta(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        vePFS backup source metadata, which is a JSON string
        """
        return pulumi.get(self, "vepfs_meta")

    @vepfs_meta.setter
    def vepfs_meta(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "vepfs_meta", value)


class BackupResourceMetaInformationArgsDict(TypedDict):
    ecs_meta: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    ECS metadata, as a JSON string. For format, see the EcsBackupConfiguration structure in the data structure.
    """
    vepfs_meta: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    vePFS backup source metadata, as a JSON string. For format, see the VePFSBackupConfiguration structure in the data structure.
    """

@pulumi.input_type
class BackupResourceMetaInformationArgs:
    def __init__(__self__, *,
                 ecs_meta: pulumi.Input[Optional[_builtins.str]] = None,
                 vepfs_meta: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.str] ecs_meta: ECS metadata, as a JSON string. For format, see the EcsBackupConfiguration structure in the data structure.
        :param pulumi.Input[_builtins.str] vepfs_meta: vePFS backup source metadata, as a JSON string. For format, see the VePFSBackupConfiguration structure in the data structure.
        """
        if ecs_meta is not None:
            pulumi.set(__self__, "ecs_meta", ecs_meta)
        if vepfs_meta is not None:
            pulumi.set(__self__, "vepfs_meta", vepfs_meta)

    @_builtins.property
    @pulumi.getter(name="ecsMeta")
    def ecs_meta(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        ECS metadata, as a JSON string. For format, see the EcsBackupConfiguration structure in the data structure.
        """
        return pulumi.get(self, "ecs_meta")

    @ecs_meta.setter
    def ecs_meta(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "ecs_meta", value)

    @_builtins.property
    @pulumi.getter(name="vepfsMeta")
    def vepfs_meta(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        vePFS backup source metadata, as a JSON string. For format, see the VePFSBackupConfiguration structure in the data structure.
        """
        return pulumi.get(self, "vepfs_meta")

    @vepfs_meta.setter
    def vepfs_meta(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "vepfs_meta", value)


class BackupResourcePlanArgsDict(TypedDict):
    plan_id: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Backup Plan ID
    """
    plan_name: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Backup Plan Name
    """

@pulumi.input_type
class BackupResourcePlanArgs:
    def __init__(__self__, *,
                 plan_id: pulumi.Input[Optional[_builtins.str]] = None,
                 plan_name: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.str] plan_id: Backup Plan ID
        :param pulumi.Input[_builtins.str] plan_name: Backup Plan Name
        """
        if plan_id is not None:
            pulumi.set(__self__, "plan_id", plan_id)
        if plan_name is not None:
            pulumi.set(__self__, "plan_name", plan_name)

    @_builtins.property
    @pulumi.getter(name="planId")
    def plan_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Backup Plan ID
        """
        return pulumi.get(self, "plan_id")

    @plan_id.setter
    def plan_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "plan_id", value)

    @_builtins.property
    @pulumi.getter(name="planName")
    def plan_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Backup Plan Name
        """
        return pulumi.get(self, "plan_name")

    @plan_name.setter
    def plan_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "plan_name", value)


class VaultTagArgsDict(TypedDict):
    key: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Tag Key
    """
    value: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Tag Value
    """

@pulumi.input_type
class VaultTagArgs:
    def __init__(__self__, *,
                 key: pulumi.Input[Optional[_builtins.str]] = None,
                 value: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.str] key: Tag Key
        :param pulumi.Input[_builtins.str] value: Tag Value
        """
        if key is not None:
            pulumi.set(__self__, "key", key)
        if value is not None:
            pulumi.set(__self__, "value", value)

    @_builtins.property
    @pulumi.getter
    def key(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Tag Key
        """
        return pulumi.get(self, "key")

    @key.setter
    def key(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "key", value)

    @_builtins.property
    @pulumi.getter
    def value(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Tag Value
        """
        return pulumi.get(self, "value")

    @value.setter
    def value(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "value", value)


