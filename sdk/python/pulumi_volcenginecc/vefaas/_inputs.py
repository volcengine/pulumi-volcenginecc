# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities

__all__ = [
    'FunctionAsyncTaskConfigArgs',
    'FunctionAsyncTaskConfigArgsDict',
    'FunctionAsyncTaskConfigDestinationConfigArgs',
    'FunctionAsyncTaskConfigDestinationConfigArgsDict',
    'FunctionAsyncTaskConfigDestinationConfigOnFailureArgs',
    'FunctionAsyncTaskConfigDestinationConfigOnFailureArgsDict',
    'FunctionAsyncTaskConfigDestinationConfigOnSuccessArgs',
    'FunctionAsyncTaskConfigDestinationConfigOnSuccessArgsDict',
    'FunctionDependencyInstallStatusArgs',
    'FunctionDependencyInstallStatusArgsDict',
    'FunctionEnvArgs',
    'FunctionEnvArgsDict',
    'FunctionNasStorageArgs',
    'FunctionNasStorageArgsDict',
    'FunctionNasStorageNasConfigArgs',
    'FunctionNasStorageNasConfigArgsDict',
    'FunctionSourceAccessConfigArgs',
    'FunctionSourceAccessConfigArgsDict',
    'FunctionTagArgs',
    'FunctionTagArgsDict',
    'FunctionTlsConfigArgs',
    'FunctionTlsConfigArgsDict',
    'FunctionTosMountConfigArgs',
    'FunctionTosMountConfigArgsDict',
    'FunctionTosMountConfigCredentialsArgs',
    'FunctionTosMountConfigCredentialsArgsDict',
    'FunctionTosMountConfigMountPointArgs',
    'FunctionTosMountConfigMountPointArgsDict',
    'FunctionVpcConfigArgs',
    'FunctionVpcConfigArgsDict',
    'KafkaTriggerKafkaCredentialsArgs',
    'KafkaTriggerKafkaCredentialsArgsDict',
    'SandboxEnvArgs',
    'SandboxEnvArgsDict',
    'SandboxInstanceImageInfoArgs',
    'SandboxInstanceImageInfoArgsDict',
    'SandboxInstanceTosMountConfigArgs',
    'SandboxInstanceTosMountConfigArgsDict',
    'SandboxInstanceTosMountConfigTosMountPointArgs',
    'SandboxInstanceTosMountConfigTosMountPointArgsDict',
    'SandboxMetadataArgs',
    'SandboxMetadataArgsDict',
]

class FunctionAsyncTaskConfigArgsDict(TypedDict):
    destination_config: NotRequired[pulumi.Input[Optional['FunctionAsyncTaskConfigDestinationConfigArgsDict']]]
    """
    Asynchronous task callback target configuration
    """
    enable_async_task: NotRequired[pulumi.Input[Optional[_builtins.bool]]]
    """
    Whether to enable asynchronous task
    """
    max_retry: NotRequired[pulumi.Input[Optional[_builtins.int]]]
    """
    Number of retry attempts for asynchronous task failures. Available values: 0–3
    """

@pulumi.input_type
class FunctionAsyncTaskConfigArgs:
    def __init__(__self__, *,
                 destination_config: pulumi.Input[Optional['FunctionAsyncTaskConfigDestinationConfigArgs']] = None,
                 enable_async_task: pulumi.Input[Optional[_builtins.bool]] = None,
                 max_retry: pulumi.Input[Optional[_builtins.int]] = None):
        """
        :param pulumi.Input['FunctionAsyncTaskConfigDestinationConfigArgs'] destination_config: Asynchronous task callback target configuration
        :param pulumi.Input[_builtins.bool] enable_async_task: Whether to enable asynchronous task
        :param pulumi.Input[_builtins.int] max_retry: Number of retry attempts for asynchronous task failures. Available values: 0–3
        """
        if destination_config is not None:
            pulumi.set(__self__, "destination_config", destination_config)
        if enable_async_task is not None:
            pulumi.set(__self__, "enable_async_task", enable_async_task)
        if max_retry is not None:
            pulumi.set(__self__, "max_retry", max_retry)

    @_builtins.property
    @pulumi.getter(name="destinationConfig")
    def destination_config(self) -> pulumi.Input[Optional['FunctionAsyncTaskConfigDestinationConfigArgs']]:
        """
        Asynchronous task callback target configuration
        """
        return pulumi.get(self, "destination_config")

    @destination_config.setter
    def destination_config(self, value: pulumi.Input[Optional['FunctionAsyncTaskConfigDestinationConfigArgs']]):
        pulumi.set(self, "destination_config", value)

    @_builtins.property
    @pulumi.getter(name="enableAsyncTask")
    def enable_async_task(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Whether to enable asynchronous task
        """
        return pulumi.get(self, "enable_async_task")

    @enable_async_task.setter
    def enable_async_task(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "enable_async_task", value)

    @_builtins.property
    @pulumi.getter(name="maxRetry")
    def max_retry(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Number of retry attempts for asynchronous task failures. Available values: 0–3
        """
        return pulumi.get(self, "max_retry")

    @max_retry.setter
    def max_retry(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "max_retry", value)


class FunctionAsyncTaskConfigDestinationConfigArgsDict(TypedDict):
    on_failure: NotRequired[pulumi.Input[Optional['FunctionAsyncTaskConfigDestinationConfigOnFailureArgsDict']]]
    """
    Callback target configuration when the task fails
    """
    on_success: NotRequired[pulumi.Input[Optional['FunctionAsyncTaskConfigDestinationConfigOnSuccessArgsDict']]]
    """
    Callback target configuration when the task succeeds
    """

@pulumi.input_type
class FunctionAsyncTaskConfigDestinationConfigArgs:
    def __init__(__self__, *,
                 on_failure: pulumi.Input[Optional['FunctionAsyncTaskConfigDestinationConfigOnFailureArgs']] = None,
                 on_success: pulumi.Input[Optional['FunctionAsyncTaskConfigDestinationConfigOnSuccessArgs']] = None):
        """
        :param pulumi.Input['FunctionAsyncTaskConfigDestinationConfigOnFailureArgs'] on_failure: Callback target configuration when the task fails
        :param pulumi.Input['FunctionAsyncTaskConfigDestinationConfigOnSuccessArgs'] on_success: Callback target configuration when the task succeeds
        """
        if on_failure is not None:
            pulumi.set(__self__, "on_failure", on_failure)
        if on_success is not None:
            pulumi.set(__self__, "on_success", on_success)

    @_builtins.property
    @pulumi.getter(name="onFailure")
    def on_failure(self) -> pulumi.Input[Optional['FunctionAsyncTaskConfigDestinationConfigOnFailureArgs']]:
        """
        Callback target configuration when the task fails
        """
        return pulumi.get(self, "on_failure")

    @on_failure.setter
    def on_failure(self, value: pulumi.Input[Optional['FunctionAsyncTaskConfigDestinationConfigOnFailureArgs']]):
        pulumi.set(self, "on_failure", value)

    @_builtins.property
    @pulumi.getter(name="onSuccess")
    def on_success(self) -> pulumi.Input[Optional['FunctionAsyncTaskConfigDestinationConfigOnSuccessArgs']]:
        """
        Callback target configuration when the task succeeds
        """
        return pulumi.get(self, "on_success")

    @on_success.setter
    def on_success(self, value: pulumi.Input[Optional['FunctionAsyncTaskConfigDestinationConfigOnSuccessArgs']]):
        pulumi.set(self, "on_success", value)


class FunctionAsyncTaskConfigDestinationConfigOnFailureArgsDict(TypedDict):
    destination: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Specific callback target. Set the value to empty ("") to disable this callback
    """

@pulumi.input_type
class FunctionAsyncTaskConfigDestinationConfigOnFailureArgs:
    def __init__(__self__, *,
                 destination: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.str] destination: Specific callback target. Set the value to empty ("") to disable this callback
        """
        if destination is not None:
            pulumi.set(__self__, "destination", destination)

    @_builtins.property
    @pulumi.getter
    def destination(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Specific callback target. Set the value to empty ("") to disable this callback
        """
        return pulumi.get(self, "destination")

    @destination.setter
    def destination(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "destination", value)


class FunctionAsyncTaskConfigDestinationConfigOnSuccessArgsDict(TypedDict):
    destination: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Specific callback target. Set the value to empty ("") to disable this callback
    """

@pulumi.input_type
class FunctionAsyncTaskConfigDestinationConfigOnSuccessArgs:
    def __init__(__self__, *,
                 destination: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.str] destination: Specific callback target. Set the value to empty ("") to disable this callback
        """
        if destination is not None:
            pulumi.set(__self__, "destination", destination)

    @_builtins.property
    @pulumi.getter
    def destination(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Specific callback target. Set the value to empty ("") to disable this callback
        """
        return pulumi.get(self, "destination")

    @destination.setter
    def destination(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "destination", value)


class FunctionDependencyInstallStatusArgsDict(TypedDict):
    create_time: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Task creation time.
    """
    finish_time: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Task completion time. The task is considered complete when it is in one of the following statuses: Canceled: Canceled. Timeout: Timed out. Failed: Failed. Succeeded: Succeeded.
    """
    status: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Task status. Possible values are as follows: Enqueued: Queued. Dequeued: Dequeued. InProgress: In progress. Canceling/Canceled: Canceling/Canceled. Timeout: Timed out. Failed: Failed. Succeeded: Succeeded.
    """

@pulumi.input_type
class FunctionDependencyInstallStatusArgs:
    def __init__(__self__, *,
                 create_time: pulumi.Input[Optional[_builtins.str]] = None,
                 finish_time: pulumi.Input[Optional[_builtins.str]] = None,
                 status: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.str] create_time: Task creation time.
        :param pulumi.Input[_builtins.str] finish_time: Task completion time. The task is considered complete when it is in one of the following statuses: Canceled: Canceled. Timeout: Timed out. Failed: Failed. Succeeded: Succeeded.
        :param pulumi.Input[_builtins.str] status: Task status. Possible values are as follows: Enqueued: Queued. Dequeued: Dequeued. InProgress: In progress. Canceling/Canceled: Canceling/Canceled. Timeout: Timed out. Failed: Failed. Succeeded: Succeeded.
        """
        if create_time is not None:
            pulumi.set(__self__, "create_time", create_time)
        if finish_time is not None:
            pulumi.set(__self__, "finish_time", finish_time)
        if status is not None:
            pulumi.set(__self__, "status", status)

    @_builtins.property
    @pulumi.getter(name="createTime")
    def create_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Task creation time.
        """
        return pulumi.get(self, "create_time")

    @create_time.setter
    def create_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "create_time", value)

    @_builtins.property
    @pulumi.getter(name="finishTime")
    def finish_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Task completion time. The task is considered complete when it is in one of the following statuses: Canceled: Canceled. Timeout: Timed out. Failed: Failed. Succeeded: Succeeded.
        """
        return pulumi.get(self, "finish_time")

    @finish_time.setter
    def finish_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "finish_time", value)

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Task status. Possible values are as follows: Enqueued: Queued. Dequeued: Dequeued. InProgress: In progress. Canceling/Canceled: Canceling/Canceled. Timeout: Timed out. Failed: Failed. Succeeded: Succeeded.
        """
        return pulumi.get(self, "status")

    @status.setter
    def status(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "status", value)


class FunctionEnvArgsDict(TypedDict):
    key: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Environment variable key. 1–63 characters; can include letters, numbers, and underscores (_), but cannot start with a number. Key must be unique within the same function
    """
    value: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Environment variable value
    """

@pulumi.input_type
class FunctionEnvArgs:
    def __init__(__self__, *,
                 key: pulumi.Input[Optional[_builtins.str]] = None,
                 value: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.str] key: Environment variable key. 1–63 characters; can include letters, numbers, and underscores (_), but cannot start with a number. Key must be unique within the same function
        :param pulumi.Input[_builtins.str] value: Environment variable value
        """
        if key is not None:
            pulumi.set(__self__, "key", key)
        if value is not None:
            pulumi.set(__self__, "value", value)

    @_builtins.property
    @pulumi.getter
    def key(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Environment variable key. 1–63 characters; can include letters, numbers, and underscores (_), but cannot start with a number. Key must be unique within the same function
        """
        return pulumi.get(self, "key")

    @key.setter
    def key(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "key", value)

    @_builtins.property
    @pulumi.getter
    def value(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Environment variable value
        """
        return pulumi.get(self, "value")

    @value.setter
    def value(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "value", value)


class FunctionNasStorageArgsDict(TypedDict):
    enable_nas: NotRequired[pulumi.Input[Optional[_builtins.bool]]]
    """
    Enable NAS storage mount. true: enabled, store data in NAS. false: disabled.
    """
    nas_configs: NotRequired[pulumi.Input[Optional[Sequence[pulumi.Input['FunctionNasStorageNasConfigArgsDict']]]]]
    """
    NAS storage configuration
    Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
    """

@pulumi.input_type
class FunctionNasStorageArgs:
    def __init__(__self__, *,
                 enable_nas: pulumi.Input[Optional[_builtins.bool]] = None,
                 nas_configs: pulumi.Input[Optional[Sequence[pulumi.Input['FunctionNasStorageNasConfigArgs']]]] = None):
        """
        :param pulumi.Input[_builtins.bool] enable_nas: Enable NAS storage mount. true: enabled, store data in NAS. false: disabled.
        :param pulumi.Input[Sequence[pulumi.Input['FunctionNasStorageNasConfigArgs']]] nas_configs: NAS storage configuration
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        if enable_nas is not None:
            pulumi.set(__self__, "enable_nas", enable_nas)
        if nas_configs is not None:
            pulumi.set(__self__, "nas_configs", nas_configs)

    @_builtins.property
    @pulumi.getter(name="enableNas")
    def enable_nas(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Enable NAS storage mount. true: enabled, store data in NAS. false: disabled.
        """
        return pulumi.get(self, "enable_nas")

    @enable_nas.setter
    def enable_nas(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "enable_nas", value)

    @_builtins.property
    @pulumi.getter(name="nasConfigs")
    def nas_configs(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['FunctionNasStorageNasConfigArgs']]]]:
        """
        NAS storage configuration
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "nas_configs")

    @nas_configs.setter
    def nas_configs(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['FunctionNasStorageNasConfigArgs']]]]):
        pulumi.set(self, "nas_configs", value)


class FunctionNasStorageNasConfigArgsDict(TypedDict):
    file_system_id: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    NAS file system ID.
    """
    gid: NotRequired[pulumi.Input[Optional[_builtins.int]]]
    """
    User group in the file system. Customization is not currently supported. If this parameter is provided, its value is 1000 (consistent with the function run user gid)
    """
    local_mount_path: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Function local mount directory. Length: 1–128 characters, must start with “/”, supports uppercase and lowercase letters, numbers, and special characters (  - . _ ~ / : @ = + ? #).
    """
    mount_point_id: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    NAS mount point ID
    """
    remote_path: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Remote directory in the file system. Length: 1–128 characters, must start with “/”, supports uppercase and lowercase letters, numbers, and special characters (  - . _ ~ / : @ = + ? #)
    """
    uid: NotRequired[pulumi.Input[Optional[_builtins.int]]]
    """
    The user in the file system cannot be customized for now. If this parameter is provided, the value must be 1000 (consistent with the function run user uid).
    """

@pulumi.input_type
class FunctionNasStorageNasConfigArgs:
    def __init__(__self__, *,
                 file_system_id: pulumi.Input[Optional[_builtins.str]] = None,
                 gid: pulumi.Input[Optional[_builtins.int]] = None,
                 local_mount_path: pulumi.Input[Optional[_builtins.str]] = None,
                 mount_point_id: pulumi.Input[Optional[_builtins.str]] = None,
                 remote_path: pulumi.Input[Optional[_builtins.str]] = None,
                 uid: pulumi.Input[Optional[_builtins.int]] = None):
        """
        :param pulumi.Input[_builtins.str] file_system_id: NAS file system ID.
        :param pulumi.Input[_builtins.int] gid: User group in the file system. Customization is not currently supported. If this parameter is provided, its value is 1000 (consistent with the function run user gid)
        :param pulumi.Input[_builtins.str] local_mount_path: Function local mount directory. Length: 1–128 characters, must start with “/”, supports uppercase and lowercase letters, numbers, and special characters (  - . _ ~ / : @ = + ? #).
        :param pulumi.Input[_builtins.str] mount_point_id: NAS mount point ID
        :param pulumi.Input[_builtins.str] remote_path: Remote directory in the file system. Length: 1–128 characters, must start with “/”, supports uppercase and lowercase letters, numbers, and special characters (  - . _ ~ / : @ = + ? #)
        :param pulumi.Input[_builtins.int] uid: The user in the file system cannot be customized for now. If this parameter is provided, the value must be 1000 (consistent with the function run user uid).
        """
        if file_system_id is not None:
            pulumi.set(__self__, "file_system_id", file_system_id)
        if gid is not None:
            pulumi.set(__self__, "gid", gid)
        if local_mount_path is not None:
            pulumi.set(__self__, "local_mount_path", local_mount_path)
        if mount_point_id is not None:
            pulumi.set(__self__, "mount_point_id", mount_point_id)
        if remote_path is not None:
            pulumi.set(__self__, "remote_path", remote_path)
        if uid is not None:
            pulumi.set(__self__, "uid", uid)

    @_builtins.property
    @pulumi.getter(name="fileSystemId")
    def file_system_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        NAS file system ID.
        """
        return pulumi.get(self, "file_system_id")

    @file_system_id.setter
    def file_system_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "file_system_id", value)

    @_builtins.property
    @pulumi.getter
    def gid(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        User group in the file system. Customization is not currently supported. If this parameter is provided, its value is 1000 (consistent with the function run user gid)
        """
        return pulumi.get(self, "gid")

    @gid.setter
    def gid(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "gid", value)

    @_builtins.property
    @pulumi.getter(name="localMountPath")
    def local_mount_path(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Function local mount directory. Length: 1–128 characters, must start with “/”, supports uppercase and lowercase letters, numbers, and special characters (  - . _ ~ / : @ = + ? #).
        """
        return pulumi.get(self, "local_mount_path")

    @local_mount_path.setter
    def local_mount_path(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "local_mount_path", value)

    @_builtins.property
    @pulumi.getter(name="mountPointId")
    def mount_point_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        NAS mount point ID
        """
        return pulumi.get(self, "mount_point_id")

    @mount_point_id.setter
    def mount_point_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "mount_point_id", value)

    @_builtins.property
    @pulumi.getter(name="remotePath")
    def remote_path(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Remote directory in the file system. Length: 1–128 characters, must start with “/”, supports uppercase and lowercase letters, numbers, and special characters (  - . _ ~ / : @ = + ? #)
        """
        return pulumi.get(self, "remote_path")

    @remote_path.setter
    def remote_path(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "remote_path", value)

    @_builtins.property
    @pulumi.getter
    def uid(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        The user in the file system cannot be customized for now. If this parameter is provided, the value must be 1000 (consistent with the function run user uid).
        """
        return pulumi.get(self, "uid")

    @uid.setter
    def uid(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "uid", value)


class FunctionSourceAccessConfigArgsDict(TypedDict):
    password: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Image repository password
    """
    username: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Image repository username
    """

@pulumi.input_type
class FunctionSourceAccessConfigArgs:
    def __init__(__self__, *,
                 password: pulumi.Input[Optional[_builtins.str]] = None,
                 username: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.str] password: Image repository password
        :param pulumi.Input[_builtins.str] username: Image repository username
        """
        if password is not None:
            pulumi.set(__self__, "password", password)
        if username is not None:
            pulumi.set(__self__, "username", username)

    @_builtins.property
    @pulumi.getter
    def password(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Image repository password
        """
        return pulumi.get(self, "password")

    @password.setter
    def password(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "password", value)

    @_builtins.property
    @pulumi.getter
    def username(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Image repository username
        """
        return pulumi.get(self, "username")

    @username.setter
    def username(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "username", value)


class FunctionTagArgsDict(TypedDict):
    key: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Function tag key
    """
    value: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Function tag value
    """

@pulumi.input_type
class FunctionTagArgs:
    def __init__(__self__, *,
                 key: pulumi.Input[Optional[_builtins.str]] = None,
                 value: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.str] key: Function tag key
        :param pulumi.Input[_builtins.str] value: Function tag value
        """
        if key is not None:
            pulumi.set(__self__, "key", key)
        if value is not None:
            pulumi.set(__self__, "value", value)

    @_builtins.property
    @pulumi.getter
    def key(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Function tag key
        """
        return pulumi.get(self, "key")

    @key.setter
    def key(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "key", value)

    @_builtins.property
    @pulumi.getter
    def value(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Function tag value
        """
        return pulumi.get(self, "value")

    @value.setter
    def value(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "value", value)


class FunctionTlsConfigArgsDict(TypedDict):
    enable_log: NotRequired[pulumi.Input[Optional[_builtins.bool]]]
    """
    TLS log feature toggle. true: enabled, function stdout/stderr logs will be collected to the specified log project and log topic in Log Service. false: disabled, logs will not be collected
    """
    tls_project_id: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    TLS log project ID
    """
    tls_topic_id: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    TLS log topic ID.
    """

@pulumi.input_type
class FunctionTlsConfigArgs:
    def __init__(__self__, *,
                 enable_log: pulumi.Input[Optional[_builtins.bool]] = None,
                 tls_project_id: pulumi.Input[Optional[_builtins.str]] = None,
                 tls_topic_id: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.bool] enable_log: TLS log feature toggle. true: enabled, function stdout/stderr logs will be collected to the specified log project and log topic in Log Service. false: disabled, logs will not be collected
        :param pulumi.Input[_builtins.str] tls_project_id: TLS log project ID
        :param pulumi.Input[_builtins.str] tls_topic_id: TLS log topic ID.
        """
        if enable_log is not None:
            pulumi.set(__self__, "enable_log", enable_log)
        if tls_project_id is not None:
            pulumi.set(__self__, "tls_project_id", tls_project_id)
        if tls_topic_id is not None:
            pulumi.set(__self__, "tls_topic_id", tls_topic_id)

    @_builtins.property
    @pulumi.getter(name="enableLog")
    def enable_log(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        TLS log feature toggle. true: enabled, function stdout/stderr logs will be collected to the specified log project and log topic in Log Service. false: disabled, logs will not be collected
        """
        return pulumi.get(self, "enable_log")

    @enable_log.setter
    def enable_log(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "enable_log", value)

    @_builtins.property
    @pulumi.getter(name="tlsProjectId")
    def tls_project_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        TLS log project ID
        """
        return pulumi.get(self, "tls_project_id")

    @tls_project_id.setter
    def tls_project_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "tls_project_id", value)

    @_builtins.property
    @pulumi.getter(name="tlsTopicId")
    def tls_topic_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        TLS log topic ID.
        """
        return pulumi.get(self, "tls_topic_id")

    @tls_topic_id.setter
    def tls_topic_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "tls_topic_id", value)


class FunctionTosMountConfigArgsDict(TypedDict):
    credentials: NotRequired[pulumi.Input[Optional['FunctionTosMountConfigCredentialsArgsDict']]]
    """
    AKSK with TOS domain access permission.
    """
    enable_tos: NotRequired[pulumi.Input[Optional[_builtins.bool]]]
    """
    Enable TOS storage mount.
    """
    mount_points: NotRequired[pulumi.Input[Optional[Sequence[pulumi.Input['FunctionTosMountConfigMountPointArgsDict']]]]]
    """
    TOS storage configuration list
    Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
    """

@pulumi.input_type
class FunctionTosMountConfigArgs:
    def __init__(__self__, *,
                 credentials: pulumi.Input[Optional['FunctionTosMountConfigCredentialsArgs']] = None,
                 enable_tos: pulumi.Input[Optional[_builtins.bool]] = None,
                 mount_points: pulumi.Input[Optional[Sequence[pulumi.Input['FunctionTosMountConfigMountPointArgs']]]] = None):
        """
        :param pulumi.Input['FunctionTosMountConfigCredentialsArgs'] credentials: AKSK with TOS domain access permission.
        :param pulumi.Input[_builtins.bool] enable_tos: Enable TOS storage mount.
        :param pulumi.Input[Sequence[pulumi.Input['FunctionTosMountConfigMountPointArgs']]] mount_points: TOS storage configuration list
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        if credentials is not None:
            pulumi.set(__self__, "credentials", credentials)
        if enable_tos is not None:
            pulumi.set(__self__, "enable_tos", enable_tos)
        if mount_points is not None:
            pulumi.set(__self__, "mount_points", mount_points)

    @_builtins.property
    @pulumi.getter
    def credentials(self) -> pulumi.Input[Optional['FunctionTosMountConfigCredentialsArgs']]:
        """
        AKSK with TOS domain access permission.
        """
        return pulumi.get(self, "credentials")

    @credentials.setter
    def credentials(self, value: pulumi.Input[Optional['FunctionTosMountConfigCredentialsArgs']]):
        pulumi.set(self, "credentials", value)

    @_builtins.property
    @pulumi.getter(name="enableTos")
    def enable_tos(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Enable TOS storage mount.
        """
        return pulumi.get(self, "enable_tos")

    @enable_tos.setter
    def enable_tos(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "enable_tos", value)

    @_builtins.property
    @pulumi.getter(name="mountPoints")
    def mount_points(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['FunctionTosMountConfigMountPointArgs']]]]:
        """
        TOS storage configuration list
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "mount_points")

    @mount_points.setter
    def mount_points(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['FunctionTosMountConfigMountPointArgs']]]]):
        pulumi.set(self, "mount_points", value)


class FunctionTosMountConfigCredentialsArgsDict(TypedDict):
    access_key_id: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Account AccessKey ID
    """
    secret_access_key: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Account Secret Access Key
    """

@pulumi.input_type
class FunctionTosMountConfigCredentialsArgs:
    def __init__(__self__, *,
                 access_key_id: pulumi.Input[Optional[_builtins.str]] = None,
                 secret_access_key: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.str] access_key_id: Account AccessKey ID
        :param pulumi.Input[_builtins.str] secret_access_key: Account Secret Access Key
        """
        if access_key_id is not None:
            pulumi.set(__self__, "access_key_id", access_key_id)
        if secret_access_key is not None:
            pulumi.set(__self__, "secret_access_key", secret_access_key)

    @_builtins.property
    @pulumi.getter(name="accessKeyId")
    def access_key_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Account AccessKey ID
        """
        return pulumi.get(self, "access_key_id")

    @access_key_id.setter
    def access_key_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "access_key_id", value)

    @_builtins.property
    @pulumi.getter(name="secretAccessKey")
    def secret_access_key(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Account Secret Access Key
        """
        return pulumi.get(self, "secret_access_key")

    @secret_access_key.setter
    def secret_access_key(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "secret_access_key", value)


class FunctionTosMountConfigMountPointArgsDict(TypedDict):
    bucket_name: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    TOS bucket name.
    """
    bucket_path: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Mounted TOS Bucket path.
    """
    endpoint: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    TOS access domain name.
    """
    local_mount_path: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Function local mount directory.
    """
    read_only: NotRequired[pulumi.Input[Optional[_builtins.bool]]]
    """
    Function local directory access permission. After mounting a TOS bucket, whether the function's local mount directory is read-only. true: read-only; false (default): read/write
    """

@pulumi.input_type
class FunctionTosMountConfigMountPointArgs:
    def __init__(__self__, *,
                 bucket_name: pulumi.Input[Optional[_builtins.str]] = None,
                 bucket_path: pulumi.Input[Optional[_builtins.str]] = None,
                 endpoint: pulumi.Input[Optional[_builtins.str]] = None,
                 local_mount_path: pulumi.Input[Optional[_builtins.str]] = None,
                 read_only: pulumi.Input[Optional[_builtins.bool]] = None):
        """
        :param pulumi.Input[_builtins.str] bucket_name: TOS bucket name.
        :param pulumi.Input[_builtins.str] bucket_path: Mounted TOS Bucket path.
        :param pulumi.Input[_builtins.str] endpoint: TOS access domain name.
        :param pulumi.Input[_builtins.str] local_mount_path: Function local mount directory.
        :param pulumi.Input[_builtins.bool] read_only: Function local directory access permission. After mounting a TOS bucket, whether the function's local mount directory is read-only. true: read-only; false (default): read/write
        """
        if bucket_name is not None:
            pulumi.set(__self__, "bucket_name", bucket_name)
        if bucket_path is not None:
            pulumi.set(__self__, "bucket_path", bucket_path)
        if endpoint is not None:
            pulumi.set(__self__, "endpoint", endpoint)
        if local_mount_path is not None:
            pulumi.set(__self__, "local_mount_path", local_mount_path)
        if read_only is not None:
            pulumi.set(__self__, "read_only", read_only)

    @_builtins.property
    @pulumi.getter(name="bucketName")
    def bucket_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        TOS bucket name.
        """
        return pulumi.get(self, "bucket_name")

    @bucket_name.setter
    def bucket_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "bucket_name", value)

    @_builtins.property
    @pulumi.getter(name="bucketPath")
    def bucket_path(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Mounted TOS Bucket path.
        """
        return pulumi.get(self, "bucket_path")

    @bucket_path.setter
    def bucket_path(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "bucket_path", value)

    @_builtins.property
    @pulumi.getter
    def endpoint(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        TOS access domain name.
        """
        return pulumi.get(self, "endpoint")

    @endpoint.setter
    def endpoint(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "endpoint", value)

    @_builtins.property
    @pulumi.getter(name="localMountPath")
    def local_mount_path(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Function local mount directory.
        """
        return pulumi.get(self, "local_mount_path")

    @local_mount_path.setter
    def local_mount_path(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "local_mount_path", value)

    @_builtins.property
    @pulumi.getter(name="readOnly")
    def read_only(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Function local directory access permission. After mounting a TOS bucket, whether the function's local mount directory is read-only. true: read-only; false (default): read/write
        """
        return pulumi.get(self, "read_only")

    @read_only.setter
    def read_only(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "read_only", value)


class FunctionVpcConfigArgsDict(TypedDict):
    enable_shared_internet_access: NotRequired[pulumi.Input[Optional[_builtins.bool]]]
    """
    Function public network access switch. Whether to access the public network via veFaaS VPC shared NAT gateway. Values: true: enabled, access public network via default network interface. Public outbound traffic fees apply. false: disabled, function cannot access public network via default network interface.
    """
    enable_vpc: NotRequired[pulumi.Input[Optional[_builtins.bool]]]
    """
    Whether the function enables private network access
    """
    security_group_ids: NotRequired[pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]]]
    """
    Security group ID
    """
    subnet_ids: NotRequired[pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]]]
    """
    Subnet ID
    """
    vpc_id: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    VPC ID. If the VPC ID is an empty string, it means the current VPC configuration will be deleted
    """

@pulumi.input_type
class FunctionVpcConfigArgs:
    def __init__(__self__, *,
                 enable_shared_internet_access: pulumi.Input[Optional[_builtins.bool]] = None,
                 enable_vpc: pulumi.Input[Optional[_builtins.bool]] = None,
                 security_group_ids: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]] = None,
                 subnet_ids: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]] = None,
                 vpc_id: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.bool] enable_shared_internet_access: Function public network access switch. Whether to access the public network via veFaaS VPC shared NAT gateway. Values: true: enabled, access public network via default network interface. Public outbound traffic fees apply. false: disabled, function cannot access public network via default network interface.
        :param pulumi.Input[_builtins.bool] enable_vpc: Whether the function enables private network access
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] security_group_ids: Security group ID
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] subnet_ids: Subnet ID
        :param pulumi.Input[_builtins.str] vpc_id: VPC ID. If the VPC ID is an empty string, it means the current VPC configuration will be deleted
        """
        if enable_shared_internet_access is not None:
            pulumi.set(__self__, "enable_shared_internet_access", enable_shared_internet_access)
        if enable_vpc is not None:
            pulumi.set(__self__, "enable_vpc", enable_vpc)
        if security_group_ids is not None:
            pulumi.set(__self__, "security_group_ids", security_group_ids)
        if subnet_ids is not None:
            pulumi.set(__self__, "subnet_ids", subnet_ids)
        if vpc_id is not None:
            pulumi.set(__self__, "vpc_id", vpc_id)

    @_builtins.property
    @pulumi.getter(name="enableSharedInternetAccess")
    def enable_shared_internet_access(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Function public network access switch. Whether to access the public network via veFaaS VPC shared NAT gateway. Values: true: enabled, access public network via default network interface. Public outbound traffic fees apply. false: disabled, function cannot access public network via default network interface.
        """
        return pulumi.get(self, "enable_shared_internet_access")

    @enable_shared_internet_access.setter
    def enable_shared_internet_access(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "enable_shared_internet_access", value)

    @_builtins.property
    @pulumi.getter(name="enableVpc")
    def enable_vpc(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Whether the function enables private network access
        """
        return pulumi.get(self, "enable_vpc")

    @enable_vpc.setter
    def enable_vpc(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "enable_vpc", value)

    @_builtins.property
    @pulumi.getter(name="securityGroupIds")
    def security_group_ids(self) -> pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]]:
        """
        Security group ID
        """
        return pulumi.get(self, "security_group_ids")

    @security_group_ids.setter
    def security_group_ids(self, value: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]]):
        pulumi.set(self, "security_group_ids", value)

    @_builtins.property
    @pulumi.getter(name="subnetIds")
    def subnet_ids(self) -> pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]]:
        """
        Subnet ID
        """
        return pulumi.get(self, "subnet_ids")

    @subnet_ids.setter
    def subnet_ids(self, value: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]]):
        pulumi.set(self, "subnet_ids", value)

    @_builtins.property
    @pulumi.getter(name="vpcId")
    def vpc_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        VPC ID. If the VPC ID is an empty string, it means the current VPC configuration will be deleted
        """
        return pulumi.get(self, "vpc_id")

    @vpc_id.setter
    def vpc_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "vpc_id", value)


class KafkaTriggerKafkaCredentialsArgsDict(TypedDict):
    mechanism: pulumi.Input[_builtins.str]
    """
    Kafka authentication mechanism. Options: PLAIN, SCRAM-SHA-256.
    """
    password: pulumi.Input[_builtins.str]
    """
    SASL/PLAIN password set when creating the Kafka instance.
    """
    username: pulumi.Input[_builtins.str]
    """
    SASL/PLAIN username set when creating the Kafka instance.
    """

@pulumi.input_type
class KafkaTriggerKafkaCredentialsArgs:
    def __init__(__self__, *,
                 mechanism: pulumi.Input[_builtins.str],
                 password: pulumi.Input[_builtins.str],
                 username: pulumi.Input[_builtins.str]):
        """
        :param pulumi.Input[_builtins.str] mechanism: Kafka authentication mechanism. Options: PLAIN, SCRAM-SHA-256.
        :param pulumi.Input[_builtins.str] password: SASL/PLAIN password set when creating the Kafka instance.
        :param pulumi.Input[_builtins.str] username: SASL/PLAIN username set when creating the Kafka instance.
        """
        pulumi.set(__self__, "mechanism", mechanism)
        pulumi.set(__self__, "password", password)
        pulumi.set(__self__, "username", username)

    @_builtins.property
    @pulumi.getter
    def mechanism(self) -> pulumi.Input[_builtins.str]:
        """
        Kafka authentication mechanism. Options: PLAIN, SCRAM-SHA-256.
        """
        return pulumi.get(self, "mechanism")

    @mechanism.setter
    def mechanism(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "mechanism", value)

    @_builtins.property
    @pulumi.getter
    def password(self) -> pulumi.Input[_builtins.str]:
        """
        SASL/PLAIN password set when creating the Kafka instance.
        """
        return pulumi.get(self, "password")

    @password.setter
    def password(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "password", value)

    @_builtins.property
    @pulumi.getter
    def username(self) -> pulumi.Input[_builtins.str]:
        """
        SASL/PLAIN username set when creating the Kafka instance.
        """
        return pulumi.get(self, "username")

    @username.setter
    def username(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "username", value)


class SandboxEnvArgsDict(TypedDict):
    key: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Environment variable key
    """
    value: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Environment variable value
    """

@pulumi.input_type
class SandboxEnvArgs:
    def __init__(__self__, *,
                 key: pulumi.Input[Optional[_builtins.str]] = None,
                 value: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.str] key: Environment variable key
        :param pulumi.Input[_builtins.str] value: Environment variable value
        """
        if key is not None:
            pulumi.set(__self__, "key", key)
        if value is not None:
            pulumi.set(__self__, "value", value)

    @_builtins.property
    @pulumi.getter
    def key(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Environment variable key
        """
        return pulumi.get(self, "key")

    @key.setter
    def key(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "key", value)

    @_builtins.property
    @pulumi.getter
    def value(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Environment variable value
        """
        return pulumi.get(self, "value")

    @value.setter
    def value(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "value", value)


class SandboxInstanceImageInfoArgsDict(TypedDict):
    command: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Startup command for the sandbox instance program. To specify a script file, use an absolute path and ensure the script has executable permissions
    """
    image: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Preheated image address used by the sandbox instance
    """
    image_id: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Pre-warmed image ID used by the sandbox instance.
    """
    port: NotRequired[pulumi.Input[Optional[_builtins.int]]]
    """
    Listening port of the sandbox instance image.
    """

@pulumi.input_type
class SandboxInstanceImageInfoArgs:
    def __init__(__self__, *,
                 command: pulumi.Input[Optional[_builtins.str]] = None,
                 image: pulumi.Input[Optional[_builtins.str]] = None,
                 image_id: pulumi.Input[Optional[_builtins.str]] = None,
                 port: pulumi.Input[Optional[_builtins.int]] = None):
        """
        :param pulumi.Input[_builtins.str] command: Startup command for the sandbox instance program. To specify a script file, use an absolute path and ensure the script has executable permissions
        :param pulumi.Input[_builtins.str] image: Preheated image address used by the sandbox instance
        :param pulumi.Input[_builtins.str] image_id: Pre-warmed image ID used by the sandbox instance.
        :param pulumi.Input[_builtins.int] port: Listening port of the sandbox instance image.
        """
        if command is not None:
            pulumi.set(__self__, "command", command)
        if image is not None:
            pulumi.set(__self__, "image", image)
        if image_id is not None:
            pulumi.set(__self__, "image_id", image_id)
        if port is not None:
            pulumi.set(__self__, "port", port)

    @_builtins.property
    @pulumi.getter
    def command(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Startup command for the sandbox instance program. To specify a script file, use an absolute path and ensure the script has executable permissions
        """
        return pulumi.get(self, "command")

    @command.setter
    def command(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "command", value)

    @_builtins.property
    @pulumi.getter
    def image(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Preheated image address used by the sandbox instance
        """
        return pulumi.get(self, "image")

    @image.setter
    def image(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "image", value)

    @_builtins.property
    @pulumi.getter(name="imageId")
    def image_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Pre-warmed image ID used by the sandbox instance.
        """
        return pulumi.get(self, "image_id")

    @image_id.setter
    def image_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "image_id", value)

    @_builtins.property
    @pulumi.getter
    def port(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Listening port of the sandbox instance image.
        """
        return pulumi.get(self, "port")

    @port.setter
    def port(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "port", value)


class SandboxInstanceTosMountConfigArgsDict(TypedDict):
    enable: NotRequired[pulumi.Input[Optional[_builtins.bool]]]
    """
    Whether instance-level TOS mount is enabled for the sandbox instance. Parameter description: true: yes, false: no
    """
    tos_mount_points: NotRequired[pulumi.Input[Optional[Sequence[pulumi.Input['SandboxInstanceTosMountConfigTosMountPointArgsDict']]]]]
    """
    Detailed TOS mount directory information for sandbox instances with instance-level TOS mount enabled
    Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
    """

@pulumi.input_type
class SandboxInstanceTosMountConfigArgs:
    def __init__(__self__, *,
                 enable: pulumi.Input[Optional[_builtins.bool]] = None,
                 tos_mount_points: pulumi.Input[Optional[Sequence[pulumi.Input['SandboxInstanceTosMountConfigTosMountPointArgs']]]] = None):
        """
        :param pulumi.Input[_builtins.bool] enable: Whether instance-level TOS mount is enabled for the sandbox instance. Parameter description: true: yes, false: no
        :param pulumi.Input[Sequence[pulumi.Input['SandboxInstanceTosMountConfigTosMountPointArgs']]] tos_mount_points: Detailed TOS mount directory information for sandbox instances with instance-level TOS mount enabled
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        if enable is not None:
            pulumi.set(__self__, "enable", enable)
        if tos_mount_points is not None:
            pulumi.set(__self__, "tos_mount_points", tos_mount_points)

    @_builtins.property
    @pulumi.getter
    def enable(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Whether instance-level TOS mount is enabled for the sandbox instance. Parameter description: true: yes, false: no
        """
        return pulumi.get(self, "enable")

    @enable.setter
    def enable(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "enable", value)

    @_builtins.property
    @pulumi.getter(name="tosMountPoints")
    def tos_mount_points(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['SandboxInstanceTosMountConfigTosMountPointArgs']]]]:
        """
        Detailed TOS mount directory information for sandbox instances with instance-level TOS mount enabled
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "tos_mount_points")

    @tos_mount_points.setter
    def tos_mount_points(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['SandboxInstanceTosMountConfigTosMountPointArgs']]]]):
        pulumi.set(self, "tos_mount_points", value)


class SandboxInstanceTosMountConfigTosMountPointArgsDict(TypedDict):
    bucket_path: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    TOS remote directory mounted by the sandbox instance
    """
    local_mount_path: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Local directory of the TOS bucket mounted by the sandbox instance. If this directory is the local directory configured for TOS storage mount in the sandbox application, the system modifies the corresponding TOS BucketPath based on the specified local directory
    """

@pulumi.input_type
class SandboxInstanceTosMountConfigTosMountPointArgs:
    def __init__(__self__, *,
                 bucket_path: pulumi.Input[Optional[_builtins.str]] = None,
                 local_mount_path: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.str] bucket_path: TOS remote directory mounted by the sandbox instance
        :param pulumi.Input[_builtins.str] local_mount_path: Local directory of the TOS bucket mounted by the sandbox instance. If this directory is the local directory configured for TOS storage mount in the sandbox application, the system modifies the corresponding TOS BucketPath based on the specified local directory
        """
        if bucket_path is not None:
            pulumi.set(__self__, "bucket_path", bucket_path)
        if local_mount_path is not None:
            pulumi.set(__self__, "local_mount_path", local_mount_path)

    @_builtins.property
    @pulumi.getter(name="bucketPath")
    def bucket_path(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        TOS remote directory mounted by the sandbox instance
        """
        return pulumi.get(self, "bucket_path")

    @bucket_path.setter
    def bucket_path(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "bucket_path", value)

    @_builtins.property
    @pulumi.getter(name="localMountPath")
    def local_mount_path(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Local directory of the TOS bucket mounted by the sandbox instance. If this directory is the local directory configured for TOS storage mount in the sandbox application, the system modifies the corresponding TOS BucketPath based on the specified local directory
        """
        return pulumi.get(self, "local_mount_path")

    @local_mount_path.setter
    def local_mount_path(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "local_mount_path", value)


class SandboxMetadataArgsDict(TypedDict):
    key: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Tag key
    """
    value: NotRequired[pulumi.Input[Optional[_builtins.str]]]
    """
    Tag value
    """

@pulumi.input_type
class SandboxMetadataArgs:
    def __init__(__self__, *,
                 key: pulumi.Input[Optional[_builtins.str]] = None,
                 value: pulumi.Input[Optional[_builtins.str]] = None):
        """
        :param pulumi.Input[_builtins.str] key: Tag key
        :param pulumi.Input[_builtins.str] value: Tag value
        """
        if key is not None:
            pulumi.set(__self__, "key", key)
        if value is not None:
            pulumi.set(__self__, "value", value)

    @_builtins.property
    @pulumi.getter
    def key(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Tag key
        """
        return pulumi.get(self, "key")

    @key.setter
    def key(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "key", value)

    @_builtins.property
    @pulumi.getter
    def value(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Tag value
        """
        return pulumi.get(self, "value")

    @value.setter
    def value(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "value", value)


