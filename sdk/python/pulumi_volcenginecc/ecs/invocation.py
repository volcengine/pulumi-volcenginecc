# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = ['InvocationArgs', 'Invocation']

@pulumi.input_type
class InvocationArgs:
    def __init__(__self__, *,
                 command_id: pulumi.Input[_builtins.str],
                 instance_ids: pulumi.Input[Sequence[pulumi.Input[_builtins.str]]],
                 invocation_name: pulumi.Input[_builtins.str],
                 frequency: pulumi.Input[Optional[_builtins.str]] = None,
                 invocation_description: pulumi.Input[Optional[_builtins.str]] = None,
                 launch_time: pulumi.Input[Optional[_builtins.str]] = None,
                 parameters: pulumi.Input[Optional[_builtins.str]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 recurrence_end_time: pulumi.Input[Optional[_builtins.str]] = None,
                 repeat_mode: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input['InvocationTagArgs']]]] = None,
                 timeout: pulumi.Input[Optional[_builtins.int]] = None,
                 username: pulumi.Input[Optional[_builtins.str]] = None,
                 windows_password: pulumi.Input[Optional[_builtins.str]] = None,
                 working_dir: pulumi.Input[Optional[_builtins.str]] = None):
        """
        The set of arguments for constructing a Invocation resource.

        :param pulumi.Input[_builtins.str] command_id: Command ID.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] instance_ids: Instance ID list, supports up to 200 IDs.
        :param pulumi.Input[_builtins.str] invocation_name: Job name. Maximum 64 characters. No restriction on special characters. Job names can be duplicated within the same region.
        :param pulumi.Input[_builtins.str] frequency: Execution frequency. This parameter is required when RepeatMode=Rate. Format: \\n\\n\\n\\n. Interval units supported: minutes (5m–60m), hours (1h–24h), and days (1d–7d). The interval is based on a fixed frequency and is independent of the actual job execution time. After job creation, it executes once based on LaunchTime, and the next execution time is determined by the interval setting.
        :param pulumi.Input[_builtins.str] invocation_description: Job description. Defaults to an empty string and must not exceed 256 characters.
        :param pulumi.Input[_builtins.str] launch_time: Execution time.
        :param pulumi.Input[_builtins.str] parameters: When the command includes custom parameters, use this parameter to pass custom parameter key-value pairs. The number of custom parameters ranges from 0 to 60. Key cannot be an empty string and supports up to 64 characters. Value can be an empty string. The combined length of custom parameters and original command content after Base64 encoding must not exceed 16 KB. The set of custom parameter names must be a subset of the parameter set defined when creating the command. Default values are used for parameters not provided.
        :param pulumi.Input[_builtins.str] project_name: Project to which the resource belongs; each resource can belong to only one project.
        :param pulumi.Input[_builtins.str] recurrence_end_time: Cycle end time, applicable only to periodic tasks (Rate).
        :param pulumi.Input[_builtins.str] repeat_mode: Set the command execution mode. Once: Default, executes the command immediately. Rate: Executes the command periodically; specify the execution cycle using the Frequency parameter and Rate expression. Fixed: Executes the command at a scheduled time; specify the execution time using the LaunchTime parameter.
        :param pulumi.Input[Sequence[pulumi.Input['InvocationTagArgs']]] tags: Tag key-value pair.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.int] timeout: Maximum timeout for command execution in ECS instances, in seconds. Default: 60. Optional range: 30–86400.
        :param pulumi.Input[_builtins.str] username: User name for executing the command in the ECS instance.
        :param pulumi.Input[_builtins.str] windows_password: Password for custom Windows user.
        :param pulumi.Input[_builtins.str] working_dir: Directory where the created command runs in the ECS instance.
        """
        pulumi.set(__self__, "command_id", command_id)
        pulumi.set(__self__, "instance_ids", instance_ids)
        pulumi.set(__self__, "invocation_name", invocation_name)
        if frequency is not None:
            pulumi.set(__self__, "frequency", frequency)
        if invocation_description is not None:
            pulumi.set(__self__, "invocation_description", invocation_description)
        if launch_time is not None:
            pulumi.set(__self__, "launch_time", launch_time)
        if parameters is not None:
            pulumi.set(__self__, "parameters", parameters)
        if project_name is not None:
            pulumi.set(__self__, "project_name", project_name)
        if recurrence_end_time is not None:
            pulumi.set(__self__, "recurrence_end_time", recurrence_end_time)
        if repeat_mode is not None:
            pulumi.set(__self__, "repeat_mode", repeat_mode)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)
        if timeout is not None:
            pulumi.set(__self__, "timeout", timeout)
        if username is not None:
            pulumi.set(__self__, "username", username)
        if windows_password is not None:
            pulumi.set(__self__, "windows_password", windows_password)
        if working_dir is not None:
            pulumi.set(__self__, "working_dir", working_dir)

    @_builtins.property
    @pulumi.getter(name="commandId")
    def command_id(self) -> pulumi.Input[_builtins.str]:
        """
        Command ID.
        """
        return pulumi.get(self, "command_id")

    @command_id.setter
    def command_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "command_id", value)

    @_builtins.property
    @pulumi.getter(name="instanceIds")
    def instance_ids(self) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]]]:
        """
        Instance ID list, supports up to 200 IDs.
        """
        return pulumi.get(self, "instance_ids")

    @instance_ids.setter
    def instance_ids(self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]]]):
        pulumi.set(self, "instance_ids", value)

    @_builtins.property
    @pulumi.getter(name="invocationName")
    def invocation_name(self) -> pulumi.Input[_builtins.str]:
        """
        Job name. Maximum 64 characters. No restriction on special characters. Job names can be duplicated within the same region.
        """
        return pulumi.get(self, "invocation_name")

    @invocation_name.setter
    def invocation_name(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "invocation_name", value)

    @_builtins.property
    @pulumi.getter
    def frequency(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Execution frequency. This parameter is required when RepeatMode=Rate. Format: \\n\\n\\n\\n. Interval units supported: minutes (5m–60m), hours (1h–24h), and days (1d–7d). The interval is based on a fixed frequency and is independent of the actual job execution time. After job creation, it executes once based on LaunchTime, and the next execution time is determined by the interval setting.
        """
        return pulumi.get(self, "frequency")

    @frequency.setter
    def frequency(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "frequency", value)

    @_builtins.property
    @pulumi.getter(name="invocationDescription")
    def invocation_description(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Job description. Defaults to an empty string and must not exceed 256 characters.
        """
        return pulumi.get(self, "invocation_description")

    @invocation_description.setter
    def invocation_description(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "invocation_description", value)

    @_builtins.property
    @pulumi.getter(name="launchTime")
    def launch_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Execution time.
        """
        return pulumi.get(self, "launch_time")

    @launch_time.setter
    def launch_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "launch_time", value)

    @_builtins.property
    @pulumi.getter
    def parameters(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        When the command includes custom parameters, use this parameter to pass custom parameter key-value pairs. The number of custom parameters ranges from 0 to 60. Key cannot be an empty string and supports up to 64 characters. Value can be an empty string. The combined length of custom parameters and original command content after Base64 encoding must not exceed 16 KB. The set of custom parameter names must be a subset of the parameter set defined when creating the command. Default values are used for parameters not provided.
        """
        return pulumi.get(self, "parameters")

    @parameters.setter
    def parameters(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "parameters", value)

    @_builtins.property
    @pulumi.getter(name="projectName")
    def project_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Project to which the resource belongs; each resource can belong to only one project.
        """
        return pulumi.get(self, "project_name")

    @project_name.setter
    def project_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "project_name", value)

    @_builtins.property
    @pulumi.getter(name="recurrenceEndTime")
    def recurrence_end_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Cycle end time, applicable only to periodic tasks (Rate).
        """
        return pulumi.get(self, "recurrence_end_time")

    @recurrence_end_time.setter
    def recurrence_end_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "recurrence_end_time", value)

    @_builtins.property
    @pulumi.getter(name="repeatMode")
    def repeat_mode(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Set the command execution mode. Once: Default, executes the command immediately. Rate: Executes the command periodically; specify the execution cycle using the Frequency parameter and Rate expression. Fixed: Executes the command at a scheduled time; specify the execution time using the LaunchTime parameter.
        """
        return pulumi.get(self, "repeat_mode")

    @repeat_mode.setter
    def repeat_mode(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "repeat_mode", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['InvocationTagArgs']]]]:
        """
        Tag key-value pair.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['InvocationTagArgs']]]]):
        pulumi.set(self, "tags", value)

    @_builtins.property
    @pulumi.getter
    def timeout(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Maximum timeout for command execution in ECS instances, in seconds. Default: 60. Optional range: 30–86400.
        """
        return pulumi.get(self, "timeout")

    @timeout.setter
    def timeout(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "timeout", value)

    @_builtins.property
    @pulumi.getter
    def username(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        User name for executing the command in the ECS instance.
        """
        return pulumi.get(self, "username")

    @username.setter
    def username(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "username", value)

    @_builtins.property
    @pulumi.getter(name="windowsPassword")
    def windows_password(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Password for custom Windows user.
        """
        return pulumi.get(self, "windows_password")

    @windows_password.setter
    def windows_password(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "windows_password", value)

    @_builtins.property
    @pulumi.getter(name="workingDir")
    def working_dir(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Directory where the created command runs in the ECS instance.
        """
        return pulumi.get(self, "working_dir")

    @working_dir.setter
    def working_dir(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "working_dir", value)


@pulumi.input_type
class _InvocationState:
    def __init__(__self__, *,
                 command_content: pulumi.Input[Optional[_builtins.str]] = None,
                 command_description: pulumi.Input[Optional[_builtins.str]] = None,
                 command_id: pulumi.Input[Optional[_builtins.str]] = None,
                 command_name: pulumi.Input[Optional[_builtins.str]] = None,
                 command_provider: pulumi.Input[Optional[_builtins.str]] = None,
                 command_type: pulumi.Input[Optional[_builtins.str]] = None,
                 enable_parameter: pulumi.Input[Optional[_builtins.bool]] = None,
                 end_time: pulumi.Input[Optional[_builtins.str]] = None,
                 frequency: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_ids: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]] = None,
                 instance_number: pulumi.Input[Optional[_builtins.int]] = None,
                 invocation_description: pulumi.Input[Optional[_builtins.str]] = None,
                 invocation_id: pulumi.Input[Optional[_builtins.str]] = None,
                 invocation_name: pulumi.Input[Optional[_builtins.str]] = None,
                 invocation_results: pulumi.Input[Optional[Sequence[pulumi.Input['InvocationInvocationResultArgs']]]] = None,
                 invocation_status: pulumi.Input[Optional[_builtins.str]] = None,
                 launch_time: pulumi.Input[Optional[_builtins.str]] = None,
                 parameter_definitions: pulumi.Input[Optional[Sequence[pulumi.Input['InvocationParameterDefinitionArgs']]]] = None,
                 parameters: pulumi.Input[Optional[_builtins.str]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 recurrence_end_time: pulumi.Input[Optional[_builtins.str]] = None,
                 repeat_mode: pulumi.Input[Optional[_builtins.str]] = None,
                 start_time: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input['InvocationTagArgs']]]] = None,
                 timeout: pulumi.Input[Optional[_builtins.int]] = None,
                 username: pulumi.Input[Optional[_builtins.str]] = None,
                 windows_password: pulumi.Input[Optional[_builtins.str]] = None,
                 working_dir: pulumi.Input[Optional[_builtins.str]] = None):
        """
        Input properties used for looking up and filtering Invocation resources.

        :param pulumi.Input[_builtins.str] command_content: Command content.
        :param pulumi.Input[_builtins.str] command_description: Description of the invoked command.
        :param pulumi.Input[_builtins.str] command_id: Command ID.
        :param pulumi.Input[_builtins.str] command_name: Command name when the task is triggered.
        :param pulumi.Input[_builtins.str] command_provider: Provider of the invoked command.
        :param pulumi.Input[_builtins.str] command_type: Command type triggered by the job.
        :param pulumi.Input[_builtins.bool] enable_parameter: Whether the invoked command uses custom parameters. true: enabled. false: not enabled.
        :param pulumi.Input[_builtins.str] end_time: Task end time.
        :param pulumi.Input[_builtins.str] frequency: Execution frequency. This parameter is required when RepeatMode=Rate. Format: \\n\\n\\n\\n. Interval units supported: minutes (5m–60m), hours (1h–24h), and days (1d–7d). The interval is based on a fixed frequency and is independent of the actual job execution time. After job creation, it executes once based on LaunchTime, and the next execution time is determined by the interval setting.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] instance_ids: Instance ID list, supports up to 200 IDs.
        :param pulumi.Input[_builtins.int] instance_number: Number of instances executed.
        :param pulumi.Input[_builtins.str] invocation_description: Job description. Defaults to an empty string and must not exceed 256 characters.
        :param pulumi.Input[_builtins.str] invocation_id: Task execution ID.
        :param pulumi.Input[_builtins.str] invocation_name: Job name. Maximum 64 characters. No restriction on special characters. Job names can be duplicated within the same region.
        :param pulumi.Input[Sequence[pulumi.Input['InvocationInvocationResultArgs']]] invocation_results: Result of command execution on a single instance.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] invocation_status: Overall execution status of the command.
        :param pulumi.Input[_builtins.str] launch_time: Execution time.
        :param pulumi.Input[Sequence[pulumi.Input['InvocationParameterDefinitionArgs']]] parameter_definitions: Custom parameter definition for the command triggered by the task.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] parameters: When the command includes custom parameters, use this parameter to pass custom parameter key-value pairs. The number of custom parameters ranges from 0 to 60. Key cannot be an empty string and supports up to 64 characters. Value can be an empty string. The combined length of custom parameters and original command content after Base64 encoding must not exceed 16 KB. The set of custom parameter names must be a subset of the parameter set defined when creating the command. Default values are used for parameters not provided.
        :param pulumi.Input[_builtins.str] project_name: Project to which the resource belongs; each resource can belong to only one project.
        :param pulumi.Input[_builtins.str] recurrence_end_time: Cycle end time, applicable only to periodic tasks (Rate).
        :param pulumi.Input[_builtins.str] repeat_mode: Set the command execution mode. Once: Default, executes the command immediately. Rate: Executes the command periodically; specify the execution cycle using the Frequency parameter and Rate expression. Fixed: Executes the command at a scheduled time; specify the execution time using the LaunchTime parameter.
        :param pulumi.Input[_builtins.str] start_time: Task start time.
        :param pulumi.Input[Sequence[pulumi.Input['InvocationTagArgs']]] tags: Tag key-value pair.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.int] timeout: Maximum timeout for command execution in ECS instances, in seconds. Default: 60. Optional range: 30–86400.
        :param pulumi.Input[_builtins.str] username: User name for executing the command in the ECS instance.
        :param pulumi.Input[_builtins.str] windows_password: Password for custom Windows user.
        :param pulumi.Input[_builtins.str] working_dir: Directory where the created command runs in the ECS instance.
        """
        if command_content is not None:
            pulumi.set(__self__, "command_content", command_content)
        if command_description is not None:
            pulumi.set(__self__, "command_description", command_description)
        if command_id is not None:
            pulumi.set(__self__, "command_id", command_id)
        if command_name is not None:
            pulumi.set(__self__, "command_name", command_name)
        if command_provider is not None:
            pulumi.set(__self__, "command_provider", command_provider)
        if command_type is not None:
            pulumi.set(__self__, "command_type", command_type)
        if enable_parameter is not None:
            pulumi.set(__self__, "enable_parameter", enable_parameter)
        if end_time is not None:
            pulumi.set(__self__, "end_time", end_time)
        if frequency is not None:
            pulumi.set(__self__, "frequency", frequency)
        if instance_ids is not None:
            pulumi.set(__self__, "instance_ids", instance_ids)
        if instance_number is not None:
            pulumi.set(__self__, "instance_number", instance_number)
        if invocation_description is not None:
            pulumi.set(__self__, "invocation_description", invocation_description)
        if invocation_id is not None:
            pulumi.set(__self__, "invocation_id", invocation_id)
        if invocation_name is not None:
            pulumi.set(__self__, "invocation_name", invocation_name)
        if invocation_results is not None:
            pulumi.set(__self__, "invocation_results", invocation_results)
        if invocation_status is not None:
            pulumi.set(__self__, "invocation_status", invocation_status)
        if launch_time is not None:
            pulumi.set(__self__, "launch_time", launch_time)
        if parameter_definitions is not None:
            pulumi.set(__self__, "parameter_definitions", parameter_definitions)
        if parameters is not None:
            pulumi.set(__self__, "parameters", parameters)
        if project_name is not None:
            pulumi.set(__self__, "project_name", project_name)
        if recurrence_end_time is not None:
            pulumi.set(__self__, "recurrence_end_time", recurrence_end_time)
        if repeat_mode is not None:
            pulumi.set(__self__, "repeat_mode", repeat_mode)
        if start_time is not None:
            pulumi.set(__self__, "start_time", start_time)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)
        if timeout is not None:
            pulumi.set(__self__, "timeout", timeout)
        if username is not None:
            pulumi.set(__self__, "username", username)
        if windows_password is not None:
            pulumi.set(__self__, "windows_password", windows_password)
        if working_dir is not None:
            pulumi.set(__self__, "working_dir", working_dir)

    @_builtins.property
    @pulumi.getter(name="commandContent")
    def command_content(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Command content.
        """
        return pulumi.get(self, "command_content")

    @command_content.setter
    def command_content(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "command_content", value)

    @_builtins.property
    @pulumi.getter(name="commandDescription")
    def command_description(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Description of the invoked command.
        """
        return pulumi.get(self, "command_description")

    @command_description.setter
    def command_description(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "command_description", value)

    @_builtins.property
    @pulumi.getter(name="commandId")
    def command_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Command ID.
        """
        return pulumi.get(self, "command_id")

    @command_id.setter
    def command_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "command_id", value)

    @_builtins.property
    @pulumi.getter(name="commandName")
    def command_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Command name when the task is triggered.
        """
        return pulumi.get(self, "command_name")

    @command_name.setter
    def command_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "command_name", value)

    @_builtins.property
    @pulumi.getter(name="commandProvider")
    def command_provider(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Provider of the invoked command.
        """
        return pulumi.get(self, "command_provider")

    @command_provider.setter
    def command_provider(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "command_provider", value)

    @_builtins.property
    @pulumi.getter(name="commandType")
    def command_type(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Command type triggered by the job.
        """
        return pulumi.get(self, "command_type")

    @command_type.setter
    def command_type(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "command_type", value)

    @_builtins.property
    @pulumi.getter(name="enableParameter")
    def enable_parameter(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Whether the invoked command uses custom parameters. true: enabled. false: not enabled.
        """
        return pulumi.get(self, "enable_parameter")

    @enable_parameter.setter
    def enable_parameter(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "enable_parameter", value)

    @_builtins.property
    @pulumi.getter(name="endTime")
    def end_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Task end time.
        """
        return pulumi.get(self, "end_time")

    @end_time.setter
    def end_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "end_time", value)

    @_builtins.property
    @pulumi.getter
    def frequency(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Execution frequency. This parameter is required when RepeatMode=Rate. Format: \\n\\n\\n\\n. Interval units supported: minutes (5m–60m), hours (1h–24h), and days (1d–7d). The interval is based on a fixed frequency and is independent of the actual job execution time. After job creation, it executes once based on LaunchTime, and the next execution time is determined by the interval setting.
        """
        return pulumi.get(self, "frequency")

    @frequency.setter
    def frequency(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "frequency", value)

    @_builtins.property
    @pulumi.getter(name="instanceIds")
    def instance_ids(self) -> pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]]:
        """
        Instance ID list, supports up to 200 IDs.
        """
        return pulumi.get(self, "instance_ids")

    @instance_ids.setter
    def instance_ids(self, value: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]]):
        pulumi.set(self, "instance_ids", value)

    @_builtins.property
    @pulumi.getter(name="instanceNumber")
    def instance_number(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Number of instances executed.
        """
        return pulumi.get(self, "instance_number")

    @instance_number.setter
    def instance_number(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "instance_number", value)

    @_builtins.property
    @pulumi.getter(name="invocationDescription")
    def invocation_description(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Job description. Defaults to an empty string and must not exceed 256 characters.
        """
        return pulumi.get(self, "invocation_description")

    @invocation_description.setter
    def invocation_description(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "invocation_description", value)

    @_builtins.property
    @pulumi.getter(name="invocationId")
    def invocation_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Task execution ID.
        """
        return pulumi.get(self, "invocation_id")

    @invocation_id.setter
    def invocation_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "invocation_id", value)

    @_builtins.property
    @pulumi.getter(name="invocationName")
    def invocation_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Job name. Maximum 64 characters. No restriction on special characters. Job names can be duplicated within the same region.
        """
        return pulumi.get(self, "invocation_name")

    @invocation_name.setter
    def invocation_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "invocation_name", value)

    @_builtins.property
    @pulumi.getter(name="invocationResults")
    def invocation_results(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['InvocationInvocationResultArgs']]]]:
        """
        Result of command execution on a single instance.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "invocation_results")

    @invocation_results.setter
    def invocation_results(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['InvocationInvocationResultArgs']]]]):
        pulumi.set(self, "invocation_results", value)

    @_builtins.property
    @pulumi.getter(name="invocationStatus")
    def invocation_status(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Overall execution status of the command.
        """
        return pulumi.get(self, "invocation_status")

    @invocation_status.setter
    def invocation_status(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "invocation_status", value)

    @_builtins.property
    @pulumi.getter(name="launchTime")
    def launch_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Execution time.
        """
        return pulumi.get(self, "launch_time")

    @launch_time.setter
    def launch_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "launch_time", value)

    @_builtins.property
    @pulumi.getter(name="parameterDefinitions")
    def parameter_definitions(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['InvocationParameterDefinitionArgs']]]]:
        """
        Custom parameter definition for the command triggered by the task.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "parameter_definitions")

    @parameter_definitions.setter
    def parameter_definitions(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['InvocationParameterDefinitionArgs']]]]):
        pulumi.set(self, "parameter_definitions", value)

    @_builtins.property
    @pulumi.getter
    def parameters(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        When the command includes custom parameters, use this parameter to pass custom parameter key-value pairs. The number of custom parameters ranges from 0 to 60. Key cannot be an empty string and supports up to 64 characters. Value can be an empty string. The combined length of custom parameters and original command content after Base64 encoding must not exceed 16 KB. The set of custom parameter names must be a subset of the parameter set defined when creating the command. Default values are used for parameters not provided.
        """
        return pulumi.get(self, "parameters")

    @parameters.setter
    def parameters(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "parameters", value)

    @_builtins.property
    @pulumi.getter(name="projectName")
    def project_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Project to which the resource belongs; each resource can belong to only one project.
        """
        return pulumi.get(self, "project_name")

    @project_name.setter
    def project_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "project_name", value)

    @_builtins.property
    @pulumi.getter(name="recurrenceEndTime")
    def recurrence_end_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Cycle end time, applicable only to periodic tasks (Rate).
        """
        return pulumi.get(self, "recurrence_end_time")

    @recurrence_end_time.setter
    def recurrence_end_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "recurrence_end_time", value)

    @_builtins.property
    @pulumi.getter(name="repeatMode")
    def repeat_mode(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Set the command execution mode. Once: Default, executes the command immediately. Rate: Executes the command periodically; specify the execution cycle using the Frequency parameter and Rate expression. Fixed: Executes the command at a scheduled time; specify the execution time using the LaunchTime parameter.
        """
        return pulumi.get(self, "repeat_mode")

    @repeat_mode.setter
    def repeat_mode(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "repeat_mode", value)

    @_builtins.property
    @pulumi.getter(name="startTime")
    def start_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Task start time.
        """
        return pulumi.get(self, "start_time")

    @start_time.setter
    def start_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "start_time", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['InvocationTagArgs']]]]:
        """
        Tag key-value pair.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['InvocationTagArgs']]]]):
        pulumi.set(self, "tags", value)

    @_builtins.property
    @pulumi.getter
    def timeout(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Maximum timeout for command execution in ECS instances, in seconds. Default: 60. Optional range: 30–86400.
        """
        return pulumi.get(self, "timeout")

    @timeout.setter
    def timeout(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "timeout", value)

    @_builtins.property
    @pulumi.getter
    def username(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        User name for executing the command in the ECS instance.
        """
        return pulumi.get(self, "username")

    @username.setter
    def username(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "username", value)

    @_builtins.property
    @pulumi.getter(name="windowsPassword")
    def windows_password(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Password for custom Windows user.
        """
        return pulumi.get(self, "windows_password")

    @windows_password.setter
    def windows_password(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "windows_password", value)

    @_builtins.property
    @pulumi.getter(name="workingDir")
    def working_dir(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Directory where the created command runs in the ECS instance.
        """
        return pulumi.get(self, "working_dir")

    @working_dir.setter
    def working_dir(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "working_dir", value)


@pulumi.type_token("volcenginecc:ecs/invocation:Invocation")
class Invocation(pulumi.CustomResource):
    @overload
    def __init__(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 command_id: pulumi.Input[Optional[_builtins.str]] = None,
                 frequency: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_ids: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]] = None,
                 invocation_description: pulumi.Input[Optional[_builtins.str]] = None,
                 invocation_name: pulumi.Input[Optional[_builtins.str]] = None,
                 launch_time: pulumi.Input[Optional[_builtins.str]] = None,
                 parameters: pulumi.Input[Optional[_builtins.str]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 recurrence_end_time: pulumi.Input[Optional[_builtins.str]] = None,
                 repeat_mode: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InvocationTagArgs', 'InvocationTagArgsDict']]]]] = None,
                 timeout: pulumi.Input[Optional[_builtins.int]] = None,
                 username: pulumi.Input[Optional[_builtins.str]] = None,
                 windows_password: pulumi.Input[Optional[_builtins.str]] = None,
                 working_dir: pulumi.Input[Optional[_builtins.str]] = None,
                 __props__=None):
        """
        Cloud Assistant is a native automated operations tool developed by Volcano Engine. You can create jobs for immediate, periodic, or scheduled execution. After creation, the Cloud Assistant client automatically starts execution according to the execution mode.

        ## Example Usage

        ```python
        import pulumi
        import pulumi_volcenginecc as volcenginecc

        invocation_demo = volcenginecc.ecs.Invocation("InvocationDemo",
            invocation_name="InvocationDemo",
            invocation_description="InvocationDemo desc",
            working_dir="/home",
            username="InvocationDemo",
            windows_password="********",
            timeout=60,
            instance_ids=["i-ye2v6l0pvkqc6inxxxxx"],
            repeat_mode="Rate",
            frequency="1h",
            launch_time="2025-08-30T11:10Z",
            recurrence_end_time="2025-08-31T11:04Z",
            project_name="default",
            tags=[{
                "key": "env",
                "value": "test",
            }],
            command_id="cmd-ye28kugp249tzrexxxxx",
            parameters="{\\"dirname\\":\\"10\\"}")
        ```

        ## Import

        ```sh
        $ pulumi import volcenginecc:ecs/invocation:Invocation example "invocation_id"
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] command_id: Command ID.
        :param pulumi.Input[_builtins.str] frequency: Execution frequency. This parameter is required when RepeatMode=Rate. Format: \\n\\n\\n\\n. Interval units supported: minutes (5m–60m), hours (1h–24h), and days (1d–7d). The interval is based on a fixed frequency and is independent of the actual job execution time. After job creation, it executes once based on LaunchTime, and the next execution time is determined by the interval setting.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] instance_ids: Instance ID list, supports up to 200 IDs.
        :param pulumi.Input[_builtins.str] invocation_description: Job description. Defaults to an empty string and must not exceed 256 characters.
        :param pulumi.Input[_builtins.str] invocation_name: Job name. Maximum 64 characters. No restriction on special characters. Job names can be duplicated within the same region.
        :param pulumi.Input[_builtins.str] launch_time: Execution time.
        :param pulumi.Input[_builtins.str] parameters: When the command includes custom parameters, use this parameter to pass custom parameter key-value pairs. The number of custom parameters ranges from 0 to 60. Key cannot be an empty string and supports up to 64 characters. Value can be an empty string. The combined length of custom parameters and original command content after Base64 encoding must not exceed 16 KB. The set of custom parameter names must be a subset of the parameter set defined when creating the command. Default values are used for parameters not provided.
        :param pulumi.Input[_builtins.str] project_name: Project to which the resource belongs; each resource can belong to only one project.
        :param pulumi.Input[_builtins.str] recurrence_end_time: Cycle end time, applicable only to periodic tasks (Rate).
        :param pulumi.Input[_builtins.str] repeat_mode: Set the command execution mode. Once: Default, executes the command immediately. Rate: Executes the command periodically; specify the execution cycle using the Frequency parameter and Rate expression. Fixed: Executes the command at a scheduled time; specify the execution time using the LaunchTime parameter.
        :param pulumi.Input[Sequence[pulumi.Input[Union['InvocationTagArgs', 'InvocationTagArgsDict']]]] tags: Tag key-value pair.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.int] timeout: Maximum timeout for command execution in ECS instances, in seconds. Default: 60. Optional range: 30–86400.
        :param pulumi.Input[_builtins.str] username: User name for executing the command in the ECS instance.
        :param pulumi.Input[_builtins.str] windows_password: Password for custom Windows user.
        :param pulumi.Input[_builtins.str] working_dir: Directory where the created command runs in the ECS instance.
        """
        ...
    @overload
    def __init__(__self__,
                 resource_name: str,
                 args: InvocationArgs,
                 opts: Optional[pulumi.ResourceOptions] = None):
        """
        Cloud Assistant is a native automated operations tool developed by Volcano Engine. You can create jobs for immediate, periodic, or scheduled execution. After creation, the Cloud Assistant client automatically starts execution according to the execution mode.

        ## Example Usage

        ```python
        import pulumi
        import pulumi_volcenginecc as volcenginecc

        invocation_demo = volcenginecc.ecs.Invocation("InvocationDemo",
            invocation_name="InvocationDemo",
            invocation_description="InvocationDemo desc",
            working_dir="/home",
            username="InvocationDemo",
            windows_password="********",
            timeout=60,
            instance_ids=["i-ye2v6l0pvkqc6inxxxxx"],
            repeat_mode="Rate",
            frequency="1h",
            launch_time="2025-08-30T11:10Z",
            recurrence_end_time="2025-08-31T11:04Z",
            project_name="default",
            tags=[{
                "key": "env",
                "value": "test",
            }],
            command_id="cmd-ye28kugp249tzrexxxxx",
            parameters="{\\"dirname\\":\\"10\\"}")
        ```

        ## Import

        ```sh
        $ pulumi import volcenginecc:ecs/invocation:Invocation example "invocation_id"
        ```


        :param str resource_name: The name of the resource.
        :param InvocationArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        ...
    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(InvocationArgs, pulumi.ResourceOptions, *args, **kwargs)
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 command_id: pulumi.Input[Optional[_builtins.str]] = None,
                 frequency: pulumi.Input[Optional[_builtins.str]] = None,
                 instance_ids: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]] = None,
                 invocation_description: pulumi.Input[Optional[_builtins.str]] = None,
                 invocation_name: pulumi.Input[Optional[_builtins.str]] = None,
                 launch_time: pulumi.Input[Optional[_builtins.str]] = None,
                 parameters: pulumi.Input[Optional[_builtins.str]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 recurrence_end_time: pulumi.Input[Optional[_builtins.str]] = None,
                 repeat_mode: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InvocationTagArgs', 'InvocationTagArgsDict']]]]] = None,
                 timeout: pulumi.Input[Optional[_builtins.int]] = None,
                 username: pulumi.Input[Optional[_builtins.str]] = None,
                 windows_password: pulumi.Input[Optional[_builtins.str]] = None,
                 working_dir: pulumi.Input[Optional[_builtins.str]] = None,
                 __props__=None):
        opts = pulumi.ResourceOptions.merge(_utilities.get_resource_opts_defaults(), opts)
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError('Expected resource options to be a ResourceOptions instance')
        if opts.id is None:
            if __props__ is not None:
                raise TypeError('__props__ is only valid when passed in combination with a valid opts.id to get an existing resource')
            __props__ = InvocationArgs.__new__(InvocationArgs)

            if command_id is None and not opts.urn:
                raise TypeError("Missing required property 'command_id'")
            __props__.__dict__["command_id"] = command_id
            __props__.__dict__["frequency"] = frequency
            if instance_ids is None and not opts.urn:
                raise TypeError("Missing required property 'instance_ids'")
            __props__.__dict__["instance_ids"] = instance_ids
            __props__.__dict__["invocation_description"] = invocation_description
            if invocation_name is None and not opts.urn:
                raise TypeError("Missing required property 'invocation_name'")
            __props__.__dict__["invocation_name"] = invocation_name
            __props__.__dict__["launch_time"] = launch_time
            __props__.__dict__["parameters"] = parameters
            __props__.__dict__["project_name"] = project_name
            __props__.__dict__["recurrence_end_time"] = recurrence_end_time
            __props__.__dict__["repeat_mode"] = repeat_mode
            __props__.__dict__["tags"] = tags
            __props__.__dict__["timeout"] = timeout
            __props__.__dict__["username"] = username
            __props__.__dict__["windows_password"] = windows_password
            __props__.__dict__["working_dir"] = working_dir
            __props__.__dict__["command_content"] = None
            __props__.__dict__["command_description"] = None
            __props__.__dict__["command_name"] = None
            __props__.__dict__["command_provider"] = None
            __props__.__dict__["command_type"] = None
            __props__.__dict__["enable_parameter"] = None
            __props__.__dict__["end_time"] = None
            __props__.__dict__["instance_number"] = None
            __props__.__dict__["invocation_id"] = None
            __props__.__dict__["invocation_results"] = None
            __props__.__dict__["invocation_status"] = None
            __props__.__dict__["parameter_definitions"] = None
            __props__.__dict__["start_time"] = None
        super(Invocation, __self__).__init__(
            'volcenginecc:ecs/invocation:Invocation',
            resource_name,
            __props__,
            opts)

    @staticmethod
    def get(resource_name: str,
            id: pulumi.Input[str],
            opts: Optional[pulumi.ResourceOptions] = None,
            command_content: pulumi.Input[Optional[_builtins.str]] = None,
            command_description: pulumi.Input[Optional[_builtins.str]] = None,
            command_id: pulumi.Input[Optional[_builtins.str]] = None,
            command_name: pulumi.Input[Optional[_builtins.str]] = None,
            command_provider: pulumi.Input[Optional[_builtins.str]] = None,
            command_type: pulumi.Input[Optional[_builtins.str]] = None,
            enable_parameter: pulumi.Input[Optional[_builtins.bool]] = None,
            end_time: pulumi.Input[Optional[_builtins.str]] = None,
            frequency: pulumi.Input[Optional[_builtins.str]] = None,
            instance_ids: pulumi.Input[Optional[Sequence[pulumi.Input[_builtins.str]]]] = None,
            instance_number: pulumi.Input[Optional[_builtins.int]] = None,
            invocation_description: pulumi.Input[Optional[_builtins.str]] = None,
            invocation_id: pulumi.Input[Optional[_builtins.str]] = None,
            invocation_name: pulumi.Input[Optional[_builtins.str]] = None,
            invocation_results: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InvocationInvocationResultArgs', 'InvocationInvocationResultArgsDict']]]]] = None,
            invocation_status: pulumi.Input[Optional[_builtins.str]] = None,
            launch_time: pulumi.Input[Optional[_builtins.str]] = None,
            parameter_definitions: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InvocationParameterDefinitionArgs', 'InvocationParameterDefinitionArgsDict']]]]] = None,
            parameters: pulumi.Input[Optional[_builtins.str]] = None,
            project_name: pulumi.Input[Optional[_builtins.str]] = None,
            recurrence_end_time: pulumi.Input[Optional[_builtins.str]] = None,
            repeat_mode: pulumi.Input[Optional[_builtins.str]] = None,
            start_time: pulumi.Input[Optional[_builtins.str]] = None,
            tags: pulumi.Input[Optional[Sequence[pulumi.Input[Union['InvocationTagArgs', 'InvocationTagArgsDict']]]]] = None,
            timeout: pulumi.Input[Optional[_builtins.int]] = None,
            username: pulumi.Input[Optional[_builtins.str]] = None,
            windows_password: pulumi.Input[Optional[_builtins.str]] = None,
            working_dir: pulumi.Input[Optional[_builtins.str]] = None) -> 'Invocation':
        """
        Get an existing Invocation resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] command_content: Command content.
        :param pulumi.Input[_builtins.str] command_description: Description of the invoked command.
        :param pulumi.Input[_builtins.str] command_id: Command ID.
        :param pulumi.Input[_builtins.str] command_name: Command name when the task is triggered.
        :param pulumi.Input[_builtins.str] command_provider: Provider of the invoked command.
        :param pulumi.Input[_builtins.str] command_type: Command type triggered by the job.
        :param pulumi.Input[_builtins.bool] enable_parameter: Whether the invoked command uses custom parameters. true: enabled. false: not enabled.
        :param pulumi.Input[_builtins.str] end_time: Task end time.
        :param pulumi.Input[_builtins.str] frequency: Execution frequency. This parameter is required when RepeatMode=Rate. Format: \\n\\n\\n\\n. Interval units supported: minutes (5m–60m), hours (1h–24h), and days (1d–7d). The interval is based on a fixed frequency and is independent of the actual job execution time. After job creation, it executes once based on LaunchTime, and the next execution time is determined by the interval setting.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] instance_ids: Instance ID list, supports up to 200 IDs.
        :param pulumi.Input[_builtins.int] instance_number: Number of instances executed.
        :param pulumi.Input[_builtins.str] invocation_description: Job description. Defaults to an empty string and must not exceed 256 characters.
        :param pulumi.Input[_builtins.str] invocation_id: Task execution ID.
        :param pulumi.Input[_builtins.str] invocation_name: Job name. Maximum 64 characters. No restriction on special characters. Job names can be duplicated within the same region.
        :param pulumi.Input[Sequence[pulumi.Input[Union['InvocationInvocationResultArgs', 'InvocationInvocationResultArgsDict']]]] invocation_results: Result of command execution on a single instance.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] invocation_status: Overall execution status of the command.
        :param pulumi.Input[_builtins.str] launch_time: Execution time.
        :param pulumi.Input[Sequence[pulumi.Input[Union['InvocationParameterDefinitionArgs', 'InvocationParameterDefinitionArgsDict']]]] parameter_definitions: Custom parameter definition for the command triggered by the task.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] parameters: When the command includes custom parameters, use this parameter to pass custom parameter key-value pairs. The number of custom parameters ranges from 0 to 60. Key cannot be an empty string and supports up to 64 characters. Value can be an empty string. The combined length of custom parameters and original command content after Base64 encoding must not exceed 16 KB. The set of custom parameter names must be a subset of the parameter set defined when creating the command. Default values are used for parameters not provided.
        :param pulumi.Input[_builtins.str] project_name: Project to which the resource belongs; each resource can belong to only one project.
        :param pulumi.Input[_builtins.str] recurrence_end_time: Cycle end time, applicable only to periodic tasks (Rate).
        :param pulumi.Input[_builtins.str] repeat_mode: Set the command execution mode. Once: Default, executes the command immediately. Rate: Executes the command periodically; specify the execution cycle using the Frequency parameter and Rate expression. Fixed: Executes the command at a scheduled time; specify the execution time using the LaunchTime parameter.
        :param pulumi.Input[_builtins.str] start_time: Task start time.
        :param pulumi.Input[Sequence[pulumi.Input[Union['InvocationTagArgs', 'InvocationTagArgsDict']]]] tags: Tag key-value pair.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.int] timeout: Maximum timeout for command execution in ECS instances, in seconds. Default: 60. Optional range: 30–86400.
        :param pulumi.Input[_builtins.str] username: User name for executing the command in the ECS instance.
        :param pulumi.Input[_builtins.str] windows_password: Password for custom Windows user.
        :param pulumi.Input[_builtins.str] working_dir: Directory where the created command runs in the ECS instance.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _InvocationState.__new__(_InvocationState)

        __props__.__dict__["command_content"] = command_content
        __props__.__dict__["command_description"] = command_description
        __props__.__dict__["command_id"] = command_id
        __props__.__dict__["command_name"] = command_name
        __props__.__dict__["command_provider"] = command_provider
        __props__.__dict__["command_type"] = command_type
        __props__.__dict__["enable_parameter"] = enable_parameter
        __props__.__dict__["end_time"] = end_time
        __props__.__dict__["frequency"] = frequency
        __props__.__dict__["instance_ids"] = instance_ids
        __props__.__dict__["instance_number"] = instance_number
        __props__.__dict__["invocation_description"] = invocation_description
        __props__.__dict__["invocation_id"] = invocation_id
        __props__.__dict__["invocation_name"] = invocation_name
        __props__.__dict__["invocation_results"] = invocation_results
        __props__.__dict__["invocation_status"] = invocation_status
        __props__.__dict__["launch_time"] = launch_time
        __props__.__dict__["parameter_definitions"] = parameter_definitions
        __props__.__dict__["parameters"] = parameters
        __props__.__dict__["project_name"] = project_name
        __props__.__dict__["recurrence_end_time"] = recurrence_end_time
        __props__.__dict__["repeat_mode"] = repeat_mode
        __props__.__dict__["start_time"] = start_time
        __props__.__dict__["tags"] = tags
        __props__.__dict__["timeout"] = timeout
        __props__.__dict__["username"] = username
        __props__.__dict__["windows_password"] = windows_password
        __props__.__dict__["working_dir"] = working_dir
        return Invocation(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="commandContent")
    def command_content(self) -> pulumi.Output[_builtins.str]:
        """
        Command content.
        """
        return pulumi.get(self, "command_content")

    @_builtins.property
    @pulumi.getter(name="commandDescription")
    def command_description(self) -> pulumi.Output[_builtins.str]:
        """
        Description of the invoked command.
        """
        return pulumi.get(self, "command_description")

    @_builtins.property
    @pulumi.getter(name="commandId")
    def command_id(self) -> pulumi.Output[_builtins.str]:
        """
        Command ID.
        """
        return pulumi.get(self, "command_id")

    @_builtins.property
    @pulumi.getter(name="commandName")
    def command_name(self) -> pulumi.Output[_builtins.str]:
        """
        Command name when the task is triggered.
        """
        return pulumi.get(self, "command_name")

    @_builtins.property
    @pulumi.getter(name="commandProvider")
    def command_provider(self) -> pulumi.Output[_builtins.str]:
        """
        Provider of the invoked command.
        """
        return pulumi.get(self, "command_provider")

    @_builtins.property
    @pulumi.getter(name="commandType")
    def command_type(self) -> pulumi.Output[_builtins.str]:
        """
        Command type triggered by the job.
        """
        return pulumi.get(self, "command_type")

    @_builtins.property
    @pulumi.getter(name="enableParameter")
    def enable_parameter(self) -> pulumi.Output[_builtins.bool]:
        """
        Whether the invoked command uses custom parameters. true: enabled. false: not enabled.
        """
        return pulumi.get(self, "enable_parameter")

    @_builtins.property
    @pulumi.getter(name="endTime")
    def end_time(self) -> pulumi.Output[_builtins.str]:
        """
        Task end time.
        """
        return pulumi.get(self, "end_time")

    @_builtins.property
    @pulumi.getter
    def frequency(self) -> pulumi.Output[_builtins.str]:
        """
        Execution frequency. This parameter is required when RepeatMode=Rate. Format: \\n\\n\\n\\n. Interval units supported: minutes (5m–60m), hours (1h–24h), and days (1d–7d). The interval is based on a fixed frequency and is independent of the actual job execution time. After job creation, it executes once based on LaunchTime, and the next execution time is determined by the interval setting.
        """
        return pulumi.get(self, "frequency")

    @_builtins.property
    @pulumi.getter(name="instanceIds")
    def instance_ids(self) -> pulumi.Output[Sequence[_builtins.str]]:
        """
        Instance ID list, supports up to 200 IDs.
        """
        return pulumi.get(self, "instance_ids")

    @_builtins.property
    @pulumi.getter(name="instanceNumber")
    def instance_number(self) -> pulumi.Output[_builtins.int]:
        """
        Number of instances executed.
        """
        return pulumi.get(self, "instance_number")

    @_builtins.property
    @pulumi.getter(name="invocationDescription")
    def invocation_description(self) -> pulumi.Output[_builtins.str]:
        """
        Job description. Defaults to an empty string and must not exceed 256 characters.
        """
        return pulumi.get(self, "invocation_description")

    @_builtins.property
    @pulumi.getter(name="invocationId")
    def invocation_id(self) -> pulumi.Output[_builtins.str]:
        """
        Task execution ID.
        """
        return pulumi.get(self, "invocation_id")

    @_builtins.property
    @pulumi.getter(name="invocationName")
    def invocation_name(self) -> pulumi.Output[_builtins.str]:
        """
        Job name. Maximum 64 characters. No restriction on special characters. Job names can be duplicated within the same region.
        """
        return pulumi.get(self, "invocation_name")

    @_builtins.property
    @pulumi.getter(name="invocationResults")
    def invocation_results(self) -> pulumi.Output[Sequence['outputs.InvocationInvocationResult']]:
        """
        Result of command execution on a single instance.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "invocation_results")

    @_builtins.property
    @pulumi.getter(name="invocationStatus")
    def invocation_status(self) -> pulumi.Output[_builtins.str]:
        """
        Overall execution status of the command.
        """
        return pulumi.get(self, "invocation_status")

    @_builtins.property
    @pulumi.getter(name="launchTime")
    def launch_time(self) -> pulumi.Output[_builtins.str]:
        """
        Execution time.
        """
        return pulumi.get(self, "launch_time")

    @_builtins.property
    @pulumi.getter(name="parameterDefinitions")
    def parameter_definitions(self) -> pulumi.Output[Sequence['outputs.InvocationParameterDefinition']]:
        """
        Custom parameter definition for the command triggered by the task.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "parameter_definitions")

    @_builtins.property
    @pulumi.getter
    def parameters(self) -> pulumi.Output[_builtins.str]:
        """
        When the command includes custom parameters, use this parameter to pass custom parameter key-value pairs. The number of custom parameters ranges from 0 to 60. Key cannot be an empty string and supports up to 64 characters. Value can be an empty string. The combined length of custom parameters and original command content after Base64 encoding must not exceed 16 KB. The set of custom parameter names must be a subset of the parameter set defined when creating the command. Default values are used for parameters not provided.
        """
        return pulumi.get(self, "parameters")

    @_builtins.property
    @pulumi.getter(name="projectName")
    def project_name(self) -> pulumi.Output[_builtins.str]:
        """
        Project to which the resource belongs; each resource can belong to only one project.
        """
        return pulumi.get(self, "project_name")

    @_builtins.property
    @pulumi.getter(name="recurrenceEndTime")
    def recurrence_end_time(self) -> pulumi.Output[_builtins.str]:
        """
        Cycle end time, applicable only to periodic tasks (Rate).
        """
        return pulumi.get(self, "recurrence_end_time")

    @_builtins.property
    @pulumi.getter(name="repeatMode")
    def repeat_mode(self) -> pulumi.Output[_builtins.str]:
        """
        Set the command execution mode. Once: Default, executes the command immediately. Rate: Executes the command periodically; specify the execution cycle using the Frequency parameter and Rate expression. Fixed: Executes the command at a scheduled time; specify the execution time using the LaunchTime parameter.
        """
        return pulumi.get(self, "repeat_mode")

    @_builtins.property
    @pulumi.getter(name="startTime")
    def start_time(self) -> pulumi.Output[_builtins.str]:
        """
        Task start time.
        """
        return pulumi.get(self, "start_time")

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Output[Sequence['outputs.InvocationTag']]:
        """
        Tag key-value pair.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "tags")

    @_builtins.property
    @pulumi.getter
    def timeout(self) -> pulumi.Output[_builtins.int]:
        """
        Maximum timeout for command execution in ECS instances, in seconds. Default: 60. Optional range: 30–86400.
        """
        return pulumi.get(self, "timeout")

    @_builtins.property
    @pulumi.getter
    def username(self) -> pulumi.Output[_builtins.str]:
        """
        User name for executing the command in the ECS instance.
        """
        return pulumi.get(self, "username")

    @_builtins.property
    @pulumi.getter(name="windowsPassword")
    def windows_password(self) -> pulumi.Output[_builtins.str]:
        """
        Password for custom Windows user.
        """
        return pulumi.get(self, "windows_password")

    @_builtins.property
    @pulumi.getter(name="workingDir")
    def working_dir(self) -> pulumi.Output[_builtins.str]:
        """
        Directory where the created command runs in the ECS instance.
        """
        return pulumi.get(self, "working_dir")

