# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = ['VpnGatewayArgs', 'VpnGateway']

@pulumi.input_type
class VpnGatewayArgs:
    def __init__(__self__, *,
                 bandwidth: pulumi.Input[_builtins.int],
                 subnet_id: pulumi.Input[_builtins.str],
                 vpc_id: pulumi.Input[_builtins.str],
                 asn: pulumi.Input[Optional[_builtins.int]] = None,
                 billing_type: pulumi.Input[Optional[_builtins.int]] = None,
                 description: pulumi.Input[Optional[_builtins.str]] = None,
                 dual_tunnel_enabled: pulumi.Input[Optional[_builtins.bool]] = None,
                 ip_stack_type: pulumi.Input[Optional[_builtins.str]] = None,
                 ip_version: pulumi.Input[Optional[_builtins.str]] = None,
                 ipsec_enabled: pulumi.Input[Optional[_builtins.bool]] = None,
                 period: pulumi.Input[Optional[_builtins.int]] = None,
                 period_unit: pulumi.Input[Optional[_builtins.str]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 secondary_subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
                 ssl_enabled: pulumi.Input[Optional[_builtins.bool]] = None,
                 ssl_max_connections: pulumi.Input[Optional[_builtins.int]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input['VpnGatewayTagArgs']]]] = None,
                 vpn_gateway_name: pulumi.Input[Optional[_builtins.str]] = None):
        """
        The set of arguments for constructing a VpnGateway resource.

        :param pulumi.Input[_builtins.int] bandwidth: VPN gateway bandwidth specification.
        :param pulumi.Input[_builtins.str] subnet_id: ID of the subnet to which the port used by the VPN gateway belongs.
        :param pulumi.Input[_builtins.str] vpc_id: VPC ID where the VPN gateway is located.
        :param pulumi.Input[_builtins.int] asn: ASN of the VPN gateway.
        :param pulumi.Input[_builtins.int] billing_type: VPN gateway billing type. 1: Subscription. 2: Pay-as-you-go (fixed specification)
        :param pulumi.Input[_builtins.str] description: Description of the VPN gateway.
        :param pulumi.Input[_builtins.bool] dual_tunnel_enabled: Whether dual tunnel mode is enabled for the VPN gateway. true: Enabled false: Disabled
        :param pulumi.Input[_builtins.str] ip_stack_type: VPN gateway protocol type. ipv4*only: IPv4 protocol type. ipv6*only: IPv6 protocol type. ipv4_ipv6: Dual-stack protocol type.
        :param pulumi.Input[_builtins.str] ip_version: IP address type of the VPN gateway. ipv4: IPv4 address type. ipv6: IPv6 address type.
        :param pulumi.Input[_builtins.bool] ipsec_enabled: Enable IPSec feature. true: enabled false: disabled
        :param pulumi.Input[_builtins.int] period: Subscription VPN gateway purchase duration, default is '1'. If PeriodUnit is Month, Period value range: 1–9, 12, 24, 36. If PeriodUnit is Year, Period value range: 1–3.
        :param pulumi.Input[_builtins.str] period_unit: Unit for the purchase duration of subscription-based VPN gateways. Default is 'Month'. Month: month Year: year
        :param pulumi.Input[_builtins.str] project_name: Project to which the VPN gateway belongs. If not specified, defaults to 'default', meaning the created resource belongs to the default project. You can call ListProjects to query project information under the current account.
        :param pulumi.Input[_builtins.str] secondary_subnet_id: ID of the second subnet associated with the VPN gateway instance. This parameter is returned only for VPN gateway instances in dual tunnel mode; it is not returned for instances in single tunnel mode.
        :param pulumi.Input[_builtins.bool] ssl_enabled: Enable SSL VPN feature. true: enabled false: disabled
        :param pulumi.Input[_builtins.int] ssl_max_connections: Maximum SSL connections for the VPN gateway.
        :param pulumi.Input[Sequence[pulumi.Input['VpnGatewayTagArgs']]] tags: All tag information added to the VPN gateway.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] vpn_gateway_name: Name of the VPN gateway.
        """
        pulumi.set(__self__, "bandwidth", bandwidth)
        pulumi.set(__self__, "subnet_id", subnet_id)
        pulumi.set(__self__, "vpc_id", vpc_id)
        if asn is not None:
            pulumi.set(__self__, "asn", asn)
        if billing_type is not None:
            pulumi.set(__self__, "billing_type", billing_type)
        if description is not None:
            pulumi.set(__self__, "description", description)
        if dual_tunnel_enabled is not None:
            pulumi.set(__self__, "dual_tunnel_enabled", dual_tunnel_enabled)
        if ip_stack_type is not None:
            pulumi.set(__self__, "ip_stack_type", ip_stack_type)
        if ip_version is not None:
            pulumi.set(__self__, "ip_version", ip_version)
        if ipsec_enabled is not None:
            pulumi.set(__self__, "ipsec_enabled", ipsec_enabled)
        if period is not None:
            pulumi.set(__self__, "period", period)
        if period_unit is not None:
            pulumi.set(__self__, "period_unit", period_unit)
        if project_name is not None:
            pulumi.set(__self__, "project_name", project_name)
        if secondary_subnet_id is not None:
            pulumi.set(__self__, "secondary_subnet_id", secondary_subnet_id)
        if ssl_enabled is not None:
            pulumi.set(__self__, "ssl_enabled", ssl_enabled)
        if ssl_max_connections is not None:
            pulumi.set(__self__, "ssl_max_connections", ssl_max_connections)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)
        if vpn_gateway_name is not None:
            pulumi.set(__self__, "vpn_gateway_name", vpn_gateway_name)

    @_builtins.property
    @pulumi.getter
    def bandwidth(self) -> pulumi.Input[_builtins.int]:
        """
        VPN gateway bandwidth specification.
        """
        return pulumi.get(self, "bandwidth")

    @bandwidth.setter
    def bandwidth(self, value: pulumi.Input[_builtins.int]):
        pulumi.set(self, "bandwidth", value)

    @_builtins.property
    @pulumi.getter(name="subnetId")
    def subnet_id(self) -> pulumi.Input[_builtins.str]:
        """
        ID of the subnet to which the port used by the VPN gateway belongs.
        """
        return pulumi.get(self, "subnet_id")

    @subnet_id.setter
    def subnet_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "subnet_id", value)

    @_builtins.property
    @pulumi.getter(name="vpcId")
    def vpc_id(self) -> pulumi.Input[_builtins.str]:
        """
        VPC ID where the VPN gateway is located.
        """
        return pulumi.get(self, "vpc_id")

    @vpc_id.setter
    def vpc_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "vpc_id", value)

    @_builtins.property
    @pulumi.getter
    def asn(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        ASN of the VPN gateway.
        """
        return pulumi.get(self, "asn")

    @asn.setter
    def asn(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "asn", value)

    @_builtins.property
    @pulumi.getter(name="billingType")
    def billing_type(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        VPN gateway billing type. 1: Subscription. 2: Pay-as-you-go (fixed specification)
        """
        return pulumi.get(self, "billing_type")

    @billing_type.setter
    def billing_type(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "billing_type", value)

    @_builtins.property
    @pulumi.getter
    def description(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Description of the VPN gateway.
        """
        return pulumi.get(self, "description")

    @description.setter
    def description(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "description", value)

    @_builtins.property
    @pulumi.getter(name="dualTunnelEnabled")
    def dual_tunnel_enabled(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Whether dual tunnel mode is enabled for the VPN gateway. true: Enabled false: Disabled
        """
        return pulumi.get(self, "dual_tunnel_enabled")

    @dual_tunnel_enabled.setter
    def dual_tunnel_enabled(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "dual_tunnel_enabled", value)

    @_builtins.property
    @pulumi.getter(name="ipStackType")
    def ip_stack_type(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        VPN gateway protocol type. ipv4*only: IPv4 protocol type. ipv6*only: IPv6 protocol type. ipv4_ipv6: Dual-stack protocol type.
        """
        return pulumi.get(self, "ip_stack_type")

    @ip_stack_type.setter
    def ip_stack_type(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "ip_stack_type", value)

    @_builtins.property
    @pulumi.getter(name="ipVersion")
    def ip_version(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        IP address type of the VPN gateway. ipv4: IPv4 address type. ipv6: IPv6 address type.
        """
        return pulumi.get(self, "ip_version")

    @ip_version.setter
    def ip_version(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "ip_version", value)

    @_builtins.property
    @pulumi.getter(name="ipsecEnabled")
    def ipsec_enabled(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Enable IPSec feature. true: enabled false: disabled
        """
        return pulumi.get(self, "ipsec_enabled")

    @ipsec_enabled.setter
    def ipsec_enabled(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "ipsec_enabled", value)

    @_builtins.property
    @pulumi.getter
    def period(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Subscription VPN gateway purchase duration, default is '1'. If PeriodUnit is Month, Period value range: 1–9, 12, 24, 36. If PeriodUnit is Year, Period value range: 1–3.
        """
        return pulumi.get(self, "period")

    @period.setter
    def period(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "period", value)

    @_builtins.property
    @pulumi.getter(name="periodUnit")
    def period_unit(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Unit for the purchase duration of subscription-based VPN gateways. Default is 'Month'. Month: month Year: year
        """
        return pulumi.get(self, "period_unit")

    @period_unit.setter
    def period_unit(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "period_unit", value)

    @_builtins.property
    @pulumi.getter(name="projectName")
    def project_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Project to which the VPN gateway belongs. If not specified, defaults to 'default', meaning the created resource belongs to the default project. You can call ListProjects to query project information under the current account.
        """
        return pulumi.get(self, "project_name")

    @project_name.setter
    def project_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "project_name", value)

    @_builtins.property
    @pulumi.getter(name="secondarySubnetId")
    def secondary_subnet_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        ID of the second subnet associated with the VPN gateway instance. This parameter is returned only for VPN gateway instances in dual tunnel mode; it is not returned for instances in single tunnel mode.
        """
        return pulumi.get(self, "secondary_subnet_id")

    @secondary_subnet_id.setter
    def secondary_subnet_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "secondary_subnet_id", value)

    @_builtins.property
    @pulumi.getter(name="sslEnabled")
    def ssl_enabled(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Enable SSL VPN feature. true: enabled false: disabled
        """
        return pulumi.get(self, "ssl_enabled")

    @ssl_enabled.setter
    def ssl_enabled(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "ssl_enabled", value)

    @_builtins.property
    @pulumi.getter(name="sslMaxConnections")
    def ssl_max_connections(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Maximum SSL connections for the VPN gateway.
        """
        return pulumi.get(self, "ssl_max_connections")

    @ssl_max_connections.setter
    def ssl_max_connections(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "ssl_max_connections", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['VpnGatewayTagArgs']]]]:
        """
        All tag information added to the VPN gateway.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['VpnGatewayTagArgs']]]]):
        pulumi.set(self, "tags", value)

    @_builtins.property
    @pulumi.getter(name="vpnGatewayName")
    def vpn_gateway_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Name of the VPN gateway.
        """
        return pulumi.get(self, "vpn_gateway_name")

    @vpn_gateway_name.setter
    def vpn_gateway_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "vpn_gateway_name", value)


@pulumi.input_type
class _VpnGatewayState:
    def __init__(__self__, *,
                 asn: pulumi.Input[Optional[_builtins.int]] = None,
                 bandwidth: pulumi.Input[Optional[_builtins.int]] = None,
                 billing_type: pulumi.Input[Optional[_builtins.int]] = None,
                 business_status: pulumi.Input[Optional[_builtins.str]] = None,
                 connection_count: pulumi.Input[Optional[_builtins.int]] = None,
                 create_time: pulumi.Input[Optional[_builtins.str]] = None,
                 deleted_time: pulumi.Input[Optional[_builtins.str]] = None,
                 description: pulumi.Input[Optional[_builtins.str]] = None,
                 dual_tunnel_enabled: pulumi.Input[Optional[_builtins.bool]] = None,
                 expired_time: pulumi.Input[Optional[_builtins.str]] = None,
                 ip_address: pulumi.Input[Optional[_builtins.str]] = None,
                 ip_stack_type: pulumi.Input[Optional[_builtins.str]] = None,
                 ip_version: pulumi.Input[Optional[_builtins.str]] = None,
                 ipsec_enabled: pulumi.Input[Optional[_builtins.bool]] = None,
                 is_blocked: pulumi.Input[Optional[_builtins.bool]] = None,
                 lock_reason: pulumi.Input[Optional[_builtins.str]] = None,
                 overdue_time: pulumi.Input[Optional[_builtins.str]] = None,
                 period: pulumi.Input[Optional[_builtins.int]] = None,
                 period_unit: pulumi.Input[Optional[_builtins.str]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 route_count: pulumi.Input[Optional[_builtins.int]] = None,
                 secondary_ip_address: pulumi.Input[Optional[_builtins.str]] = None,
                 secondary_subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
                 ssl_enabled: pulumi.Input[Optional[_builtins.bool]] = None,
                 ssl_max_connections: pulumi.Input[Optional[_builtins.int]] = None,
                 status: pulumi.Input[Optional[_builtins.str]] = None,
                 subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input['VpnGatewayTagArgs']]]] = None,
                 update_time: pulumi.Input[Optional[_builtins.str]] = None,
                 vpc_id: pulumi.Input[Optional[_builtins.str]] = None,
                 vpn_gateway_id: pulumi.Input[Optional[_builtins.str]] = None,
                 vpn_gateway_name: pulumi.Input[Optional[_builtins.str]] = None):
        """
        Input properties used for looking up and filtering VpnGateway resources.

        :param pulumi.Input[_builtins.int] asn: ASN of the VPN gateway.
        :param pulumi.Input[_builtins.int] bandwidth: VPN gateway bandwidth specification.
        :param pulumi.Input[_builtins.int] billing_type: VPN gateway billing type. 1: Subscription. 2: Pay-as-you-go (fixed specification)
        :param pulumi.Input[_builtins.str] business_status: VPN gateway billing status. Normal: Normal. FinancialLocked: Locked.
        :param pulumi.Input[_builtins.int] connection_count: Number of IPsec connections associated with the VPN gateway.
        :param pulumi.Input[_builtins.str] create_time: Time when the VPN gateway was created.
        :param pulumi.Input[_builtins.str] deleted_time: Expected resource forced reclaim time.
        :param pulumi.Input[_builtins.str] description: Description of the VPN gateway.
        :param pulumi.Input[_builtins.bool] dual_tunnel_enabled: Whether dual tunnel mode is enabled for the VPN gateway. true: Enabled false: Disabled
        :param pulumi.Input[_builtins.str] expired_time: VPN gateway expiration time.
        :param pulumi.Input[_builtins.str] ip_address: VPN gateway public IP address.
        :param pulumi.Input[_builtins.str] ip_stack_type: VPN gateway protocol type. ipv4*only: IPv4 protocol type. ipv6*only: IPv6 protocol type. ipv4_ipv6: Dual-stack protocol type.
        :param pulumi.Input[_builtins.str] ip_version: IP address type of the VPN gateway. ipv4: IPv4 address type. ipv6: IPv6 address type.
        :param pulumi.Input[_builtins.bool] ipsec_enabled: Enable IPSec feature. true: enabled false: disabled
        :param pulumi.Input[_builtins.bool] is_blocked: Whether the current resource is banned. true: Banned false: Not banned.
        :param pulumi.Input[_builtins.str] lock_reason: VPN gateway lock reasons. financial: Locked due to overdue payment. security: Locked for security reasons. unlock: Not locked.
        :param pulumi.Input[_builtins.str] overdue_time: Resource freeze time. This parameter returns a value only if the resource is frozen due to overdue payment; otherwise, it returns an empty value.
        :param pulumi.Input[_builtins.int] period: Subscription VPN gateway purchase duration, default is '1'. If PeriodUnit is Month, Period value range: 1–9, 12, 24, 36. If PeriodUnit is Year, Period value range: 1–3.
        :param pulumi.Input[_builtins.str] period_unit: Unit for the purchase duration of subscription-based VPN gateways. Default is 'Month'. Month: month Year: year
        :param pulumi.Input[_builtins.str] project_name: Project to which the VPN gateway belongs. If not specified, defaults to 'default', meaning the created resource belongs to the default project. You can call ListProjects to query project information under the current account.
        :param pulumi.Input[_builtins.int] route_count: Number of route entries for the VPN gateway.
        :param pulumi.Input[_builtins.str] secondary_ip_address: Second public IP address of the VPN gateway. This parameter is returned only for VPN gateway instances in dual tunnel mode; it is not returned for instances in single tunnel mode.
        :param pulumi.Input[_builtins.str] secondary_subnet_id: ID of the second subnet associated with the VPN gateway instance. This parameter is returned only for VPN gateway instances in dual tunnel mode; it is not returned for instances in single tunnel mode.
        :param pulumi.Input[_builtins.bool] ssl_enabled: Enable SSL VPN feature. true: enabled false: disabled
        :param pulumi.Input[_builtins.int] ssl_max_connections: Maximum SSL connections for the VPN gateway.
        :param pulumi.Input[_builtins.str] status: Status of the VPN gateway. Creating: creating Deleting: deleting Pending: configuring Available: available
        :param pulumi.Input[_builtins.str] subnet_id: ID of the subnet to which the port used by the VPN gateway belongs.
        :param pulumi.Input[Sequence[pulumi.Input['VpnGatewayTagArgs']]] tags: All tag information added to the VPN gateway.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] update_time: VPN gateway update time.
        :param pulumi.Input[_builtins.str] vpc_id: VPC ID where the VPN gateway is located.
        :param pulumi.Input[_builtins.str] vpn_gateway_id: ID of the VPN gateway.
        :param pulumi.Input[_builtins.str] vpn_gateway_name: Name of the VPN gateway.
        """
        if asn is not None:
            pulumi.set(__self__, "asn", asn)
        if bandwidth is not None:
            pulumi.set(__self__, "bandwidth", bandwidth)
        if billing_type is not None:
            pulumi.set(__self__, "billing_type", billing_type)
        if business_status is not None:
            pulumi.set(__self__, "business_status", business_status)
        if connection_count is not None:
            pulumi.set(__self__, "connection_count", connection_count)
        if create_time is not None:
            pulumi.set(__self__, "create_time", create_time)
        if deleted_time is not None:
            pulumi.set(__self__, "deleted_time", deleted_time)
        if description is not None:
            pulumi.set(__self__, "description", description)
        if dual_tunnel_enabled is not None:
            pulumi.set(__self__, "dual_tunnel_enabled", dual_tunnel_enabled)
        if expired_time is not None:
            pulumi.set(__self__, "expired_time", expired_time)
        if ip_address is not None:
            pulumi.set(__self__, "ip_address", ip_address)
        if ip_stack_type is not None:
            pulumi.set(__self__, "ip_stack_type", ip_stack_type)
        if ip_version is not None:
            pulumi.set(__self__, "ip_version", ip_version)
        if ipsec_enabled is not None:
            pulumi.set(__self__, "ipsec_enabled", ipsec_enabled)
        if is_blocked is not None:
            pulumi.set(__self__, "is_blocked", is_blocked)
        if lock_reason is not None:
            pulumi.set(__self__, "lock_reason", lock_reason)
        if overdue_time is not None:
            pulumi.set(__self__, "overdue_time", overdue_time)
        if period is not None:
            pulumi.set(__self__, "period", period)
        if period_unit is not None:
            pulumi.set(__self__, "period_unit", period_unit)
        if project_name is not None:
            pulumi.set(__self__, "project_name", project_name)
        if route_count is not None:
            pulumi.set(__self__, "route_count", route_count)
        if secondary_ip_address is not None:
            pulumi.set(__self__, "secondary_ip_address", secondary_ip_address)
        if secondary_subnet_id is not None:
            pulumi.set(__self__, "secondary_subnet_id", secondary_subnet_id)
        if ssl_enabled is not None:
            pulumi.set(__self__, "ssl_enabled", ssl_enabled)
        if ssl_max_connections is not None:
            pulumi.set(__self__, "ssl_max_connections", ssl_max_connections)
        if status is not None:
            pulumi.set(__self__, "status", status)
        if subnet_id is not None:
            pulumi.set(__self__, "subnet_id", subnet_id)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)
        if update_time is not None:
            pulumi.set(__self__, "update_time", update_time)
        if vpc_id is not None:
            pulumi.set(__self__, "vpc_id", vpc_id)
        if vpn_gateway_id is not None:
            pulumi.set(__self__, "vpn_gateway_id", vpn_gateway_id)
        if vpn_gateway_name is not None:
            pulumi.set(__self__, "vpn_gateway_name", vpn_gateway_name)

    @_builtins.property
    @pulumi.getter
    def asn(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        ASN of the VPN gateway.
        """
        return pulumi.get(self, "asn")

    @asn.setter
    def asn(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "asn", value)

    @_builtins.property
    @pulumi.getter
    def bandwidth(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        VPN gateway bandwidth specification.
        """
        return pulumi.get(self, "bandwidth")

    @bandwidth.setter
    def bandwidth(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "bandwidth", value)

    @_builtins.property
    @pulumi.getter(name="billingType")
    def billing_type(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        VPN gateway billing type. 1: Subscription. 2: Pay-as-you-go (fixed specification)
        """
        return pulumi.get(self, "billing_type")

    @billing_type.setter
    def billing_type(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "billing_type", value)

    @_builtins.property
    @pulumi.getter(name="businessStatus")
    def business_status(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        VPN gateway billing status. Normal: Normal. FinancialLocked: Locked.
        """
        return pulumi.get(self, "business_status")

    @business_status.setter
    def business_status(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "business_status", value)

    @_builtins.property
    @pulumi.getter(name="connectionCount")
    def connection_count(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Number of IPsec connections associated with the VPN gateway.
        """
        return pulumi.get(self, "connection_count")

    @connection_count.setter
    def connection_count(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "connection_count", value)

    @_builtins.property
    @pulumi.getter(name="createTime")
    def create_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Time when the VPN gateway was created.
        """
        return pulumi.get(self, "create_time")

    @create_time.setter
    def create_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "create_time", value)

    @_builtins.property
    @pulumi.getter(name="deletedTime")
    def deleted_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Expected resource forced reclaim time.
        """
        return pulumi.get(self, "deleted_time")

    @deleted_time.setter
    def deleted_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "deleted_time", value)

    @_builtins.property
    @pulumi.getter
    def description(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Description of the VPN gateway.
        """
        return pulumi.get(self, "description")

    @description.setter
    def description(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "description", value)

    @_builtins.property
    @pulumi.getter(name="dualTunnelEnabled")
    def dual_tunnel_enabled(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Whether dual tunnel mode is enabled for the VPN gateway. true: Enabled false: Disabled
        """
        return pulumi.get(self, "dual_tunnel_enabled")

    @dual_tunnel_enabled.setter
    def dual_tunnel_enabled(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "dual_tunnel_enabled", value)

    @_builtins.property
    @pulumi.getter(name="expiredTime")
    def expired_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        VPN gateway expiration time.
        """
        return pulumi.get(self, "expired_time")

    @expired_time.setter
    def expired_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "expired_time", value)

    @_builtins.property
    @pulumi.getter(name="ipAddress")
    def ip_address(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        VPN gateway public IP address.
        """
        return pulumi.get(self, "ip_address")

    @ip_address.setter
    def ip_address(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "ip_address", value)

    @_builtins.property
    @pulumi.getter(name="ipStackType")
    def ip_stack_type(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        VPN gateway protocol type. ipv4*only: IPv4 protocol type. ipv6*only: IPv6 protocol type. ipv4_ipv6: Dual-stack protocol type.
        """
        return pulumi.get(self, "ip_stack_type")

    @ip_stack_type.setter
    def ip_stack_type(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "ip_stack_type", value)

    @_builtins.property
    @pulumi.getter(name="ipVersion")
    def ip_version(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        IP address type of the VPN gateway. ipv4: IPv4 address type. ipv6: IPv6 address type.
        """
        return pulumi.get(self, "ip_version")

    @ip_version.setter
    def ip_version(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "ip_version", value)

    @_builtins.property
    @pulumi.getter(name="ipsecEnabled")
    def ipsec_enabled(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Enable IPSec feature. true: enabled false: disabled
        """
        return pulumi.get(self, "ipsec_enabled")

    @ipsec_enabled.setter
    def ipsec_enabled(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "ipsec_enabled", value)

    @_builtins.property
    @pulumi.getter(name="isBlocked")
    def is_blocked(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Whether the current resource is banned. true: Banned false: Not banned.
        """
        return pulumi.get(self, "is_blocked")

    @is_blocked.setter
    def is_blocked(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "is_blocked", value)

    @_builtins.property
    @pulumi.getter(name="lockReason")
    def lock_reason(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        VPN gateway lock reasons. financial: Locked due to overdue payment. security: Locked for security reasons. unlock: Not locked.
        """
        return pulumi.get(self, "lock_reason")

    @lock_reason.setter
    def lock_reason(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "lock_reason", value)

    @_builtins.property
    @pulumi.getter(name="overdueTime")
    def overdue_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Resource freeze time. This parameter returns a value only if the resource is frozen due to overdue payment; otherwise, it returns an empty value.
        """
        return pulumi.get(self, "overdue_time")

    @overdue_time.setter
    def overdue_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "overdue_time", value)

    @_builtins.property
    @pulumi.getter
    def period(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Subscription VPN gateway purchase duration, default is '1'. If PeriodUnit is Month, Period value range: 1–9, 12, 24, 36. If PeriodUnit is Year, Period value range: 1–3.
        """
        return pulumi.get(self, "period")

    @period.setter
    def period(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "period", value)

    @_builtins.property
    @pulumi.getter(name="periodUnit")
    def period_unit(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Unit for the purchase duration of subscription-based VPN gateways. Default is 'Month'. Month: month Year: year
        """
        return pulumi.get(self, "period_unit")

    @period_unit.setter
    def period_unit(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "period_unit", value)

    @_builtins.property
    @pulumi.getter(name="projectName")
    def project_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Project to which the VPN gateway belongs. If not specified, defaults to 'default', meaning the created resource belongs to the default project. You can call ListProjects to query project information under the current account.
        """
        return pulumi.get(self, "project_name")

    @project_name.setter
    def project_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "project_name", value)

    @_builtins.property
    @pulumi.getter(name="routeCount")
    def route_count(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Number of route entries for the VPN gateway.
        """
        return pulumi.get(self, "route_count")

    @route_count.setter
    def route_count(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "route_count", value)

    @_builtins.property
    @pulumi.getter(name="secondaryIpAddress")
    def secondary_ip_address(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Second public IP address of the VPN gateway. This parameter is returned only for VPN gateway instances in dual tunnel mode; it is not returned for instances in single tunnel mode.
        """
        return pulumi.get(self, "secondary_ip_address")

    @secondary_ip_address.setter
    def secondary_ip_address(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "secondary_ip_address", value)

    @_builtins.property
    @pulumi.getter(name="secondarySubnetId")
    def secondary_subnet_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        ID of the second subnet associated with the VPN gateway instance. This parameter is returned only for VPN gateway instances in dual tunnel mode; it is not returned for instances in single tunnel mode.
        """
        return pulumi.get(self, "secondary_subnet_id")

    @secondary_subnet_id.setter
    def secondary_subnet_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "secondary_subnet_id", value)

    @_builtins.property
    @pulumi.getter(name="sslEnabled")
    def ssl_enabled(self) -> pulumi.Input[Optional[_builtins.bool]]:
        """
        Enable SSL VPN feature. true: enabled false: disabled
        """
        return pulumi.get(self, "ssl_enabled")

    @ssl_enabled.setter
    def ssl_enabled(self, value: pulumi.Input[Optional[_builtins.bool]]):
        pulumi.set(self, "ssl_enabled", value)

    @_builtins.property
    @pulumi.getter(name="sslMaxConnections")
    def ssl_max_connections(self) -> pulumi.Input[Optional[_builtins.int]]:
        """
        Maximum SSL connections for the VPN gateway.
        """
        return pulumi.get(self, "ssl_max_connections")

    @ssl_max_connections.setter
    def ssl_max_connections(self, value: pulumi.Input[Optional[_builtins.int]]):
        pulumi.set(self, "ssl_max_connections", value)

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Status of the VPN gateway. Creating: creating Deleting: deleting Pending: configuring Available: available
        """
        return pulumi.get(self, "status")

    @status.setter
    def status(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "status", value)

    @_builtins.property
    @pulumi.getter(name="subnetId")
    def subnet_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        ID of the subnet to which the port used by the VPN gateway belongs.
        """
        return pulumi.get(self, "subnet_id")

    @subnet_id.setter
    def subnet_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "subnet_id", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['VpnGatewayTagArgs']]]]:
        """
        All tag information added to the VPN gateway.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['VpnGatewayTagArgs']]]]):
        pulumi.set(self, "tags", value)

    @_builtins.property
    @pulumi.getter(name="updateTime")
    def update_time(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        VPN gateway update time.
        """
        return pulumi.get(self, "update_time")

    @update_time.setter
    def update_time(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "update_time", value)

    @_builtins.property
    @pulumi.getter(name="vpcId")
    def vpc_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        VPC ID where the VPN gateway is located.
        """
        return pulumi.get(self, "vpc_id")

    @vpc_id.setter
    def vpc_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "vpc_id", value)

    @_builtins.property
    @pulumi.getter(name="vpnGatewayId")
    def vpn_gateway_id(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        ID of the VPN gateway.
        """
        return pulumi.get(self, "vpn_gateway_id")

    @vpn_gateway_id.setter
    def vpn_gateway_id(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "vpn_gateway_id", value)

    @_builtins.property
    @pulumi.getter(name="vpnGatewayName")
    def vpn_gateway_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        Name of the VPN gateway.
        """
        return pulumi.get(self, "vpn_gateway_name")

    @vpn_gateway_name.setter
    def vpn_gateway_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "vpn_gateway_name", value)


@pulumi.type_token("volcenginecc:vpn/vpnGateway:VpnGateway")
class VpnGateway(pulumi.CustomResource):
    @overload
    def __init__(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 asn: pulumi.Input[Optional[_builtins.int]] = None,
                 bandwidth: pulumi.Input[Optional[_builtins.int]] = None,
                 billing_type: pulumi.Input[Optional[_builtins.int]] = None,
                 description: pulumi.Input[Optional[_builtins.str]] = None,
                 dual_tunnel_enabled: pulumi.Input[Optional[_builtins.bool]] = None,
                 ip_stack_type: pulumi.Input[Optional[_builtins.str]] = None,
                 ip_version: pulumi.Input[Optional[_builtins.str]] = None,
                 ipsec_enabled: pulumi.Input[Optional[_builtins.bool]] = None,
                 period: pulumi.Input[Optional[_builtins.int]] = None,
                 period_unit: pulumi.Input[Optional[_builtins.str]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 secondary_subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
                 ssl_enabled: pulumi.Input[Optional[_builtins.bool]] = None,
                 ssl_max_connections: pulumi.Input[Optional[_builtins.int]] = None,
                 subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input[Union['VpnGatewayTagArgs', 'VpnGatewayTagArgsDict']]]]] = None,
                 vpc_id: pulumi.Input[Optional[_builtins.str]] = None,
                 vpn_gateway_name: pulumi.Input[Optional[_builtins.str]] = None,
                 __props__=None):
        """
        To enable communication between your private network and a local data center or another private network via a VPN connection, you must first create a VPN gateway.

        ## Example Usage

        ```python
        import pulumi
        import pulumi_volcenginecc as volcenginecc

        vpn_vpn_gateway_demo = volcenginecc.vpn.VpnGateway("VpnVpnGatewayDemo",
            bandwidth=5,
            billing_type=2,
            description="this is a test description",
            dual_tunnel_enabled=False,
            ip_stack_type="ipv4_only",
            ip_version="ipv4",
            vpn_gateway_name="VpnVpnGatewayDemo",
            ipsec_enabled=True,
            ssl_enabled=True,
            ssl_max_connections=5,
            project_name="default",
            vpc_id="vpc-3nr6adxxxxu8931eb64y4z2",
            subnet_id="subnet-btepcsxxxxw5h0b2u6hppyd")
        ```

        ## Import

        ```sh
        $ pulumi import volcenginecc:vpn/vpnGateway:VpnGateway example "vpn_gateway_id"
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.int] asn: ASN of the VPN gateway.
        :param pulumi.Input[_builtins.int] bandwidth: VPN gateway bandwidth specification.
        :param pulumi.Input[_builtins.int] billing_type: VPN gateway billing type. 1: Subscription. 2: Pay-as-you-go (fixed specification)
        :param pulumi.Input[_builtins.str] description: Description of the VPN gateway.
        :param pulumi.Input[_builtins.bool] dual_tunnel_enabled: Whether dual tunnel mode is enabled for the VPN gateway. true: Enabled false: Disabled
        :param pulumi.Input[_builtins.str] ip_stack_type: VPN gateway protocol type. ipv4*only: IPv4 protocol type. ipv6*only: IPv6 protocol type. ipv4_ipv6: Dual-stack protocol type.
        :param pulumi.Input[_builtins.str] ip_version: IP address type of the VPN gateway. ipv4: IPv4 address type. ipv6: IPv6 address type.
        :param pulumi.Input[_builtins.bool] ipsec_enabled: Enable IPSec feature. true: enabled false: disabled
        :param pulumi.Input[_builtins.int] period: Subscription VPN gateway purchase duration, default is '1'. If PeriodUnit is Month, Period value range: 1–9, 12, 24, 36. If PeriodUnit is Year, Period value range: 1–3.
        :param pulumi.Input[_builtins.str] period_unit: Unit for the purchase duration of subscription-based VPN gateways. Default is 'Month'. Month: month Year: year
        :param pulumi.Input[_builtins.str] project_name: Project to which the VPN gateway belongs. If not specified, defaults to 'default', meaning the created resource belongs to the default project. You can call ListProjects to query project information under the current account.
        :param pulumi.Input[_builtins.str] secondary_subnet_id: ID of the second subnet associated with the VPN gateway instance. This parameter is returned only for VPN gateway instances in dual tunnel mode; it is not returned for instances in single tunnel mode.
        :param pulumi.Input[_builtins.bool] ssl_enabled: Enable SSL VPN feature. true: enabled false: disabled
        :param pulumi.Input[_builtins.int] ssl_max_connections: Maximum SSL connections for the VPN gateway.
        :param pulumi.Input[_builtins.str] subnet_id: ID of the subnet to which the port used by the VPN gateway belongs.
        :param pulumi.Input[Sequence[pulumi.Input[Union['VpnGatewayTagArgs', 'VpnGatewayTagArgsDict']]]] tags: All tag information added to the VPN gateway.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] vpc_id: VPC ID where the VPN gateway is located.
        :param pulumi.Input[_builtins.str] vpn_gateway_name: Name of the VPN gateway.
        """
        ...
    @overload
    def __init__(__self__,
                 resource_name: str,
                 args: VpnGatewayArgs,
                 opts: Optional[pulumi.ResourceOptions] = None):
        """
        To enable communication between your private network and a local data center or another private network via a VPN connection, you must first create a VPN gateway.

        ## Example Usage

        ```python
        import pulumi
        import pulumi_volcenginecc as volcenginecc

        vpn_vpn_gateway_demo = volcenginecc.vpn.VpnGateway("VpnVpnGatewayDemo",
            bandwidth=5,
            billing_type=2,
            description="this is a test description",
            dual_tunnel_enabled=False,
            ip_stack_type="ipv4_only",
            ip_version="ipv4",
            vpn_gateway_name="VpnVpnGatewayDemo",
            ipsec_enabled=True,
            ssl_enabled=True,
            ssl_max_connections=5,
            project_name="default",
            vpc_id="vpc-3nr6adxxxxu8931eb64y4z2",
            subnet_id="subnet-btepcsxxxxw5h0b2u6hppyd")
        ```

        ## Import

        ```sh
        $ pulumi import volcenginecc:vpn/vpnGateway:VpnGateway example "vpn_gateway_id"
        ```


        :param str resource_name: The name of the resource.
        :param VpnGatewayArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        ...
    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(VpnGatewayArgs, pulumi.ResourceOptions, *args, **kwargs)
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 asn: pulumi.Input[Optional[_builtins.int]] = None,
                 bandwidth: pulumi.Input[Optional[_builtins.int]] = None,
                 billing_type: pulumi.Input[Optional[_builtins.int]] = None,
                 description: pulumi.Input[Optional[_builtins.str]] = None,
                 dual_tunnel_enabled: pulumi.Input[Optional[_builtins.bool]] = None,
                 ip_stack_type: pulumi.Input[Optional[_builtins.str]] = None,
                 ip_version: pulumi.Input[Optional[_builtins.str]] = None,
                 ipsec_enabled: pulumi.Input[Optional[_builtins.bool]] = None,
                 period: pulumi.Input[Optional[_builtins.int]] = None,
                 period_unit: pulumi.Input[Optional[_builtins.str]] = None,
                 project_name: pulumi.Input[Optional[_builtins.str]] = None,
                 secondary_subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
                 ssl_enabled: pulumi.Input[Optional[_builtins.bool]] = None,
                 ssl_max_connections: pulumi.Input[Optional[_builtins.int]] = None,
                 subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input[Union['VpnGatewayTagArgs', 'VpnGatewayTagArgsDict']]]]] = None,
                 vpc_id: pulumi.Input[Optional[_builtins.str]] = None,
                 vpn_gateway_name: pulumi.Input[Optional[_builtins.str]] = None,
                 __props__=None):
        opts = pulumi.ResourceOptions.merge(_utilities.get_resource_opts_defaults(), opts)
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError('Expected resource options to be a ResourceOptions instance')
        if opts.id is None:
            if __props__ is not None:
                raise TypeError('__props__ is only valid when passed in combination with a valid opts.id to get an existing resource')
            __props__ = VpnGatewayArgs.__new__(VpnGatewayArgs)

            __props__.__dict__["asn"] = asn
            if bandwidth is None and not opts.urn:
                raise TypeError("Missing required property 'bandwidth'")
            __props__.__dict__["bandwidth"] = bandwidth
            __props__.__dict__["billing_type"] = billing_type
            __props__.__dict__["description"] = description
            __props__.__dict__["dual_tunnel_enabled"] = dual_tunnel_enabled
            __props__.__dict__["ip_stack_type"] = ip_stack_type
            __props__.__dict__["ip_version"] = ip_version
            __props__.__dict__["ipsec_enabled"] = ipsec_enabled
            __props__.__dict__["period"] = period
            __props__.__dict__["period_unit"] = period_unit
            __props__.__dict__["project_name"] = project_name
            __props__.__dict__["secondary_subnet_id"] = secondary_subnet_id
            __props__.__dict__["ssl_enabled"] = ssl_enabled
            __props__.__dict__["ssl_max_connections"] = ssl_max_connections
            if subnet_id is None and not opts.urn:
                raise TypeError("Missing required property 'subnet_id'")
            __props__.__dict__["subnet_id"] = subnet_id
            __props__.__dict__["tags"] = tags
            if vpc_id is None and not opts.urn:
                raise TypeError("Missing required property 'vpc_id'")
            __props__.__dict__["vpc_id"] = vpc_id
            __props__.__dict__["vpn_gateway_name"] = vpn_gateway_name
            __props__.__dict__["business_status"] = None
            __props__.__dict__["connection_count"] = None
            __props__.__dict__["create_time"] = None
            __props__.__dict__["deleted_time"] = None
            __props__.__dict__["expired_time"] = None
            __props__.__dict__["ip_address"] = None
            __props__.__dict__["is_blocked"] = None
            __props__.__dict__["lock_reason"] = None
            __props__.__dict__["overdue_time"] = None
            __props__.__dict__["route_count"] = None
            __props__.__dict__["secondary_ip_address"] = None
            __props__.__dict__["status"] = None
            __props__.__dict__["update_time"] = None
            __props__.__dict__["vpn_gateway_id"] = None
        super(VpnGateway, __self__).__init__(
            'volcenginecc:vpn/vpnGateway:VpnGateway',
            resource_name,
            __props__,
            opts)

    @staticmethod
    def get(resource_name: str,
            id: pulumi.Input[str],
            opts: Optional[pulumi.ResourceOptions] = None,
            asn: pulumi.Input[Optional[_builtins.int]] = None,
            bandwidth: pulumi.Input[Optional[_builtins.int]] = None,
            billing_type: pulumi.Input[Optional[_builtins.int]] = None,
            business_status: pulumi.Input[Optional[_builtins.str]] = None,
            connection_count: pulumi.Input[Optional[_builtins.int]] = None,
            create_time: pulumi.Input[Optional[_builtins.str]] = None,
            deleted_time: pulumi.Input[Optional[_builtins.str]] = None,
            description: pulumi.Input[Optional[_builtins.str]] = None,
            dual_tunnel_enabled: pulumi.Input[Optional[_builtins.bool]] = None,
            expired_time: pulumi.Input[Optional[_builtins.str]] = None,
            ip_address: pulumi.Input[Optional[_builtins.str]] = None,
            ip_stack_type: pulumi.Input[Optional[_builtins.str]] = None,
            ip_version: pulumi.Input[Optional[_builtins.str]] = None,
            ipsec_enabled: pulumi.Input[Optional[_builtins.bool]] = None,
            is_blocked: pulumi.Input[Optional[_builtins.bool]] = None,
            lock_reason: pulumi.Input[Optional[_builtins.str]] = None,
            overdue_time: pulumi.Input[Optional[_builtins.str]] = None,
            period: pulumi.Input[Optional[_builtins.int]] = None,
            period_unit: pulumi.Input[Optional[_builtins.str]] = None,
            project_name: pulumi.Input[Optional[_builtins.str]] = None,
            route_count: pulumi.Input[Optional[_builtins.int]] = None,
            secondary_ip_address: pulumi.Input[Optional[_builtins.str]] = None,
            secondary_subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
            ssl_enabled: pulumi.Input[Optional[_builtins.bool]] = None,
            ssl_max_connections: pulumi.Input[Optional[_builtins.int]] = None,
            status: pulumi.Input[Optional[_builtins.str]] = None,
            subnet_id: pulumi.Input[Optional[_builtins.str]] = None,
            tags: pulumi.Input[Optional[Sequence[pulumi.Input[Union['VpnGatewayTagArgs', 'VpnGatewayTagArgsDict']]]]] = None,
            update_time: pulumi.Input[Optional[_builtins.str]] = None,
            vpc_id: pulumi.Input[Optional[_builtins.str]] = None,
            vpn_gateway_id: pulumi.Input[Optional[_builtins.str]] = None,
            vpn_gateway_name: pulumi.Input[Optional[_builtins.str]] = None) -> 'VpnGateway':
        """
        Get an existing VpnGateway resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.int] asn: ASN of the VPN gateway.
        :param pulumi.Input[_builtins.int] bandwidth: VPN gateway bandwidth specification.
        :param pulumi.Input[_builtins.int] billing_type: VPN gateway billing type. 1: Subscription. 2: Pay-as-you-go (fixed specification)
        :param pulumi.Input[_builtins.str] business_status: VPN gateway billing status. Normal: Normal. FinancialLocked: Locked.
        :param pulumi.Input[_builtins.int] connection_count: Number of IPsec connections associated with the VPN gateway.
        :param pulumi.Input[_builtins.str] create_time: Time when the VPN gateway was created.
        :param pulumi.Input[_builtins.str] deleted_time: Expected resource forced reclaim time.
        :param pulumi.Input[_builtins.str] description: Description of the VPN gateway.
        :param pulumi.Input[_builtins.bool] dual_tunnel_enabled: Whether dual tunnel mode is enabled for the VPN gateway. true: Enabled false: Disabled
        :param pulumi.Input[_builtins.str] expired_time: VPN gateway expiration time.
        :param pulumi.Input[_builtins.str] ip_address: VPN gateway public IP address.
        :param pulumi.Input[_builtins.str] ip_stack_type: VPN gateway protocol type. ipv4*only: IPv4 protocol type. ipv6*only: IPv6 protocol type. ipv4_ipv6: Dual-stack protocol type.
        :param pulumi.Input[_builtins.str] ip_version: IP address type of the VPN gateway. ipv4: IPv4 address type. ipv6: IPv6 address type.
        :param pulumi.Input[_builtins.bool] ipsec_enabled: Enable IPSec feature. true: enabled false: disabled
        :param pulumi.Input[_builtins.bool] is_blocked: Whether the current resource is banned. true: Banned false: Not banned.
        :param pulumi.Input[_builtins.str] lock_reason: VPN gateway lock reasons. financial: Locked due to overdue payment. security: Locked for security reasons. unlock: Not locked.
        :param pulumi.Input[_builtins.str] overdue_time: Resource freeze time. This parameter returns a value only if the resource is frozen due to overdue payment; otherwise, it returns an empty value.
        :param pulumi.Input[_builtins.int] period: Subscription VPN gateway purchase duration, default is '1'. If PeriodUnit is Month, Period value range: 1–9, 12, 24, 36. If PeriodUnit is Year, Period value range: 1–3.
        :param pulumi.Input[_builtins.str] period_unit: Unit for the purchase duration of subscription-based VPN gateways. Default is 'Month'. Month: month Year: year
        :param pulumi.Input[_builtins.str] project_name: Project to which the VPN gateway belongs. If not specified, defaults to 'default', meaning the created resource belongs to the default project. You can call ListProjects to query project information under the current account.
        :param pulumi.Input[_builtins.int] route_count: Number of route entries for the VPN gateway.
        :param pulumi.Input[_builtins.str] secondary_ip_address: Second public IP address of the VPN gateway. This parameter is returned only for VPN gateway instances in dual tunnel mode; it is not returned for instances in single tunnel mode.
        :param pulumi.Input[_builtins.str] secondary_subnet_id: ID of the second subnet associated with the VPN gateway instance. This parameter is returned only for VPN gateway instances in dual tunnel mode; it is not returned for instances in single tunnel mode.
        :param pulumi.Input[_builtins.bool] ssl_enabled: Enable SSL VPN feature. true: enabled false: disabled
        :param pulumi.Input[_builtins.int] ssl_max_connections: Maximum SSL connections for the VPN gateway.
        :param pulumi.Input[_builtins.str] status: Status of the VPN gateway. Creating: creating Deleting: deleting Pending: configuring Available: available
        :param pulumi.Input[_builtins.str] subnet_id: ID of the subnet to which the port used by the VPN gateway belongs.
        :param pulumi.Input[Sequence[pulumi.Input[Union['VpnGatewayTagArgs', 'VpnGatewayTagArgsDict']]]] tags: All tag information added to the VPN gateway.
               Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        :param pulumi.Input[_builtins.str] update_time: VPN gateway update time.
        :param pulumi.Input[_builtins.str] vpc_id: VPC ID where the VPN gateway is located.
        :param pulumi.Input[_builtins.str] vpn_gateway_id: ID of the VPN gateway.
        :param pulumi.Input[_builtins.str] vpn_gateway_name: Name of the VPN gateway.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _VpnGatewayState.__new__(_VpnGatewayState)

        __props__.__dict__["asn"] = asn
        __props__.__dict__["bandwidth"] = bandwidth
        __props__.__dict__["billing_type"] = billing_type
        __props__.__dict__["business_status"] = business_status
        __props__.__dict__["connection_count"] = connection_count
        __props__.__dict__["create_time"] = create_time
        __props__.__dict__["deleted_time"] = deleted_time
        __props__.__dict__["description"] = description
        __props__.__dict__["dual_tunnel_enabled"] = dual_tunnel_enabled
        __props__.__dict__["expired_time"] = expired_time
        __props__.__dict__["ip_address"] = ip_address
        __props__.__dict__["ip_stack_type"] = ip_stack_type
        __props__.__dict__["ip_version"] = ip_version
        __props__.__dict__["ipsec_enabled"] = ipsec_enabled
        __props__.__dict__["is_blocked"] = is_blocked
        __props__.__dict__["lock_reason"] = lock_reason
        __props__.__dict__["overdue_time"] = overdue_time
        __props__.__dict__["period"] = period
        __props__.__dict__["period_unit"] = period_unit
        __props__.__dict__["project_name"] = project_name
        __props__.__dict__["route_count"] = route_count
        __props__.__dict__["secondary_ip_address"] = secondary_ip_address
        __props__.__dict__["secondary_subnet_id"] = secondary_subnet_id
        __props__.__dict__["ssl_enabled"] = ssl_enabled
        __props__.__dict__["ssl_max_connections"] = ssl_max_connections
        __props__.__dict__["status"] = status
        __props__.__dict__["subnet_id"] = subnet_id
        __props__.__dict__["tags"] = tags
        __props__.__dict__["update_time"] = update_time
        __props__.__dict__["vpc_id"] = vpc_id
        __props__.__dict__["vpn_gateway_id"] = vpn_gateway_id
        __props__.__dict__["vpn_gateway_name"] = vpn_gateway_name
        return VpnGateway(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter
    def asn(self) -> pulumi.Output[_builtins.int]:
        """
        ASN of the VPN gateway.
        """
        return pulumi.get(self, "asn")

    @_builtins.property
    @pulumi.getter
    def bandwidth(self) -> pulumi.Output[_builtins.int]:
        """
        VPN gateway bandwidth specification.
        """
        return pulumi.get(self, "bandwidth")

    @_builtins.property
    @pulumi.getter(name="billingType")
    def billing_type(self) -> pulumi.Output[_builtins.int]:
        """
        VPN gateway billing type. 1: Subscription. 2: Pay-as-you-go (fixed specification)
        """
        return pulumi.get(self, "billing_type")

    @_builtins.property
    @pulumi.getter(name="businessStatus")
    def business_status(self) -> pulumi.Output[_builtins.str]:
        """
        VPN gateway billing status. Normal: Normal. FinancialLocked: Locked.
        """
        return pulumi.get(self, "business_status")

    @_builtins.property
    @pulumi.getter(name="connectionCount")
    def connection_count(self) -> pulumi.Output[_builtins.int]:
        """
        Number of IPsec connections associated with the VPN gateway.
        """
        return pulumi.get(self, "connection_count")

    @_builtins.property
    @pulumi.getter(name="createTime")
    def create_time(self) -> pulumi.Output[_builtins.str]:
        """
        Time when the VPN gateway was created.
        """
        return pulumi.get(self, "create_time")

    @_builtins.property
    @pulumi.getter(name="deletedTime")
    def deleted_time(self) -> pulumi.Output[_builtins.str]:
        """
        Expected resource forced reclaim time.
        """
        return pulumi.get(self, "deleted_time")

    @_builtins.property
    @pulumi.getter
    def description(self) -> pulumi.Output[_builtins.str]:
        """
        Description of the VPN gateway.
        """
        return pulumi.get(self, "description")

    @_builtins.property
    @pulumi.getter(name="dualTunnelEnabled")
    def dual_tunnel_enabled(self) -> pulumi.Output[_builtins.bool]:
        """
        Whether dual tunnel mode is enabled for the VPN gateway. true: Enabled false: Disabled
        """
        return pulumi.get(self, "dual_tunnel_enabled")

    @_builtins.property
    @pulumi.getter(name="expiredTime")
    def expired_time(self) -> pulumi.Output[_builtins.str]:
        """
        VPN gateway expiration time.
        """
        return pulumi.get(self, "expired_time")

    @_builtins.property
    @pulumi.getter(name="ipAddress")
    def ip_address(self) -> pulumi.Output[_builtins.str]:
        """
        VPN gateway public IP address.
        """
        return pulumi.get(self, "ip_address")

    @_builtins.property
    @pulumi.getter(name="ipStackType")
    def ip_stack_type(self) -> pulumi.Output[_builtins.str]:
        """
        VPN gateway protocol type. ipv4*only: IPv4 protocol type. ipv6*only: IPv6 protocol type. ipv4_ipv6: Dual-stack protocol type.
        """
        return pulumi.get(self, "ip_stack_type")

    @_builtins.property
    @pulumi.getter(name="ipVersion")
    def ip_version(self) -> pulumi.Output[_builtins.str]:
        """
        IP address type of the VPN gateway. ipv4: IPv4 address type. ipv6: IPv6 address type.
        """
        return pulumi.get(self, "ip_version")

    @_builtins.property
    @pulumi.getter(name="ipsecEnabled")
    def ipsec_enabled(self) -> pulumi.Output[_builtins.bool]:
        """
        Enable IPSec feature. true: enabled false: disabled
        """
        return pulumi.get(self, "ipsec_enabled")

    @_builtins.property
    @pulumi.getter(name="isBlocked")
    def is_blocked(self) -> pulumi.Output[_builtins.bool]:
        """
        Whether the current resource is banned. true: Banned false: Not banned.
        """
        return pulumi.get(self, "is_blocked")

    @_builtins.property
    @pulumi.getter(name="lockReason")
    def lock_reason(self) -> pulumi.Output[_builtins.str]:
        """
        VPN gateway lock reasons. financial: Locked due to overdue payment. security: Locked for security reasons. unlock: Not locked.
        """
        return pulumi.get(self, "lock_reason")

    @_builtins.property
    @pulumi.getter(name="overdueTime")
    def overdue_time(self) -> pulumi.Output[_builtins.str]:
        """
        Resource freeze time. This parameter returns a value only if the resource is frozen due to overdue payment; otherwise, it returns an empty value.
        """
        return pulumi.get(self, "overdue_time")

    @_builtins.property
    @pulumi.getter
    def period(self) -> pulumi.Output[_builtins.int]:
        """
        Subscription VPN gateway purchase duration, default is '1'. If PeriodUnit is Month, Period value range: 1–9, 12, 24, 36. If PeriodUnit is Year, Period value range: 1–3.
        """
        return pulumi.get(self, "period")

    @_builtins.property
    @pulumi.getter(name="periodUnit")
    def period_unit(self) -> pulumi.Output[_builtins.str]:
        """
        Unit for the purchase duration of subscription-based VPN gateways. Default is 'Month'. Month: month Year: year
        """
        return pulumi.get(self, "period_unit")

    @_builtins.property
    @pulumi.getter(name="projectName")
    def project_name(self) -> pulumi.Output[_builtins.str]:
        """
        Project to which the VPN gateway belongs. If not specified, defaults to 'default', meaning the created resource belongs to the default project. You can call ListProjects to query project information under the current account.
        """
        return pulumi.get(self, "project_name")

    @_builtins.property
    @pulumi.getter(name="routeCount")
    def route_count(self) -> pulumi.Output[_builtins.int]:
        """
        Number of route entries for the VPN gateway.
        """
        return pulumi.get(self, "route_count")

    @_builtins.property
    @pulumi.getter(name="secondaryIpAddress")
    def secondary_ip_address(self) -> pulumi.Output[_builtins.str]:
        """
        Second public IP address of the VPN gateway. This parameter is returned only for VPN gateway instances in dual tunnel mode; it is not returned for instances in single tunnel mode.
        """
        return pulumi.get(self, "secondary_ip_address")

    @_builtins.property
    @pulumi.getter(name="secondarySubnetId")
    def secondary_subnet_id(self) -> pulumi.Output[_builtins.str]:
        """
        ID of the second subnet associated with the VPN gateway instance. This parameter is returned only for VPN gateway instances in dual tunnel mode; it is not returned for instances in single tunnel mode.
        """
        return pulumi.get(self, "secondary_subnet_id")

    @_builtins.property
    @pulumi.getter(name="sslEnabled")
    def ssl_enabled(self) -> pulumi.Output[_builtins.bool]:
        """
        Enable SSL VPN feature. true: enabled false: disabled
        """
        return pulumi.get(self, "ssl_enabled")

    @_builtins.property
    @pulumi.getter(name="sslMaxConnections")
    def ssl_max_connections(self) -> pulumi.Output[_builtins.int]:
        """
        Maximum SSL connections for the VPN gateway.
        """
        return pulumi.get(self, "ssl_max_connections")

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Output[_builtins.str]:
        """
        Status of the VPN gateway. Creating: creating Deleting: deleting Pending: configuring Available: available
        """
        return pulumi.get(self, "status")

    @_builtins.property
    @pulumi.getter(name="subnetId")
    def subnet_id(self) -> pulumi.Output[_builtins.str]:
        """
        ID of the subnet to which the port used by the VPN gateway belongs.
        """
        return pulumi.get(self, "subnet_id")

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Output[Sequence['outputs.VpnGatewayTag']]:
        """
        All tag information added to the VPN gateway.
        Important Note: When using SetNestedAttribute, you must fully define all attributes of its nested structure. Incomplete definitions may cause Terraform to detect unexpected differences during plan comparison, triggering unnecessary resource updates and affecting resource stability and predictability.
        """
        return pulumi.get(self, "tags")

    @_builtins.property
    @pulumi.getter(name="updateTime")
    def update_time(self) -> pulumi.Output[_builtins.str]:
        """
        VPN gateway update time.
        """
        return pulumi.get(self, "update_time")

    @_builtins.property
    @pulumi.getter(name="vpcId")
    def vpc_id(self) -> pulumi.Output[_builtins.str]:
        """
        VPC ID where the VPN gateway is located.
        """
        return pulumi.get(self, "vpc_id")

    @_builtins.property
    @pulumi.getter(name="vpnGatewayId")
    def vpn_gateway_id(self) -> pulumi.Output[_builtins.str]:
        """
        ID of the VPN gateway.
        """
        return pulumi.get(self, "vpn_gateway_id")

    @_builtins.property
    @pulumi.getter(name="vpnGatewayName")
    def vpn_gateway_name(self) -> pulumi.Output[_builtins.str]:
        """
        Name of the VPN gateway.
        """
        return pulumi.get(self, "vpn_gateway_name")

